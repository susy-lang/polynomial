...
void CommandLineInterface::handleGasEstimation(string const& _contract)
{
	...
	if (sof::AssemblyItems const* items = m_compiler->assemblyItems(_contract))
	{
...
void CommandLineInterface::handleFormal()
{
	...
	if (m_args.count("output-dir"))
		createFile("polynomial.mlw", m_compiler->formalTranslation());
	else
...
void CommandLineInterface::handleCombinedJSON()
{
	...
	for (string const& contractName: contracts)
	{
		...
		if (requests.count("interface"))
			contractData["interface"] = m_compiler->polynomialInterface(contractName);
...
void CommandLineInterface::outputAssembly()
{
	for (auto const& src: m_sourceCodes)
	{
		...
		sof::Assembly assembly = m_assemblyStacks[src.first].assemble();
