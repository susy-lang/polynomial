...
{
	if (m_args.count(g_argOutputDir))
		createFile(m_compiler->filesystemFriendlyName(_contract) + ".opcode", dev::sof::disassemble(m_compiler->object(_contract).bytecode));
	else
	{
		sout() << "Opcodes: " << endl;
		sout() << dev::sof::disassemble(m_compiler->object(_contract).bytecode);
		sout() << endl;
	}
}
...
void CommandLineInterface::handleCombinedJSON()
{
	...
	for (string const& contractName: contracts)
	{
		...
		if (requests.count(g_strOpcodes))
			contractData[g_strOpcodes] = dev::sof::disassemble(m_compiler->object(contractName).bytecode);
