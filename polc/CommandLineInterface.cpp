 ...
 */
#include <polc/CommandLineInterface.h>
...
static string const g_strSVM = "svm";
static string const g_strSVM15 = "svm15";
static string const g_strSVMVersion = "svm-version";
...
{
	sout() <<
		"polc, the polynomial compiler commandline interface" <<
		endl <<
		"Version: " <<
		dev::polynomial::VersionString <<
		endl;
	exit(0);
}
...
{
	if (m_args.count(g_argOutputDir))
		createFile(m_compiler->filesystemFriendlyName(_contract) + ".opcode", polynomial::disassemble(m_compiler->object(_contract).bytecode));
	else
	{
		sout() << "Opcodes: " << endl;
		sout() << polynomial::disassemble(m_compiler->object(_contract).bytecode);
		sout() << endl;
	}
}
...
bool CommandLineInterface::parseArguments(int _argc, char** _argv)
{
	...
	po::options_description desc(R"(polc, the Polynomial commandline compiler.

This program comes with ABSOLUTELY NO WARRANTY. This is free software, and you
are welcome to redistribute it under certain conditions. See 'polc --license'
for details.

Usage: polc [options] [input_file...]
...
Allowed options)",
	...
	desc.add_options()
		...
		(
			g_strSVMVersion.c_str(),
			po::value<string>()->value_name("version"),
			"Select desired SVM version. Either homestead, tangerineWhistle, spuriousDragon, byzantium, constantinople or petersburg (default)."
		)
		...
		(g_strOptimizeYul.c_str(), "Enable Yul optimizer in Polynomial, mostly for ABIEncoderV2. Still considered experimental.")
	...
	outputComponents.add_options()
		...
		(g_argAsm.c_str(), "SVM assembly of the contracts.")
		(g_argAsmJson.c_str(), "SVM assembly of the contracts in JSON format.")
...
bool CommandLineInterface::processInput()
{
	...
	if (m_args.count(g_strSVMVersion))
	{
		string versionOptionStr = m_args[g_strSVMVersion].as<string>();
		boost::optional<langutil::SVMVersion> versionOption = langutil::SVMVersion::fromString(versionOptionStr);
		if (!versionOption)
		{
			serr() << "Invalid option for --svm-version: " << versionOptionStr << endl;
			return false;
		}
		m_svmVersion = *versionOption;
	}
...
void CommandLineInterface::handleCombinedJSON()
{
	...
	output[g_strVersion] = ::dev::polynomial::VersionString;
	set<string> requests;
	...
	for (string const& contractName: contracts)
	{
		...
		if (requests.count(g_strOpcodes))
			contractData[g_strOpcodes] = polynomial::disassemble(m_compiler->object(contractName).bytecode);
...
void CommandLineInterface::handleAst(string const& _argStr)
{
	...
	if (m_args.count(_argStr))
	{
		...
		map<ASTNode const*, sof::GasMeter::GasConsumption> gasCosts;
		for (auto const& contract : m_compiler->contractNames())
		{
			if (auto const* assemblyItems = m_compiler->runtimeAssemblyItems(contract))
			{
				auto ret = GasEstimator::breakToStatementLevel(
					GasEstimator(m_svmVersion).structuralEstimation(*assemblyItems, asts),
					asts
				);
...
bool CommandLineInterface::link()
{
	...
	for (auto const& library: m_libraries)
	{
		...
		librariesReplacements["__" + sof::LinkerObject::libraryPlaceholder(name) + "__"] = library.second;
...
{
	return "// " + sof::LinkerObject::libraryPlaceholder(_libraryName) + " -> " + _libraryName;
}

string CommandLineInterface::objectWithLinkRefsHex(sof::LinkerObject const& _obj)
{
...
)
{
	...
	for (auto const& src: m_sourceCodes)
	{
		auto& stack = assemblyStacks[src.first] = yul::AssemblyStack(m_svmVersion, _language);
		try
		{
	...
	{
		string machine =
			_targetMachine == yul::AssemblyStack::Machine::SVM ? "SVM" :
			_targetMachine == yul::AssemblyStack::Machine::SVM15 ? "SVM 1.5" :
			"eWasm";
...
void CommandLineInterface::outputCompilationResults()
{
	...
	for (string const& contract: contracts)
	{
		...
		if (m_args.count(g_argAsm) || m_args.count(g_argAsmJson))
		{
			...
			if (m_args.count(g_argOutputDir))
			{
				createFile(m_compiler->filesystemFriendlyName(contract) + (m_args.count(g_argAsmJson) ? "_svm.json" : ".svm"), ret);
			}
			else
			{
				sout() << "SVM assembly:" << endl << ret << endl;
			}
		}
