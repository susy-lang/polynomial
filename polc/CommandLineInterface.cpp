...
static string const g_argPolInterfaceStr = "interface";
...
static bool needsHumanTargetedStdout(po::variables_map const& _args)
{
	...
	return
		...
		humanTargetedStdout(_args, g_argPolInterfaceStr) ||
...
{
	if (m_args.count("output-dir"))
		createFile(_contract + ".opcode", sof::disassemble(m_compiler->getBytecode(_contract)));
	else
	{
		cout << "Opcodes: " << endl;
		cout << sof::disassemble(m_compiler->getBytecode(_contract));
		cout << endl;
	}
...
void CommandLineInterface::handleMeta(DocumentationType _type, string const& _contract)
{
	...
	case DocumentationType::ABIPolynomialInterface:
		argName = g_argPolInterfaceStr;
		suffix = "_interface.pol";
		title = "Contract Polynomial ABI";
		break;
...
void CommandLineInterface::handleGasEstimation(string const& _contract)
{
	...
	if (sof::AssemblyItems const* items = m_compiler->getAssemblyItems(_contract))
	{
	...
	if (sof::AssemblyItems const* items = m_compiler->getRuntimeAssemblyItems(_contract))
	{
...
bool CommandLineInterface::parseArguments(int _argc, char** _argv)
{
	...
	po::options_description desc(
		R"(polc, the Polynomial commandline compiler.
Usage: polc [options] [input_file...]
Compiles the given Polynomial input files (or the standard input if none given) and
...
Allowed options)",
	...
	outputComponents.add_options()
		...
		(g_argAsmStr.c_str(), "SVM assembly of the contracts.")
		(g_argAsmJsonStr.c_str(), "SVM assembly of the contracts in JSON format.")
		...
		(g_argPolInterfaceStr.c_str(), "Polynomial interface of the contracts.")
...
void CommandLineInterface::handleCombinedJSON()
{
	...
	for (string const& contractName: contracts)
	{
		...
		if (requests.count("interface"))
			contractData["interface"] = m_compiler->getPolynomialInterface(contractName);
		...
		if (requests.count("opcodes"))
			contractData["opcodes"] = sof::disassemble(m_compiler->getBytecode(contractName));
	...
	for (string const& contract: contracts)
	{
		...
		if (m_args.count(g_argAsmStr) || m_args.count(g_argAsmJsonStr))
		{
			if (m_args.count("output-dir"))
			{
				...
				createFile(contract + (m_args.count(g_argAsmJsonStr) ? "_svm.json" : ".svm"), data.str());
			}
			else
			{
				cout << "SVM assembly:" << endl;
		...
		handleMeta(DocumentationType::ABIPolynomialInterface, contract);
