...
Allowed options)",
	...
	desc.add_options()
		...
		(
			...
			"Select desired SVM version. Either homestead, tangerineWhistle, spuriousDragon, byzantium (default) or constantinople."
		)
...
bool CommandLineInterface::processInput()
{
	...
	if (m_args.count(g_strSVMVersion))
	{
		...
		boost::optional<SVMVersion> versionOption = SVMVersion::fromString(versionOptionStr);
		if (!versionOption)
		{
			cerr << "Invalid option for --svm-version: " << versionOptionStr << endl;
			return false;
		}
	...
	auto scannerFromSourceName = [&](string const& _sourceName) -> polynomial::Scanner const& { return m_compiler->scanner(_sourceName); };
	...
	if (m_args.count(_argStr))
	{
		...
		for (auto const& contract : m_compiler->contractNames())
		{
			auto ret = GasEstimator::breakToStatementLevel(
				GasEstimator(m_svmVersion).structuralEstimation(*m_compiler->runtimeAssemblyItems(contract), asts),
				asts
			);
...
)
{
	...
	for (auto const& src: m_sourceCodes)
	{
		auto& stack = assemblyStacks[src.first] = AssemblyStack(m_svmVersion, _language);
		try
		{
	...
	{
		string machine =
			_targetMachine == AssemblyStack::Machine::SVM ? "SVM" :
			_targetMachine == AssemblyStack::Machine::SVM15 ? "SVM 1.5" :
			"eWasm";
...
void CommandLineInterface::outputCompilationResults()
{
	...
	for (string const& contract: contracts)
	{
		...
		if (m_args.count(g_argAsm) || m_args.count(g_argAsmJson))
		{
			...
			{
				cout << "SVM assembly:" << endl << ret << endl;
			}
		}
