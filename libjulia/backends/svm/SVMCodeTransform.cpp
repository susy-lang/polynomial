...
{
	if (m_svm15 && (
		_instruction.instruction.instruction == polynomial::Instruction::JUMP ||
		_instruction.instruction.instruction == polynomial::Instruction::JUMPI
	))
	{
		bool const isJumpI = _instruction.instruction.instruction == polynomial::Instruction::JUMPI;
		if (isJumpI)
		{
...
void CodeTransform::operator()(FunctionDefinition const& _function)
{
	...
	CodeTransform(m_assembly, m_info, m_julia, m_svm15, m_identifierAccess, localStackAdjustment, m_context)
		(_function.body);
...
int CodeTransform::variableHeightDiff(polynomial::assembly::Scope::Variable const& _var, bool _forSwap)
{
