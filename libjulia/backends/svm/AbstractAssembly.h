...
///
/// Assembly class that abstracts both the libsvmasm assembly and the new julia svm assembly.
///
...
public:
	...
	virtual void appendInstruction(polynomial::Instruction _instruction) = 0;
	/// Append a constant.
...
{
	using Resolver = std::function<size_t(polynomial::assembly::Identifier const&, IdentifierContext, bool /*_crossesFunctionBoundary*/)>;
	/// Resolve a an external reference given by the identifier in the given context.
	/// @returns the size of the value (number of stack slots) or size_t(-1) if not found.
	Resolver resolve;
	using CodeGenerator = std::function<void(polynomial::assembly::Identifier const&, IdentifierContext, julia::AbstractAssembly&)>;
