...
void ArrayUtils::copyArrayToStorage(ArrayType const& _targetType, ArrayType const& _sourceType) const
{
	...
	{
		polAssert(targetBaseType->getCategory() == Type::Category::Mapping, "");
	...
	else
	{
		...
		m_context << sof::dupInstruction(2 + byteOffsetSize + sourceBaseType->getSizeOnStack());
		if (haveByteOffsetTarget)
			m_context << sof::dupInstruction(1 + byteOffsetSize + sourceBaseType->getSizeOnStack());
		else
			m_context << u256(0);
...
void ArrayUtils::clearArray(ArrayType const& _type) const
{
	...
	{
		polAssert(_type.getBaseType()->isValueType(), "Invalid storage size for non-value type.");
		polAssert(_type.getBaseType()->getStorageSize() <= 1, "Invalid storage size for type.");
	}
	if (_type.getBaseType()->isValueType())
		polAssert(_type.getBaseType()->getStorageSize() <= 1, "Invalid size for value type.");
	...
	{
		// unroll loop for small arrays @todo choose a good value
		polAssert(_type.getBaseType()->getStorageBytes() >= 32, "Invalid storage size.");
		for (unsigned i = 1; i < _type.getLength(); ++i)
		{
			...
			m_context
				<< sof::Instruction::POP
				<< u256(_type.getBaseType()->getStorageSize()) << sof::Instruction::ADD;
		}
		m_context << u256(0);
	...
	{
		m_context << sof::Instruction::DUP1 << _type.getLength();
	...
	polAssert(m_context.getStackHeight() == stackHeightStart - 2, "");
}
...
void ArrayUtils::clearDynamicArray(ArrayType const& _type) const
{
	...
	// cleanup
	m_context << sof::Instruction::POP;
	polAssert(m_context.getStackHeight() == stackHeightStart - 1, "");
}
...
void ArrayUtils::convertLengthToSize(ArrayType const& _arrayType, bool _pad) const
{
	if (_arrayType.location() == DataLocation::Storage)
	{
		if (_arrayType.getBaseType()->getStorageSize() <= 1)
		{
			...
			m_context << _arrayType.getBaseType()->getStorageSize() << sof::Instruction::MUL;
	}
	else
	{
