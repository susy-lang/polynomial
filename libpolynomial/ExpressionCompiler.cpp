...
void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	polAssert(accessorType.getReturnParameterTypes().size() >= 1, "");
	...
	polAssert(retSizeOnStack == utils().getSizeOnStack(returnTypes), "");
...
bool ExpressionCompiler::visit(Assignment const& _assignment)
{
	...
	{
		polAssert(_assignment.getType()->isValueType(), "Compound operators not implemented for non-value types.");
...
bool ExpressionCompiler::visit(UnaryOperation const& _unaryOperation)
{
	...
	case Token::Dec: // -- (pre- or postfix)
		...
		if (!_unaryOperation.isPrefixOperation())
		{
			...
			polAssert(_unaryOperation.getType()->getSizeOnStack() == 1, "Stack size != 1 not implemented.");
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	{
		polAssert(_functionCall.getArguments().size() == 1, "");
		polAssert(_functionCall.getNames().empty(), "");
	...
	else
	{
		...
		case Location::Creation:
		{
			...
			polAssert(function.getReturnParameterTypes().size() == 1, "");
			...
			m_context << u256(bytecode.size()) << sof::Instruction::DUP1;
...
void ExpressionCompiler::endVisit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::Contract:
	{
		...
		if (type.isSuper())
		{
			polAssert(!!_memberAccess.referencedDeclaration(), "Referenced declaration not resolved.");
	...
	case Type::Category::Function:
		polAssert(!!_memberAccess.getExpression().getType()->getMemberType(member),
...
bool ExpressionCompiler::visit(IndexAccess const& _indexAccess)
{
	...
	if (baseType.getCategory() == Type::Category::Mapping)
	{
		...
		polAssert(_indexAccess.getIndexExpression(), "Index expression expected.");
...
{
	polAssert(_functionType.takesArbitraryParameters() ||
	...
	{
		m_context << sof::dupInstruction(2 + gasValueSize + CompilerUtils::getSizeOnStack(argumentTypes));
