...
void ExpressionCompiler::appendConstStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	// append return
	m_context << sof::dupInstruction(_varDecl.type()->sizeOnStack() + 1);
...
bool ExpressionCompiler::visit(Assignment const& _assignment)
{
	...
	{
		polAssert(_assignment.type()->isValueType(), "Compound operators not implemented for non-value types.");
...
bool ExpressionCompiler::visit(UnaryOperation const& _unaryOperation)
{
	...
	case Token::Dec: // -- (pre- or postfix)
		...
		if (!_unaryOperation.isPrefixOperation())
		{
			...
			polAssert(_unaryOperation.type()->sizeOnStack() == 1, "Stack size != 1 not implemented.");
...
void ExpressionCompiler::endVisit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::Function:
		polAssert(!!_memberAccess.expression().type()->memberType(member),
