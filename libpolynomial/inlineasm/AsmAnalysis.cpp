...
#include <libpolynomial/inlineasm/AsmScopeFiller.h>
...
bool AsmAnalyzer::operator()(FunctionalInstruction const& _instr)
{
	...
	polAssert(instructionInfo(_instr.instruction.instruction).args == int(_instr.arguments.size()), "");
...
bool AsmAnalyzer::operator()(assembly::Assignment const& _assignment)
{
	...
	polAssert(m_stackHeight >= stackHeight, "Negative value size.");
...
{
	polAssert(m_info.scopes.count(_block) == 1, "Scope requested but not present.");
	auto scopePtr = m_info.scopes.at(_block);
	polAssert(scopePtr, "Scope requested but not present.");
	return *scopePtr;
}
...
{
	static set<polynomial::Instruction> futureInstructions{
		polynomial::Instruction::CREATE2,
		polynomial::Instruction::RETURNDATACOPY,
		polynomial::Instruction::RETURNDATASIZE,
		polynomial::Instruction::STATICCALL
	};
	...
	if (_instr == polynomial::Instruction::JUMP || _instr == polynomial::Instruction::JUMPI)
		m_errorReporter.warning(
			_location,
			"Jump instructions are low-level SVM features that can lead to "
