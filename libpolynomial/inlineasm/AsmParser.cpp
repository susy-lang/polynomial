...
assembly::Statement Parser::parseStatement()
{
	...
	{
		polAssert(elementary.type() == typeid(assembly::Instruction), "Invalid elementary operation.");
...
assembly::Expression Parser::parseExpression()
{
	...
	else if (operation.type() == typeid(Instruction))
	{
		...
		polAssert(m_flavour == AsmFlavour::Loose, "");
	...
	{
		polAssert(operation.type() == typeid(assembly::Literal), "");
		return boost::get<assembly::Literal>(operation);
	}
}

std::map<string, dev::polynomial::Instruction> const& Parser::instructions()
{
	...
	if (s_instructions.empty())
	{
		...
		s_instructions["suicide"] = polynomial::Instruction::SELFDESTRUCT;
		// add alis for sha3
		s_instructions["sha3"] = polynomial::Instruction::KECCAK256;
	}
	return s_instructions;
}

std::map<dev::polynomial::Instruction, string> const& Parser::instructionNames()
{
	static map<dev::polynomial::Instruction, string> s_instructionNames;
	if (s_instructionNames.empty())
	{
		...
		s_instructionNames[polynomial::Instruction::SELFDESTRUCT] = "selfdestruct";
		s_instructionNames[polynomial::Instruction::KECCAK256] = "keccak256";
	}
...
Parser::ElementaryOperation Parser::parseElementaryOperation()
{
	...
	case Token::Address:
	{
		...
		{
			dev::polynomial::Instruction const& instr = instructions().at(literal);
...
assembly::Expression Parser::parseCall(Parser::ElementaryOperation&& _initialOp)
{
	...
	{
		polAssert(m_flavour != AsmFlavour::IULIA, "Instructions are invalid in JULIA");
		Instruction& instruction = boost::get<Instruction>(_initialOp);
		FunctionalInstruction ret;
		ret.instruction = instruction.instruction;
		ret.location = std::move(instruction.location);
		polynomial::Instruction instr = ret.instruction;
