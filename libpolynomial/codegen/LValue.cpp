...
void MemoryItem::storeValue(Type const& _sourceType, SourceLocation const&, bool _move) const
{
	...
	if (m_dataType->isValueType())
	{
		...
		if (!m_padded)
		{
			polAssert(m_dataType->calldataEncodedSize(false) == 1, "Invalid non-padded type.");
	...
	{
		polUnimplementedAssert(_sourceType == *m_dataType, "Conversion not implemented for assignment to memory.");
...
void StorageItem::storeValue(Type const& _sourceType, SourceLocation const& _location, bool _move) const
{
	...
	if (m_dataType->isValueType())
	{
		...
		if (m_dataType->storageBytes() == 32)
		{
			polAssert(m_dataType->sizeOnStack() == 1, "Invalid stack size.");
		...
		else
		{
			...
			{
				polAssert(_sourceType == *m_dataType, "function item stored but target is not equal to source");
				if (fun->kind() == FunctionType::Kind::External)
			...
			{
				polAssert(_sourceType.category() == Type::Category::FixedBytes, "source not fixed bytes");
				CompilerUtils(m_context).rightShiftNumberOnStack(256 - 8 * dynamic_cast<FixedBytesType const&>(*m_dataType).numBytes());
			}
			else
			{
				polAssert(m_dataType->sizeOnStack() == 1, "Invalid stack size for opaque type.");
...
{
	polAssert(false, "Tried to retrieve value of tuple.");
}
...
void TupleObject::storeValue(Type const& _sourceType, SourceLocation const& _location, bool) const
{
	...
	polAssert(valueTypes.size() == m_lvalues.size(), "");
	// valuePos .... refPos ...
...
{
	polAssert(false, "Tried to delete tuple.");
}
