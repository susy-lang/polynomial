...
*/
/**
 * Component that translates Polynomial code into Yul at statement level and below.
 */

#include <libpolynomial/codegen/ir/IRGeneratorForStatements.h>
...
struct CopyTranslate: public yul::ASTCopier
{
	...
	yul::Identifier translate(yul::Identifier const& _identifier) override
	{
		...
		polUnimplementedAssert(varDecl, "");
		polUnimplementedAssert(
...
{
	polAssert(!m_currentLValue, "LValue not reset!");
	return m_code.str();
}
...
void IRGeneratorForStatements::endVisit(VariableDeclarationStatement const& _varDeclStatement)
{
	...
	{
		polUnimplementedAssert(_varDeclStatement.declarations().size() == 1, "");
...
bool IRGeneratorForStatements::visit(Assignment const& _assignment)
{
	...
	{
		polAssert(type(_assignment.leftHandSide()) == *intermediateType, "");
		polAssert(intermediateType->isValueType(), "Compound operators only available for value types.");
...
bool IRGeneratorForStatements::visit(TupleExpression const& _tuple)
{
	...
	{
		polUnimplementedAssert(!_tuple.annotation().lValueRequested, "");
		polUnimplementedAssert(_tuple.components().size() == 1, "");
		polAssert(_tuple.components().front(), "");
...
void IRGeneratorForStatements::endVisit(Return const& _return)
{
	if (Expression const* value = _return.expression())
	{
		...
		// TODO support tuples
		polUnimplementedAssert(types.size() == 1, "Multi-returns not implemented.");
		m_code <<
...
void IRGeneratorForStatements::endVisit(UnaryOperation const& _unaryOperation)
{
	...
	{
		polAssert(resultType == type(_unaryOperation.subExpression()), "Result type doesn't match!");
		...
		if (op == Token::Inc || op == Token::Dec)
		{
			...
			polAssert(false, "Use of unary + is disallowed.");
		else if (op == Token::Sub)
		{
			...
			polUnimplementedAssert(false, "Unary operator not yet implemented");
	}
...
{
	polAssert(!!_binOp.annotation().commonType, "");
	...
	else if (TokenTraits::isCompareOp(op))
	{
		...
		{
			polAssert(op == Token::Equal || op == Token::NotEqual, "Invalid function pointer comparison!");
			polAssert(type->kind() != FunctionType::Kind::External, "External function comparison not allowed!");
		}

		polAssert(commonType->isValueType(), "");
		bool isSigned = false;
		...
		if (op == Token::Equal)
			...
			polAssert(false, "Unknown comparison operator.");
...
void IRGeneratorForStatements::endVisit(FunctionCall const& _functionCall)
{
	...
	{
		polAssert(funcType.category() == Type::Category::TypeType, "Expected category to be TypeType");
		polAssert(_functionCall.arguments().size() == 1, "Expected one argument for type conversion");
	...
	if (callArgumentNames.empty())
		...
		for (auto const& parameterName: functionType->parameterNames())
		{
			...
			polAssert(it != callArgumentNames.cend(), "");
			arguments.push_back(callArguments[std::distance(callArgumentNames.begin(), it)]);
		}

	polUnimplementedAssert(!functionType->bound(), "");
	...
	case FunctionType::Kind::BareCallCode:
		polAssert(false, "Callcode has been removed.");
	case FunctionType::Kind::Event:
	{
		...
		ABIFunctions abi(m_context.svmVersion(), m_context.functionCollector());
		...
		polAssert(indexedArgs.size() <= 4, "Too many indexed arguments.");
		Whiskers templ(R"({
	...
	{
		polAssert(arguments.size() > 0, "Expected at least one parameter for require/assert");
		polAssert(arguments.size() <= 2, "Expected no more than two parameters for require/assert");
...
void IRGeneratorForStatements::endVisit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::Contract:
	{
		...
		else if (Declaration const* declaration = _memberAccess.annotation().referencedDeclaration)
		{
			...
			polAssert(false, "Invalid member access in contract");
		break;
	}
	case Type::Category::Integer:
	{
		polAssert(false, "Invalid member access to integer");
		break;
	}
	case Type::Category::Address:
	{
		...
		{
			polAssert(dynamic_cast<AddressType const&>(*_memberAccess.expression().annotation().type).stateMutability() == StateMutability::Payable, "");
			defineExpression(_memberAccess) <<
			...
			polAssert(false, "Invalid member access to address");
		break;
	}
	...
	case Type::Category::Magic:
		...
		if (member == "coinbase")
			...
			polAssert(false, "Gas has been removed.");
		else if (member == "blockhash")
			polAssert(false, "Blockhash has been removed.");
		...
		else if (set<string>{"encode", "encodePacked", "encodeWithSelector", "encodeWithSignature", "decode"}.count(member))
		{
			...
			polAssert(false, "Unknown magic member.");
		break;
	...
	default:
		polAssert(false, "Member access to unknown type.");
	}
}
...
bool IRGeneratorForStatements::visit(InlineAssembly const& _inlineAsm)
{
	...
	polAssert(modified.type() == typeid(yul::Block), "");
...
void IRGeneratorForStatements::endVisit(IndexAccess const& _indexAccess)
{
	...
	if (baseType.category() == Type::Category::Mapping)
	{
		...
		polAssert(keyType.sizeOnStack() <= 1, "");
...
void IRGeneratorForStatements::endVisit(Identifier const& _identifier)
{
	...
	if (MagicVariableDeclaration const* magicVar = dynamic_cast<MagicVariableDeclaration const*>(declaration))
	{
		...
		case Type::Category::Contract:
			if (dynamic_cast<ContractType const&>(*magicVar->type()).isSuper())
				polAssert(_identifier.name() == "super", "");
			else
			{
				polAssert(_identifier.name() == "this", "");
				defineExpression(_identifier) << "address()\n";
			}
			break;
		case Type::Category::Integer:
			polAssert(_identifier.name() == "now", "");
	...
	else if (VariableDeclaration const* varDecl = dynamic_cast<VariableDeclaration const*>(declaration))
	{
		...
		// the same expression.
		polUnimplementedAssert(!varDecl->isConstant(), "");
		...
		if (m_context.isLocalVariable(*varDecl))
			...
			polAssert(false, "Invalid variable kind.");

		setLValue(_identifier, move(lvalue));
	}
	else if (auto contract = dynamic_cast<ContractDefinition const*>(declaration))
	{
		polUnimplementedAssert(!contract->isLibrary(), "Libraries not yet supported.");
	}
	...
	{
		polAssert(false, "Identifier type not expected in expression context.");
	}
}
...
bool IRGeneratorForStatements::visit(Literal const& _literal)
{
	...
	default:
		polUnimplemented("Only integer, boolean and string literals implemented for now.");
	}
	return false;
}
...
)
{
	...
	polUnimplementedAssert(!funType.bound(), "");
	FunctionType::Kind funKind = funType.kind();

	polAssert(funKind != FunctionType::Kind::BareStaticCall || m_context.svmVersion().hasStaticCall(), "");
	polAssert(funKind != FunctionType::Kind::BareCallCode, "Callcode has been removed.");
	...
	bool useStaticCall = funKind == FunctionType::Kind::BareStaticCall || (funType.stateMutability() <= StateMutability::View && m_context.svmVersion().hasStaticCall());

	bool haveReturndatacopy = m_context.svmVersion().supportsReturndata();
	unsigned retSize = 0;
	...
	polUnimplementedAssert(funKind != FunctionType::Kind::ECRecover, "");

	if (!m_context.svmVersion().canOvsrchargeGasForCall())
	{
	...
	polUnimplementedAssert(!funType.isBareCall(), "");
	Whiskers templ(R"(
		<?checkExistence>
	...
	polUnimplementedAssert(!encodeInPlace, "");
	polUnimplementedAssert(!funType.padArguments(), "");
	...
	if (isDelegateCall)
		polAssert(!funType.valueSet(), "Value set for delegatecall");
	else if (useStaticCall)
		polAssert(!funType.valueSet(), "Value set for staticcall");
	...
	else if (m_context.svmVersion().canOvsrchargeGasForCall())
		// Send all gas (requires tangerine whistle SVM)
		templ("gas", "gas()");
	else
	{
		// send all gas except the amount needed to execute "SUB" and "CALL"
		// @todo this retains too much gas for now, needs to be fine-tuned.
		u256 gasNeededByCaller = sof::GasCosts::callGas(m_context.svmVersion()) + 10;
		if (funType.valueSet())
			gasNeededByCaller += sof::GasCosts::callValueTransferGas;
		if (!checkExistence)
			gasNeededByCaller += sof::GasCosts::callNewAccountGas; // we never know
	...
	polUnimplementedAssert(!returnSuccessConditionAndReturndata, "");
	polUnimplementedAssert(funKind != FunctionType::Kind::RIPEMD160, "");
	polUnimplementedAssert(funKind != FunctionType::Kind::ECRecover, "");
...
)
{
	if (IntegerType const* type = dynamic_cast<IntegerType const*>(&_type))
	{
		...
		polUnimplementedAssert(!fun.empty(), "");
...
void IRGeneratorForStatements::appendAndOrOperatorCode(BinaryOperation const& _binOp)
{
	...
	polAssert(op == Token::Or || op == Token::And, "");
...
)
{
	...
	if (_isDoWhile)
	{
		polAssert(_conditionExpression, "Expected condition for doWhile");
...
{
	polAssert(_expression.annotation().type, "Type of expression not set.");
