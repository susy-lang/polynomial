...
{
	polUnimplementedAssert(_assignment.assignmentOperator() == Token::Assign, "");
...
{
	polUnimplemented("Return not yet implemented in yul code generation");
}

void IRGeneratorForStatements::endVisit(BinaryOperation const& _binOp)
{
	polUnimplementedAssert(_binOp.getOperator() == Token::Add, "");
	polUnimplementedAssert(*_binOp.leftExpression().annotation().type == *_binOp.rightExpression().annotation().type, "");
	if (IntegerType const* type = dynamic_cast<IntegerType const*>(_binOp.annotation().commonType))
	{
		polUnimplementedAssert(!type->isSigned(), "");
		m_code <<
			"let " <<
