...
string IRGenerationContext::variablePart(Expression const& _expression, size_t _part)
{
	...
	polAssert(numVars > 1, "");
	polAssert(1 <= _part && _part <= numVars, "");
...
string IRGenerationContext::internalDispatch(size_t _in, size_t _out)
{
	...
	return m_functions->createFunction(funName, [&]() {
		...
		templ("comma", _in > 0 ? "," : "");
		YulUtilFunctions utils(m_svmVersion, m_functions);
		...
		for (auto const& contract: m_inheritanceHierarchy)
			for (FunctionDefinition const* function: contract->definedFunctions())
				...
				)
				{
					...
					polAssert(function->id() != 0, "Unexpected function ID: 0");
...
{
	return YulUtilFunctions(m_svmVersion, m_functions);
}
