...
pair<string, string> IRGenerator::run(ContractDefinition const& _contract)
{
	...
	yul::AssemblyStack asmStack(m_svmVersion, yul::AssemblyStack::Language::StrictAssembly, m_optimiserSettings);
	if (!asmStack.parseAndAnalyze("", ir))
	{
		string errorMessage;
		for (auto const& error: asmStack.errors())
			errorMessage += langutil::SourceReferenceFormatter::formatErrorInformation(*error);
		polAssert(false, "Invalid IR generated:\n" + errorMessage + "\n" + ir);
	}
	asmStack.optimize();
	...
	string warning =
		"/*******************************************************\n"
		" *                       WARNING                       *\n"
		" *  Polynomial to Yul compilation is still EXPERIMENTAL  *\n"
...
{
	polUnimplementedAssert(!_contract.isLibrary(), "Libraries not yet implemented.");
...
string IRGenerator::constructorCode(ContractDefinition const& _contract)
{
	...
	if (FunctionDefinition const* constructor = _contract.constructor())
	{
		...
		if (!constructor->isPayable())
			out = callValueCheck();
		polUnimplementedAssert(constructor->parameters().empty(), "");
...
string IRGenerator::dispatchRoutine(ContractDefinition const& _contract)
{
	...
	for (auto const& function: _contract.interfaceFunctions())
	{
		...
		ABIFunctions abiFunctions(m_svmVersion, m_context.functionCollector());
...
{
	// This function should be called at the beginning of the SVM call frame
...
void IRGenerator::resetContext(ContractDefinition const& _contract)
{
	...
	m_context = IRGenerationContext(m_svmVersion, m_optimiserSettings);
	m_utils = YulUtilFunctions(m_svmVersion, m_context.functionCollector());
