...
void ContractCompiler::appendCalldataUnpacker(TypePointers const& _typeParameters, bool _fromMemory)
{
	...
	if (m_context.experimentalFeatureActive(ExperimentalFeature::ABIEncoderV2))
	{
		...
		polAssert(m_context.stackHeight() - stackHeightBefore == CompilerUtils(m_context).sizeOnStack(_typeParameters) - 1, "");
		return;
	}
...
void ContractCompiler::appendModifierOrFunctionCode()
{
	...
	else
	{
		...
		else
		{
			...
			polAssert(modifier.parameters().size() == modifierInvocation->arguments().size(), "");
...
sof::AssemblyPointer ContractCompiler::cloneRuntime() const
{
	sof::Assembly a;
	...
	a << u256(sof::GasCosts::callGas(m_context.svmVersion()) + 10) << Instruction::GAS << Instruction::SUB;
	a << Instruction::DELEGATECALL;
	//Propagate error condition (if DELEGATECALL pushes 0 on stack).
	a << Instruction::ISZERO;
	a << Instruction::ISZERO;
	sof::AssemblyItem afterTag = a.appendJumpI().tag();
	a << Instruction::INVALID << afterTag;
	//@todo adjust for larger return values, make this dynamic.
	a << u256(0x20) << u256(0) << Instruction::RETURN;
	return make_shared<sof::Assembly>(a);
}
