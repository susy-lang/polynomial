...
public:
	...
	void check() { polAssert(m_context.stackHeight() == stackHeight, std::string("I sense a disturbance in the stack: ") + to_string(m_context.stackHeight()) + " vs " + to_string(stackHeight)); }
private:
...
{
	// Throw if function is not payable but call contained sophy.
...
{
	polAssert(!_contract.isLibrary(), "Tried to initialize library.");
	CompilerContext::LocationSetter locationSetter(m_context, _contract);
...
{
	polAssert(!!m_runtimeCompiler, "");
	polAssert(!_contract.isLibrary(), "Tried to use contract creator or library.");
	...
	sof::AssemblyItem deployRoutine = m_context.appendJumpToNew();
	...
	polAssert(m_context.runtimeSub() != size_t(-1), "Runtime sub not registered");
...
{
	polAssert(!!m_runtimeCompiler, "");
	polAssert(_contract.isLibrary(), "Tried to deploy contract as library.");
...
void ContractCompiler::appendBaseConstructor(FunctionDefinition const& _constructor)
{
	...
	{
		polAssert(m_baseArguments, "");
		polAssert(m_baseArguments->count(&_constructor), "");
		...
		polAssert(arguments, "");
		polAssert(arguments->size() == constructorType.parameterTypes().size(), "");
...
void ContractCompiler::appendInternalSelector(
	map<FixedHash<4>, sof::AssemblyItem const> const& _entryPoints,
	vector<FixedHash<4>> const& _ids,
	sof::AssemblyItem const& _notFoundTag,
	size_t _runs
)
{
	...
	if (_ids.size() <= 4)
		split = false;
	else if (_runs > (17 * sof::GasCosts::createDataGas) / 6)
		split = true;
	else
		split = (_runs * 6 * (_ids.size() - 4) > 17 * sof::GasCosts::createDataGas);
	...
	if (split)
	{
		...
		sof::AssemblyItem lessTag{m_context.appendConditionalJump()};
...
void ContractCompiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	map<FixedHash<4>, sof::AssemblyItem const> callDataUnpackerEntryPoints;

	if (_contract.isLibrary())
	{
		polAssert(m_context.stackHeight() == 1, "CALL / DELEGATECALL flag expected.");
	}

	FunctionDefinition const* fallback = _contract.fallbackFunction();
	polAssert(!_contract.isLibrary() || !fallback, "Libraries can't have fallback functions");
	...
	sof::AssemblyItem notFound = m_context.newTag();
	...
	if (fallback)
	{
		polAssert(!_contract.isLibrary(), "");
		if (!fallback->isPayable() && needToAddCallvalueCheck)
			appendCallValueCheck();

		polAssert(fallback->isFallback(), "");
		polAssert(FunctionType(*fallback).parameterTypes().empty(), "");
		polAssert(FunctionType(*fallback).returnParameterTypes().empty(), "");
	...
	for (auto const& it: interfaceFunctions)
	{
		...
		polAssert(functionType->hasDeclaration(), "");
		...
		m_context.appendJumpTo(
			...
			sof::AssemblyItem::JumpType::IntoFunction
		);
		m_context << returnTag;
...
{
	polAssert(!_contract.isLibrary(), "Tried to initialize state variables of library.");
...
{
	polAssert(_variableDeclaration.isStateVariable(), "Compiler visit to non-state variable declaration.");
...
bool ContractCompiler::visit(FunctionDefinition const& _function)
{
	...
	for (int i = 0; i < int(stackLayout.size()); ++i)
		if (stackLayout[i] != i)
			polAssert(false, "Invalid stack layout on cleanup.");
	...
	if (!_function.isConstructor())
	{
		polAssert(m_context.numberOfLocalVariables() == 0, "");
	...
	identifierAccess.generateCode = [&](yul::Identifier const& _identifier, yul::IdentifierContext _context, yul::AbstractAssembly& _assembly)
	{
		...
		if (_context == yul::IdentifierContext::RValue)
		{
			...
			if (FunctionDefinition const* functionDef = dynamic_cast<FunctionDefinition const*>(decl))
			{
				...
				polAssert(functionEntryLabel.data() <= std::numeric_limits<size_t>::max(), "");
				...
				if (CompilerContext* rtc = m_context.runtimeContext())
				{
					...
					polAssert(runtimeEntryLabel.data() <= std::numeric_limits<size_t>::max(), "");
			...
			polAssert(_assembly.stackHeight() - depositBefore == int(ref->second.valueSize), "");
		}
		else
		{
			// lvalue context
...
bool ContractCompiler::visit(IfStatement const& _ifStatement)
{
	...
	sof::AssemblyItem falseTag = m_context.appendConditionalJump();
	sof::AssemblyItem endTag = falseTag;
...
bool ContractCompiler::visit(WhileStatement const& _whileStatement)
{
	...
	{
		sof::AssemblyItem condition = m_context.newTag();
...
bool ContractCompiler::visit(ForStatement const& _forStatement)
{
	...
	sof::AssemblyItem loopStart = m_context.newTag();
	sof::AssemblyItem loopEnd = m_context.newTag();
	sof::AssemblyItem loopNext = m_context.newTag();
...
bool ContractCompiler::visit(Continue const& _continueStatement)
{
	...
	polAssert(!m_continueTags.empty(), "");
...
{
	polAssert(false, "Throw statement is disallowed.");
	return false;
}
...
bool ContractCompiler::visit(VariableDeclarationStatement const& _variableDeclarationStatement)
{
	...
	if (Expression const* expression = _variableDeclarationStatement.initialValue())
	{
		...
		polAssert(declarations.size() == valueTypes.size(), "");
...
void ContractCompiler::appendMissingFunctions()
{
	while (Declaration const* function = m_context.nextFunctionToCompile())
	{
		...
		polAssert(m_context.nextFunctionToCompile() != function, "Compiled the wrong function?");
	}
...
void ContractCompiler::appendModifierOrFunctionCode()
{
	...
	else
	{
		...
		else
		{
			...
			polAssert(modifier.parameters().size() == modifierArguments.size(), "");
...
void ContractCompiler::popScopedVariables(ASTNode const* _node)
{
	...
	polAssert(m_context.stackHeight() >= blockHeight, "");
