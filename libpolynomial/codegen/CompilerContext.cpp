...
)
{
	...
	polAssert(sizeOnStack == 1 || sizeOnStack == 2, "");
...
{
	polAssert(m_localVariables.count(&_declaration) && !m_localVariables[&_declaration].empty(), "");
...
void CompilerContext::removeVariablesAboveStackHeight(unsigned _stackHeight)
{
	...
	{
		polAssert(!_var.second.empty(), "");
		polAssert(_var.second.back() <= stackHeight(), "");
...
shared_ptr<sof::Assembly> CompilerContext::compiledContract(ContractDefinition const& _contract) const
{
	auto ret = m_otherCompilers.find(&_contract);
	polAssert(ret != m_otherCompilers.end(), "Compiled contract not found.");
	return ret->second->assemblyPtr();
}

shared_ptr<sof::Assembly> CompilerContext::compiledContractRuntime(ContractDefinition const& _contract) const
{
	auto ret = m_otherCompilers.find(&_contract);
	polAssert(ret != m_otherCompilers.end(), "Compiled contract not found.");
	...
	for (ContractDefinition const* contract: m_inheritanceHierarchy)
		for (ModifierDefinition const* modifier: contract->functionModifiers())
			if (modifier->name() == _modifier.name())
				return *modifier;
	polAssert(false, "Function modifier " + _modifier.name() + " not found in inheritance hierarchy.");
}
...
{
	if (_forwardReturnData && m_svmVersion.supportsReturndata())
...
)
{
	...
	yul::CodeGenerator::assemble(*parserResult, analysisInfo, *m_asm, m_svmVersion, identifierAccess, _system, _optimise);
...
sof::Assembly::OptimiserSettings CompilerContext::translateOptimiserSettings(OptimiserSettings const& _settings)
{
	// Constructing it this way so that we notice changes in the fields.
	sof::Assembly::OptimiserSettings asmSettings{false, false, false, false, false, false, m_svmVersion, 0};
	...
	asmSettings.svmVersion = m_svmVersion;
	return asmSettings;
}
