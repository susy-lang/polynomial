...
public:
	explicit ContractCompiler(ContractCompiler* _runtimeCompiler, CompilerContext& _context, bool _optimise, size_t _optimise_runs = 200):
		...
		m_context(_context)
	{
		m_context = CompilerContext(_context.svmVersion(), _runtimeCompiler ? &_runtimeCompiler->m_context : nullptr);
	}
...
private:
	...
	void appendInternalSelector(
		std::map<FixedHash<4>, sof::AssemblyItem const> const& _entryPoints,
	...
	std::vector<std::pair<sof::AssemblyItem, unsigned>> m_breakTags;
	/// Tag to jump to for a "continue" statement and the stack height after freeing the local loop variables.
	std::vector<std::pair<sof::AssemblyItem, unsigned>> m_continueTags;
	...
	std::vector<std::pair<sof::AssemblyItem, unsigned>> m_returnTags;
