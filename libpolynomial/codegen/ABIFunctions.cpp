...
)
{
	...
	return createFunction(functionName, [&]() {
		...
		if (_from.dataStoredIn(DataLocation::Storage))
		{
			...
			polAssert(to == *TypeProvider::uint256(), "");
...
)
{
	...
	"");
	polAssert(fromArrayType.calldataStride() == toArrayType.memoryStride(), "");
	...
	return createFunction(functionName, [&]() {
		...
		{
			polAssert(fromArrayType.calldataStride() == 32, "");
			Whiskers templ(R"(
				// <readableTypeNameFrom> -> <readableTypeNameTo>
...
{
	polAssert(_type.isValueType() || _type.dataStoredIn(DataLocation::CallData), "");
