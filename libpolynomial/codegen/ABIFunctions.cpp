...
)
{
	...
	return createExternallyUsedFunction(functionName, [&]() {
		...
		{
			polAssert(_givenTypes[i], "");
			polAssert(_targetTypes[i], "");
...
string ABIFunctions::tupleDecoder(TypePointers const& _types, bool _fromMemory)
{
	...
	return createExternallyUsedFunction(functionName, [&]() {
		...
		{
			polAssert(_types[i], "");
			polAssert(decodingTypes[i], "");
			size_t sizeOnStack = _types[i]->sizeOnStack();
			polAssert(sizeOnStack == decodingTypes[i]->sizeOnStack(), "");
			polAssert(sizeOnStack > 0, "");
...
string ABIFunctions::cleanupFunction(Type const& _type, bool _revertOnFailure)
{
	...
	return createFunction(functionName, [&]() {
		...
		case Type::Category::FixedPoint:
			polUnimplemented("Fixed point types not implemented.");
			break;
		case Type::Category::Array:
		case Type::Category::Struct:
		case Type::Category::Mapping:
			polAssert(_type.dataStoredIn(DataLocation::Storage), "Cleanup requested for non-storage reference type.");
			templ("body", "cleaned := value");
			break;
		...
		case Type::Category::Enum:
		{
			...
			polAssert(members > 0, "empty enum should have caused a parser error.");
		...
		case Type::Category::InaccessibleDynamic:
			...
			break;
		default:
			polAssert(false, "Cleanup of type " + _type.identifier() + " requested.");
		}
	...
	return createFunction(functionName, [&]() {
		...
		case Type::Category::Contract:
		{
			if (RationalNumberType const* rational = dynamic_cast<RationalNumberType const*>(&_from))
				polUnimplementedAssert(!rational->isFractional(), "Not yet implemented - FixedPointType.");
			...
			else if (toCategory == Type::Category::Enum)
			{
				...
				polUnimplemented("Not yet implemented - FixedPointType.");
		...
		case Type::Category::Bool:
		{
			polAssert(_from == _to, "Invalid conversion for bool.");
			body =
		...
		case Type::Category::FixedPoint:
			...
			break;
		case Type::Category::Array:
			polUnimplementedAssert(false, "Array conversion not implemented.");
			break;
		case Type::Category::Struct:
			polUnimplementedAssert(false, "Struct conversion not implemented.");
			break;
		case Type::Category::FixedBytes:
		{
			...
			else
			{
				...
				polAssert(toCategory == Type::Category::FixedBytes, "Invalid type conversion requested.");
				body =
					Whiskers("converted := <clean>(value)")
		...
		{
			polAssert(false, "Conversion should not be called for function types.");
			break;
		}
		case Type::Category::Enum:
		{
			polAssert(toCategory == Type::Category::Integer || _from == _to, "");
		...
		{
			polUnimplementedAssert(false, "Tuple conversion not implemented.");
			break;
		}
		default:
			polAssert(false, "");
		...
		polAssert(!body.empty(), _from.canonicalName() + " to " + _to.canonicalName());
		templ("body", body);
		return templ.render();
	});
}
...
)
{
	...
	polUnimplementedAssert(toInterface, "Encoding type \"" + _to.toString() + "\" not yet implemented.");
	...
	{
		polAssert(_from.category() == Type::Category::Array, "");
		polAssert(to.dataStoredIn(DataLocation::Memory), "");
	...
	polAssert(_from.sizeOnStack() == 1, "");
	polAssert(to.isValueType(), "");
	polAssert(to.calldataEncodedSize() == 32, "");
	string functionName =
		"abi_encode_" +
		_from.identifier() +
		"_to_" +
	...
	return createFunction(functionName, [&]() {
		polAssert(!to.isDynamicallyEncoded(), "");
		...
		if (_from.dataStoredIn(DataLocation::Storage))
		{
			...
			polAssert(_options.encodeAsLibraryTypes, "");
			polAssert(_options.padded && !_options.dynamicInplace, "Non-padded / inplace encoding for library call requested.");
...
)
{
	...
	return createFunction(functionName, [&]() {
		...
		else
		{
			...
			polAssert(encodedSize != 0, "Invalid encoded size.");
			return Whiskers(R"(
				function <functionName>(value, pos) -> updatedPos {
					<encode>(value, pos)
...
{
	polAssert(_to.isDynamicallySized(), "");
	polAssert(_from.category() == Type::Category::Array, "Unknown dynamic type.");
	polAssert(_to.category() == Type::Category::Array, "Unknown dynamic type.");
	auto const& fromArrayType = dynamic_cast<ArrayType const&>(_from);
	auto const& toArrayType = dynamic_cast<ArrayType const&>(_to);

	polAssert(fromArrayType.location() == DataLocation::CallData, "");
	...
	string functionName =
		...
		_options.toFunctionNameSuffix();
	return createFunction(functionName, [&]() {
		polUnimplementedAssert(fromArrayType.isByteArray(), "Only byte arrays can be encoded from calldata currently.");
...
)
{
	...
	polAssert(_from.isDynamicallySized() == _to.isDynamicallySized(), "");
	polAssert(_from.length() == _to.length(), "");
	polAssert(_from.dataStoredIn(DataLocation::Memory) || _from.dataStoredIn(DataLocation::Storage), "");
	polAssert(!_from.isByteArray(), "");
	polAssert(_from.dataStoredIn(DataLocation::Memory) || _from.baseType()->storageBytes() > 16, "");
...
)
{
	...
	polAssert(_from.isDynamicallySized() == _to.isDynamicallySized(), "");
	polAssert(_from.length() == _to.length(), "");
	polAssert(_from.dataStoredIn(DataLocation::Memory), "");
	polAssert(_from.isByteArray(), "");

	return createFunction(functionName, [&]() {
		polAssert(_to.isByteArray(), "");
		Whiskers templ(R"(
...
)
{
	...
	polAssert(_from.isDynamicallySized() == _to.isDynamicallySized(), "");
	polAssert(_from.length() == _to.length(), "");
	polAssert(_from.dataStoredIn(DataLocation::Storage), "");
	...
	return createFunction(functionName, [&]() {
		...
		else
		{
			...
			polAssert(_from.baseType()->storageBytes() <= 16, "");
			polAssert(!_from.baseType()->isDynamicallyEncoded(), "");
			polAssert(_from.baseType()->isValueType(), "");
			bool dynamic = _to.isDynamicallyEncoded();
...
)
{
	...
	polUnimplementedAssert(!_from.dataStoredIn(DataLocation::CallData), "Encoding struct from calldata is not yet supported.");
	polAssert(&_from.structDefinition() == &_to.structDefinition(), "");
	...
	return createFunction(functionName, [&]() {
		...
		{
			polAssert(member.type, "");
			if (!member.type->canLiveOutsideStorage())
				continue;
			TypePointer memberTypeTo = member.type->fullEncodingType(_options.encodeAsLibraryTypes, true, false);
			polUnimplementedAssert(memberTypeTo, "Encoding type \"" + member.type->toString() + "\" not yet implemented.");
			auto memberTypeFrom = _from.memberType(member.name);
			polAssert(memberTypeFrom, "");
			...
			if (fromStorage)
			{
				polAssert(memberTypeFrom->isValueType() == memberTypeTo->isValueType(), "");
				...
				{
					polAssert(memberTypeFrom->dataStoredIn(DataLocation::Storage), "");
					polAssert(intraSlotOffset == 0, "");
		...
		if (_options.dynamicInplace)
			polAssert(encodingOffset == 0, "In-place encoding should enforce zero head size.");
...
{
	polAssert(_from.category() == Type::Category::StringLiteral, "");
	...
	return createFunction(functionName, [&]() {
		...
		polAssert(_from.sizeOnStack() == 0, "");

		if (_to.isDynamicallySized())
		{
			polAssert(_to.category() == Type::Category::Array, "");
			Whiskers templ(R"(
		...
		{
			polAssert(_to.category() == Type::Category::FixedBytes, "");
			polAssert(value.size() <= 32, "");
			Whiskers templ(R"(
...
{
	polAssert(_from.kind() == FunctionType::Kind::External, "");
	polAssert(_from == _to, "");
...
string ABIFunctions::abiDecodingFunction(Type const& _type, bool _fromMemory, bool _forUseOnStack)
{
	...
	else if (auto const* structType = dynamic_cast<StructType const*>(decodingType.get()))
	{
		...
		{
			polAssert(!_fromMemory, "");
			polUnimplementedAssert(!structType->isDynamicallyEncoded(), "Dynamically encoded calldata structs are not yet implemented.");
...
string ABIFunctions::abiDecodingFunctionValueType(Type const& _type, bool _fromMemory)
{
	TypePointer decodingType = _type.decodingType();
	polAssert(decodingType, "");
	polAssert(decodingType->sizeOnStack() == 1, "");
	polAssert(decodingType->isValueType(), "");
	polAssert(decodingType->calldataEncodedSize() == 32, "");
	polAssert(!decodingType->isDynamicallyEncoded(), "");
...
{
	polAssert(_type.dataStoredIn(DataLocation::Memory), "");
	polAssert(!_type.isByteArray(), "");
	...
	polAssert(!_type.dataStoredIn(DataLocation::Storage), "");
...
string ABIFunctions::abiDecodingFunctionCalldataArray(ArrayType const& _type)
{
	...
	// is not yet implemented in Polynomial.
	polAssert(_type.dataStoredIn(DataLocation::CallData), "");
	if (!_type.isDynamicallySized())
		polAssert(_type.length() < u256("0xffffffffffffffff"), "");
	if (_type.baseType()->isDynamicallyEncoded())
		polUnimplemented("Calldata arrays with non-value base types are not yet supported by Polynomial.");
	polAssert(_type.baseType()->calldataEncodedSize() < u256("0xffffffffffffffff"), "");
...
{
	polAssert(_type.dataStoredIn(DataLocation::CallData), "");
	polAssert(_type.calldataEncodedSize(true) != 0, "");
	string functionName =
		"abi_decode_" +
		_type.identifier();
...
string ABIFunctions::abiDecodingFunctionStruct(StructType const& _type, bool _fromMemory)
{
	...
	return createFunction(functionName, [&]() {
		...
		templ("readableTypeName", _type.toString(true));
		templ("allocate", allocationFunction());
		polAssert(_type.memorySize() < u256("0xffffffffffffffff"), "");
		...
		{
			polAssert(member.type, "");
			polAssert(member.type->canLiveOutsideStorage(), "");
...
{
	polAssert(_type.kind() == FunctionType::Kind::External, "");
...
string ABIFunctions::leftAlignFunction(Type const& _type)
{
	...
	return createFunction(functionName, [&]() {
		...
		case Type::Category::RationalNumber:
			polAssert(false, "Left align requested for rational number.");
			break;
		case Type::Category::Bool:
		...
		case Type::Category::Array:
		case Type::Category::Struct:
			polAssert(false, "Left align requested for non-value type.");
			break;
		case Type::Category::FixedBytes:
			templ("body", "aligned := value");
			break;
		...
		case Type::Category::InaccessibleDynamic:
			polAssert(false, "Left align requested for inaccessible dynamic type.");
			break;
		default:
			polAssert(false, "Left align of type " + _type.identifier() + " requested.");
		}
...
{
	polAssert(_numBits < 256, "");

	string functionName = "shift_left_" + to_string(_numBits);
	if (m_svmVersion.hasBitwiseShifting())
	{
		return createFunction(functionName, [&]() {
			return
				Whiskers(R"(
	...
	return createFunction(functionName, [&]() {
		...
		else
		{
			...
			case DataLocation::CallData:
				polAssert(false, "called regular array length function on calldata array");
				break;
