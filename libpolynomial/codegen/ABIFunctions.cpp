...
string ABIFunctions::cleanupFunction(Type const& _type, bool _revertOnFailure)
{
	...
	return createFunction(functionName, [&]() {
		...
		case Type::Category::Array:
			polAssert(false, "Array cleanup requested.");
			break;
		case Type::Category::Struct:
			polAssert(false, "Struct cleanup requested.");
			break;
...
)
{
	...
	string functionName =
		...
		(_encodeAsLibraryTypes ? "_library" : "");
	return createFunction(functionName, [&]() {
		polUnimplementedAssert(fromArrayType.isByteArray(), "");
...
)
{
	...
	polUnimplementedAssert(!_from.dataStoredIn(DataLocation::CallData), "");
...
size_t ABIFunctions::headSize(TypePointers const& _targetTypes)
{
	...
	for (auto const& t: _targetTypes)
	{
		...
		{
			polAssert(t->calldataEncodedSize() > 0, "");
