...
{
public:
	explicit CompilerContext(langutil::SVMVersion _svmVersion, CompilerContext* _runtimeContext = nullptr):
	...
	langutil::SVMVersion const& svmVersion() const { return m_svmVersion; }
	...
	std::shared_ptr<sof::Assembly> compiledContract(ContractDefinition const& _contract) const;
	std::shared_ptr<sof::Assembly> compiledContractRuntime(ContractDefinition const& _contract) const;

	void setStackOffset(int _offset) { m_asm->setDeposit(_offset); }
	void adjustStackOffset(int _adjustment) { m_asm->adjustDeposit(_adjustment); }
	unsigned stackHeight() const { polAssert(m_asm->deposit() >= 0, ""); return unsigned(m_asm->deposit()); }

	bool isLocalVariable(Declaration const* _declaration) const;
	bool isStateVariable(Declaration const* _declaration) const { return m_stateVariables.count(_declaration) != 0; }

	/// @returns the entry label of the given function and creates it if it does not exist yet.
	sof::AssemblyItem functionEntryLabel(Declaration const& _declaration);
	/// @returns the entry label of the given function. Might return an AssemblyItem of type
	/// UndefinedItem if it does not exist yet.
	sof::AssemblyItem functionEntryLabelIfExists(Declaration const& _declaration) const;
	...
	/// Note that the generator should not assume that objects are still alive when it is called,
	/// unless they are guaranteed to be alive for the whole run of the compiler (AST nodes, for example).
	sof::AssemblyItem lowLevelFunctionTag(
		std::string const& _name,
		unsigned _inArgs,
		unsigned _outArgs,
		std::function<void(CompilerContext&)> const& _generator
	...
	sof::AssemblyItem appendConditionalJump() { return m_asm->appendJumpI().tag(); }
	/// Appends a JUMPI instruction to @a _tag
	CompilerContext& appendConditionalJumpTo(sof::AssemblyItem const& _tag) { m_asm->appendJumpI(_tag); return *this; }
	/// Appends a JUMP to a new tag and @returns the tag
	sof::AssemblyItem appendJumpToNew() { return m_asm->appendJump().tag(); }
	/// Appends a JUMP to a tag already on the stack
	CompilerContext& appendJump(sof::AssemblyItem::JumpType _jumpType = sof::AssemblyItem::JumpType::Ordinary);
	...
	/// If the current SVM version does not support RETURNDATA, uses REVERT but does not forward
	/// the data.
	...
	sof::AssemblyItem newTag() { return m_asm->newTag(); }
	/// @returns a new tag identified by name.
	sof::AssemblyItem namedTag(std::string const& _name) { return m_asm->namedTag(_name); }
	/// Adds a subroutine to the code (in the data section) and pushes its size (via a tag)
	/// on the stack. @returns the pushsub assembly item.
	sof::AssemblyItem addSubroutine(sof::AssemblyPointer const& _assembly) { return m_asm->appendSubroutine(_assembly); }
	...
	sof::AssemblyItem appendData(bytes const& _data) { return m_asm->append(_data); }
	...
	void appendDeployTimeAddress() { m_asm->append(sof::PushDeployTimeAddress); }
	...
	CompilerContext& operator<<(sof::AssemblyItem const& _item) { m_asm->append(_item); return *this; }
	CompilerContext& operator<<(dev::sof::Instruction _instruction) { m_asm->append(_instruction); return *this; }
	...
	sof::Assembly const& assembly() const { return *m_asm; }
	/// @returns a shared pointer to the assembly.
	/// Should be avoided except when adding sub-assemblies.
	std::shared_ptr<sof::Assembly> assemblyPtr() const { return m_asm; }
	...
	sof::LinkerObject const& assembledObject() const { return m_asm->assemble(); }
	sof::LinkerObject const& assembledRuntimeObject(size_t _subIndex) const { return m_asm->sub(_subIndex).assemble(); }
...
private:
	...
	sof::Assembly::OptimiserSettings translateOptimiserSettings(OptimiserSettings const& _settings);
	...
	struct FunctionCompilationQueue
	{
		...
		sof::AssemblyItem entryLabel(Declaration const& _declaration, CompilerContext& _context);
		/// @returns the entry label of the given function. Might return an AssemblyItem of type
		/// UndefinedItem if it does not exist yet.
		sof::AssemblyItem entryLabelIfExists(Declaration const& _declaration) const;
		...
		std::map<Declaration const*, sof::AssemblyItem> m_entryLabels;
	...
	std::map<std::string, sof::AssemblyItem> m_lowLevelFunctions;
