...
{
	polAssert(_argumentType.isImplicitlyConvertibleTo(*TypeProvider::fromElementaryTypeName("string memory")), "");
	fetchFreeMemoryPointer();
...
void CompilerUtils::accessCalldataTail(Type const& _type)
{
	...
	else
	{
		...
		polAssert(!!arrayType, "Invalid dynamically sized type.");
		unsigned int calldataStride = arrayType->calldataStride();
		polAssert(calldataStride > 0, "");
...
{
	polAssert(_type.category() != Type::Category::Array, "Unable to statically load dynamic type.");
...
void CompilerUtils::abiDecode(TypePointers const& _typeParameters, bool _fromMemory)
{
	...
	if (m_context.experimentalFeatureActive(ExperimentalFeature::ABIEncoderV2))
	{
		...
		polAssert(m_context.stackHeight() - stackHeightBefore == sizeOnStack(_typeParameters) - 2, "");
		return;
	}
	...
	for (TypePointer const& parameterType: _typeParameters)
	{
		...
		polUnimplementedAssert(type, "No decoding type found.");
		if (type->category() == Type::Category::Array)
		{
			auto const& arrayType = dynamic_cast<ArrayType const&>(*type);
			polUnimplementedAssert(!arrayType.baseType()->isDynamicallyEncoded(), "Nested arrays not yet implemented.");
			if (_fromMemory)
			{
				polUnimplementedAssert(
		...
		{
			polAssert(!type->isDynamicallyEncoded(), "Unknown dynamically sized type: " + type->toString());
...
)
{
	...
	for (TypePointer& t: targetTypes)
	{
		...
		polUnimplementedAssert(tEncoding, "Encoding type \"" + t->toString() + "\" not yet implemented.");
	...
	if (encoderV2)
	{
		...
		polAssert(stackHeightBefore - m_context.stackHeight() == sizeOnStack(_givenTypes), "");
		return;
	}
	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			dynPointers++;
			polAssert((argSize + dynPointers) < 16, "Stack too deep, try using fewer variables.");
		}
		else
		{
	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			{
				polAssert(_givenTypes[i]->category() == Type::Category::Array, "Unknown dynamic type.");
...
{
	if (!_padToWordBoundaries)
		polAssert(!_encodeAsLibraryTypes, "Library calls cannot be packed.");
...
void CompilerUtils::zeroInitialiseMemoryArray(ArrayType const& _type)
{
	...
	{
		polAssert(_type.baseType()->isValueType(), "");
		Whiskers templ(R"({
...
)
{
	...
	case Type::Category::FixedBytes:
	{
		...
		{
			polAssert(typeOnStack.numBytes() * 8 == 160, "");
			rightShiftNumberOnStack(256 - 160);
		}
		else
		{
			// clear for conversion to longer bytes
			polAssert(targetTypeCategory == Type::Category::FixedBytes, "Invalid type conversion requested.");
	...
	case Type::Category::Enum:
		polAssert(_targetType == _typeOnStack || targetTypeCategory == Type::Category::Integer, "");
		if (enumOverflowCheckPending)
		{
			EnumType const& enumType = dynamic_cast<decltype(enumType)>(_typeOnStack);
			polAssert(enumType.numberOfMembers() > 0, "empty enum should have caused a parser error.");
	...
	case Type::Category::RationalNumber:
		if (targetTypeCategory == Type::Category::FixedBytes)
		{
			...
			if (auto typeOnStack = dynamic_cast<IntegerType const*>(&_typeOnStack))
			{
				...
				polAssert(targetBytesType.numBytes() * 8 == 160, "");
			leftShiftNumberOnStack(256 - targetBytesType.numBytes() * 8);
		}
		else if (targetTypeCategory == Type::Category::Enum)
		{
			polAssert(stackTypeCategory != Type::Category::Address, "Invalid conversion to EnumType requested.");
			polAssert(_typeOnStack.mobileType(), "");
			// just clean
		...
		{
			polAssert(targetTypeCategory == Type::Category::Integer || targetTypeCategory == Type::Category::Contract || targetTypeCategory == Type::Category::Address, "");
			...
			if (stackTypeCategory == Type::Category::RationalNumber)
			{
				...
				polUnimplementedAssert(!constType.isFractional(), "Not yet implemented - FixedPointType.");
	...
	{
		polAssert(targetTypeCategory == stackTypeCategory, "");
		...
		case DataLocation::Memory:
		{
			...
			if (typeOnStack.location() != DataLocation::Memory)
			{
				...
				{
					polAssert(typeOnStack.baseType()->isValueType(), "");
	...
	case Type::Category::Struct:
	{
		...
		case DataLocation::Memory:
			...
			case DataLocation::Storage:
			{
					...
					[typeOnStack = &typeOnStack, targetType = &targetType](CompilerContext& _context)
				{
					...
					for (auto const& member: typeOnStack->members(nullptr))
					{
						...
						polAssert(!!targetMemberType, "Member not found in target type.");
			...
			case DataLocation::CallData:
			{
				polUnimplementedAssert(!typeOnStack.isDynamicallyEncoded(), "");
				m_context << Instruction::DUP1;
			...
			break;
		case DataLocation::CallData:
			polAssert(false, "Invalid type conversion target location CallData.");
			break;
		}
		break;
	}
	case Type::Category::Tuple:
	{
		...
		polAssert(targetTuple.components().size() == sourceTuple.components().size(), "");
	...
	case Type::Category::Bool:
		polAssert(_targetType == _typeOnStack, "Invalid conversion for bool.");
		if (_cleanupNeeded)
	...
	default:
		if (stackTypeCategory == Type::Category::Function && targetTypeCategory == Type::Category::Address)
		{
			...
			polAssert(typeOnStack.kind() == FunctionType::Kind::External, "Only external function type can be converted.");
		...
		else
		{
			if (stackTypeCategory == Type::Category::Function && targetTypeCategory == Type::Category::Function)
			{
				...
				polAssert(_typeOnStack == _targetType, "Invalid type conversion requested.");
	...
	polAssert(!enumOverflowCheckPending, "enum overflow checking missing.");
	polAssert(!chopSignBitsPending, "forgot to chop the sign bits.");
}
...
void CompilerUtils::pushZeroValue(Type const& _type)
{
	...
	polAssert(referenceType->location() == DataLocation::Memory, "");
	if (auto arrayType = dynamic_cast<ArrayType const*>(&_type))
		if (arrayType->isDynamicallySized())
		{
	...
	m_context.callLowLevelFunction(
		...
		[type](CompilerContext& _context) {
			...
			else if (auto arrayType = dynamic_cast<ArrayType const*>(type))
			{
				...
				polAssert(false, "Requested initialisation for unknown type: " + type->toString());
...
void CompilerUtils::popAndJump(unsigned _toHeight, sof::AssemblyItem const& _jumpTo)
{
	polAssert(m_context.stackHeight() >= _toHeight, "");
...
void CompilerUtils::copyContractCodeToMemory(ContractDefinition const& contract, bool _creation)
{
	...
	m_context.callLowLevelFunction(
		...
		[&contract, _creation](CompilerContext& _context)
		{
			...
			shared_ptr<sof::Assembly> assembly =
				_creation ?
...
unsigned CompilerUtils::loadFromMemoryHelper(Type const& _type, bool _fromCalldata, bool _padToWords)
{
	...
	polAssert(numBytes <= 32, "Static memory load of more than 32 bytes requested.");
