...
void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	for (size_t i = 0; i < paramTypes.size(); ++i)
	{
		if (auto mappingType = dynamic_cast<MappingType const*>(returnType.get()))
		{
			...
			{
				polAssert(paramTypes[i]->isValueType(), "Expected value type for mapping key");
...
bool ExpressionCompiler::visit(Assignment const& _assignment)
{
	...
	{
		polAssert(*_assignment.annotation().type == TupleType(), "");
		polAssert(op == Token::Assign, "");
	}
	else
		polAssert(*_assignment.annotation().type == leftType, "");
	...
	polAssert(rightIntermediateType, "");
	...
	{
		polAssert(leftType.isValueType(), "Compound operators only available for value types.");
		...
		{
			polAssert(leftType == *rightIntermediateType, "");
...
bool ExpressionCompiler::visit(UnaryOperation const& _unaryOperation)
{
	...
	case Token::Dec: // -- (pre- or postfix)
		...
		if (!_unaryOperation.isPrefixOperation())
		{
			...
			polUnimplementedAssert(_unaryOperation.annotation().type->sizeOnStack() == 1, "Stack size != 1 not implemented.");
			m_context << Instruction::DUP1;
	...
	case Token::Sub: // -
		...
		break;
	default:
		polAssert(false, "Invalid unary operator: " + string(TokenTraits::toString(_unaryOperation.getOperator())));
	}
	return false;
}
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		if (function.bound())
			...
			polAssert(function.kind() == FunctionType::Kind::DelegateCall || function.kind() == FunctionType::Kind::Internal, "");
		...
		case FunctionType::Kind::Transfer:
			_functionCall.expression().accept(*this);
			// Provide the gas stipend manually at first because we may send zero sophy.
			// Will be zeroed if we send more than zero sophy.
			m_context << u256(sof::GasCosts::callStipend);
			arguments.front()->accept(*this);
			utils().convertType(
		...
		{
			polAssert(arguments.size() == 1, "");
			polAssert(!function.padArguments(), "");
		...
		case FunctionType::Kind::Log4:
		{
			...
			arguments.front()->accept(*this);
			utils().fetchFreeMemoryPointer();
			polAssert(function.parameterTypes().front()->isValueType(), "");
			utils().packedEncode(
				{arguments.front()->annotation().type},
		...
		case FunctionType::Kind::Event:
		{
			...
			for (unsigned arg = arguments.size(); arg > 0; --arg)
				if (event.parameters()[arg - 1]->isIndexed())
				{
					...
					{
						polAssert(paramTypes[arg - 1]->isValueType(), "");
						utils().convertType(
							*arguments[arg - 1]->annotation().type,
							*paramTypes[arg - 1],
							true
		...
		{
			_functionCall.expression().accept(*this);
			polAssert(function.parameterTypes().empty(), "");
			...
			polAssert(arrayType.dataStoredIn(DataLocation::Storage), "");
		...
		case FunctionType::Kind::Require:
		{
			...
			if (arguments.size() > 1)
			{
				...
				// function call.
				polAssert(arguments.size() == 2, "");
				polAssert(function.kind() == FunctionType::Kind::Require, "");
		...
		case FunctionType::Kind::ABIEncodeWithSignature:
		{
			...
			if (hasSelectorOrSignature)
			{
				...
				{
					polAssert(function.kind() == FunctionType::Kind::ABIEncodeWithSelector, "");
				}
...
bool ExpressionCompiler::visit(MemberAccess const& _memberAccess)
{
	...
	if (auto funType = dynamic_cast<FunctionType const*>(_memberAccess.annotation().type.get()))
		if (funType->bound())
		{
			...
			{
				polAssert(funType->kind() == FunctionType::Kind::DelegateCall, "");
	...
	if (TypeType const* type = dynamic_cast<TypeType const*>(_memberAccess.expression().annotation().type.get()))
	{
		...
		{
			polAssert(_memberAccess.annotation().type, "_memberAccess has no type");
			...
			else if (auto funType = dynamic_cast<FunctionType const*>(_memberAccess.annotation().type.get()))
			{
				...
				case FunctionType::Kind::RIPEMD160:
				default:
					polAssert(false, "unsupported member function");
				}
			}
	...
	case Type::Category::Struct:
	{
		...
		{
			polUnimplementedAssert(!type.isDynamicallyEncoded(), "");
			...
			{
				polAssert(_memberAccess.annotation().type->calldataEncodedSize(false) > 0, "");
...
bool ExpressionCompiler::visit(IndexAccess const& _indexAccess)
{
	...
	else if (baseType.category() == Type::Category::Array)
	{
		...
		case DataLocation::CallData:
			...
			polUnimplementedAssert(!arrayType.baseType()->isDynamicallySized(), "Nested arrays not yet implemented.");
...
{
	polAssert(_type.sizeOnStack() == 1, "Comparison of multi-slot types.");
...
void ExpressionCompiler::appendOrdinaryBinaryOperatorCode(Token _operator, Type const& _type)
{
	if (TokenTraits::isArithmeticOp(_operator))
		...
		polAssert(false, "Unknown binary operator.");
}
...
void ExpressionCompiler::appendArithmeticOperatorCode(Token _operator, Type const& _type)
{
	...
	case Token::Exp:
		...
		break;
	default:
		polAssert(false, "Unknown arithmetic operator.");
	}
}
...
void ExpressionCompiler::appendBitOperatorCode(Token _operator)
{
	...
	case Token::BitXor:
		...
		break;
	default:
		polAssert(false, "Unknown bit operator.");
	}
}
...
void ExpressionCompiler::appendShiftOperatorCode(Token _operator, Type const& _valueType, Type const& _shiftAmountType)
{
	...
	if (auto valueType = dynamic_cast<IntegerType const*>(&_valueType))
		...
		polAssert(dynamic_cast<FixedBytesType const*>(&_valueType), "Only integer and fixed bytes type supported for shifts.");
	...
	if (auto amountType = dynamic_cast<RationalNumberType const*>(&_shiftAmountType))
	{
		...
		polAssert(amountType->integerType(), "");
		polAssert(!amountType->integerType()->isSigned(), "");
	}
	else if (auto amountType = dynamic_cast<IntegerType const*>(&_shiftAmountType))
		c_amountSigned = amountType->isSigned();
	else
		polAssert(false, "Invalid shift amount type.");
	...
	{
	case Token::SHL:
		if (m_context.svmVersion().hasBitwiseShifting())
	...
	case Token::SHR:
	default:
		polAssert(false, "Unknown shift operator.");
	}
}
...
)
{
	...
	bool useStaticCall = funKind == FunctionType::Kind::BareStaticCall || (_functionType.stateMutability() <= StateMutability::View && m_context.svmVersion().hasStaticCall());
	...
	if (funKind == FunctionType::Kind::ECRecover)
	{
		...
		polAssert(0 < retSize && retSize <= 32, "");
		utils().fetchFreeMemoryPointer();
	...
	if (isDelegateCall)
		...
		polAssert(!_functionType.valueSet(), "Value set for staticcall");
	...
	else if (!returnTypes.empty())
	{
		...
		m_context << (haveReturndatacopy ? sof::AssemblyItem(Instruction::RETURNDATASIZE) : u256(retSize));
...
{
	polUnimplementedAssert(_expectedType.isValueType(), "Not implemented for non-value types.");
	_expression.accept(*this);
