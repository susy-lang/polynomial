...
void ArrayUtils::copyArrayToStorage(ArrayType const& _targetType, ArrayType const& _sourceType) const
{
	...
	m_context.callLowLevelFunction(
		...
		[=](CompilerContext& _context)
		{
			...
			sof::AssemblyItem copyLoopEndWithoutByteOffset = _context.newTag();
			...
			if (_targetType.isByteArray())
			{
				...
				sof::AssemblyItem longByteArray = _context.appendConditionalJump();
			...
			sof::AssemblyItem copyLoopStart = _context.newTag();
			_context << copyLoopStart;
			// check for loop condition
			_context
				<< dupInstruction(3 + byteOffsetSize) << dupInstruction(2 + byteOffsetSize)
				<< Instruction::GT << Instruction::ISZERO;
			sof::AssemblyItem copyLoopEnd = _context.appendConditionalJump();
			// stack: target_ref target_data_end source_data_pos target_data_pos source_data_end [target_byte_offset] [source_byte_offset]
			// copy
			...
			else
			{
				...
				if (_sourceType.location() == DataLocation::Storage)
				{
					...
					polUnimplemented("Copying of type " + _sourceType.toString(false) + " to storage not yet supported.");
			...
			if (haveByteOffsetTarget)
			{
				...
				sof::AssemblyItem copyCleanupLoopEnd = _context.appendConditionalJump();
...
void ArrayUtils::clearArray(ArrayType const& _typeIn) const
{
	...
	m_context.callLowLevelFunction(
		...
		[type](CompilerContext& _context)
		{
			...
			polAssert(_context.stackHeight() == stackHeightStart - 2, "");
		}
	);
}
...
void ArrayUtils::resizeDynamicArray(ArrayType const& _typeIn) const
{
	...
	m_context.callLowLevelFunction(
		...
		[type](CompilerContext& _context)
		{
			...
			unsigned stackHeightStart = _context.stackHeight();
			sof::AssemblyItem resizeEnd = _context.newTag();

			// stack: ref new_length
			// fetch old length
			ArrayUtils(_context).retrieveLength(_type, 1);
			// stack: ref new_length old_length
			polAssert(_context.stackHeight() - stackHeightStart == 3 - 2, "2");

			// Special case for short byte arrays, they are stored together with their length
			if (_type.isByteArray())
			{
				sof::AssemblyItem regularPath = _context.newTag();
				// We start by a large case-distinction about the old and new length of the byte array.

				_context << Instruction::DUP3 << Instruction::SLOAD;
				// stack: ref new_length current_length ref_value

				polAssert(_context.stackHeight() - stackHeightStart == 4 - 2, "3");
				_context << Instruction::DUP2 << u256(31) << Instruction::LT;
				sof::AssemblyItem currentIsLong = _context.appendConditionalJump();
				_context << Instruction::DUP3 << u256(31) << Instruction::LT;
				sof::AssemblyItem newIsLong = _context.appendConditionalJump();

				// Here: short -> short

				// Compute 1 << (256 - 8 * new_size)
				sof::AssemblyItem shortToShort = _context.newTag();
				...
				polAssert(_context.stackHeight() - stackHeightStart == 3 - 2, "3");
...
void ArrayUtils::clearStorageLoop(TypePointer const& _type) const
{
	m_context.callLowLevelFunction(
		...
		[_type](CompilerContext& _context)
		{
			...
			sof::AssemblyItem returnTag = _context.pushNewTag();
			_context << Instruction::SWAP2 << Instruction::SWAP1;

			// stack: <return tag> end_pos pos
			sof::AssemblyItem loopStart = _context.appendJumpToNew();
			_context << loopStart;
			// check for loop condition
			_context <<
				Instruction::DUP1 <<
				...
				Instruction::GT <<
				Instruction::ISZERO;
			sof::AssemblyItem zeroLoopEnd = _context.newTag();
			...
			_context << returnTag;
			polAssert(_context.stackHeight() == stackHeightStart - 1, "");
		}
	);
}
