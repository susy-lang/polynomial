...
string YulUtilFunctions::requireOrAssertFunction(bool _assert, Type const* _messageType)
{
	...
	polAssert(!_assert || !_messageType, "Asserts can't have messages!");
	...
	return m_functionCollector->createFunction(functionName, [&]() {
		...
		string const encodeFunc = ABIFunctions(m_svmVersion, m_functionCollector)
			.tupleEncoder(
				{_messageType},
...
string YulUtilFunctions::shiftRightFunction(size_t _numBits)
{
	...
	string functionName = "shift_right_" + to_string(_numBits) + "_unsigned_" + m_svmVersion.name();
	return m_functionCollector->createFunction(functionName, [&]() {
		return
			Whiskers(R"(
			function <functionName>(value) -> newValue {
			...
			("functionName", functionName)
			("hasShifts", m_svmVersion.hasBitwiseShifting())
			("numBits", to_string(_numBits))
			("multiplier", toCompactHexWithPrefix(u256(1) << _numBits))
			.render();
	});
...
{
	polAssert(_numBytes <= 32, "");
	polAssert(_shiftBytes <= 32, "");
...
{
	polAssert(0 < _bits && _bits <= 256 && _bits % 8 == 0, "");
...
string YulUtilFunctions::overflowCheckedIntDivFunction(IntegerType const& _type)
{
	...
	polAssert(0 < bits && bits <= 256 && bits % 8 == 0, "");
...
string YulUtilFunctions::nextArrayElementFunction(ArrayType const& _type)
{
	...
	if (_type.dataStoredIn(DataLocation::Storage))
		polAssert(_type.baseType()->storageBytes() > 16, "");
...
{
	polAssert(_keyType.sizeOnStack() <= 1, "");
	...
	return m_functionCollector->createFunction(functionName, [&]() {
		...
		else
		{
			...
			polAssert(!_mappingType.keyType()->isDynamicallyEncoded(), "");
			polAssert(_mappingType.keyType()->calldataEncodedSize(false) <= 0x20, "");
			Whiskers templ(R"(
...
{
	polUnimplementedAssert(_type.category() != Type::Category::Function, "");
...
string YulUtilFunctions::cleanupFunction(Type const& _type)
{
	...
	return m_functionCollector->createFunction(functionName, [&]() {
		...
		case Type::Category::FixedPoint:
			...
			break;
		case Type::Category::Function:
			polAssert(dynamic_cast<FunctionType const&>(_type).kind() == FunctionType::Kind::External, "");
...
string YulUtilFunctions::validatorFunction(Type const& _type, bool _revertOnFailure)
{
	...
	return m_functionCollector->createFunction(functionName, [&]() {
		...
		case Type::Category::InaccessibleDynamic:
			...
			break;
		default:
			polAssert(false, "Validation of type " + _type.identifier() + " requested.");
		}
...
)
{
	...
	return m_functionCollector->createFunction(functionName, [&]() {
		...
		templ("packedEncode", ABIFunctions(m_svmVersion, m_functionCollector).tupleEncoderPacked(_givenTypes, _targetTypes));
		return templ.render();
	});
}
...
{
	bool forward = m_svmVersion.supportsReturndata();
...
{
	IntegerType const& type = dynamic_cast<IntegerType const&>(_type);
	polAssert(type.isSigned(), "Expected signed type!");
...
{
	polUnimplementedAssert(_type.sizeOnStack() == 1, "Stacksize not yet implemented!");
	polUnimplementedAssert(_type.isValueType(), "Zero value for non-value types not yet implemented");
