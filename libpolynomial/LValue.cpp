...
void MemoryItem::storeValue(Type const& _sourceType, SourceLocation const&, bool _move) const
{
	...
	else
	{
		...
		polAssert(m_dataType.getSizeOnStack() == 1, "");
		if (!_move)
	...
	LValue(_compilerContext, _type)
{
	if (m_dataType.isValueType())
	{
		polAssert(m_dataType.getStorageSize() == m_dataType.getSizeOnStack(), "");
		polAssert(m_dataType.getStorageSize() == 1, "Invalid storage size.");
	}
}
...
void StorageItem::retrieveValue(SourceLocation const&, bool _remove) const
{
	...
	if (!m_dataType.isValueType())
	{
		polAssert(m_dataType.getSizeOnStack() == 1, "Invalid storage ref size.");
		if (_remove)
	...
	else
	{
		...
		if (m_dataType.getCategory() == Type::Category::FixedBytes)
			m_context << (u256(0x1) << (256 - 8 * m_dataType.getStorageBytes())) << sof::Instruction::MUL;
		else if (
			m_dataType.getCategory() == Type::Category::Integer &&
			dynamic_cast<IntegerType const&>(m_dataType).isSigned()
		)
			m_context << u256(m_dataType.getStorageBytes() - 1) << sof::Instruction::SIGNEXTEND;
		else
			m_context << ((u256(0x1) << (8 * m_dataType.getStorageBytes())) - 1) << sof::Instruction::AND;
	}
}
...
void StorageItem::storeValue(Type const& _sourceType, SourceLocation const& _location, bool _move) const
{
	...
	if (m_dataType.isValueType())
	{
		polAssert(m_dataType.getStorageBytes() <= 32, "Invalid storage bytes size.");
		polAssert(m_dataType.getStorageBytes() > 0, "Invalid storage bytes size.");
		if (m_dataType.getStorageBytes() == 32)
		{
		...
		else
		{
			...
			m_context
				<< sof::Instruction::DUP2 << ((u256(1) << (8 * m_dataType.getStorageBytes())) - 1)
				<< sof::Instruction::MUL;
	...
	else
	{
		...
		else if (m_dataType.getCategory() == Type::Category::Struct)
		{
			...
			// stack layout: source_ref target_ref
			polAssert(sourceType.getSizeOnStack() == 1, "Unexpected source size.");
			if (_move)
...
void StorageItem::setToZero(SourceLocation const&, bool _removeReference) const
{
	...
	else
	{
		...
		else
		{
			...
			// clear bytes in old value
			m_context
				<< sof::Instruction::SWAP1 << ((u256(1) << (8 * m_dataType.getStorageBytes())) - 1)
				<< sof::Instruction::MUL;
			m_context << sof::Instruction::NOT << sof::Instruction::AND;
