	...
	m_globalContext(_globalContext)
{
	...
	{
		polAssert(m_scopes[nullptr]->registerDeclaration(*declaration), "Unable to register global declaration.");
	}
}
...
bool NameAndTypeResolver::updateDeclaration(Declaration const& _declaration)
{
	try
	{
		...
		polAssert(_declaration.scope() == nullptr, "Updated declaration outside global scope.");
	}
	catch (langutil::FatalError const&)
	{
		if (m_errorReporter.errors().empty())
...
{
	polAssert(_declarations.size() > 1, "");
	...
	for (Declaration const* declaration: _declarations)
	{
		...
		polAssert(functionType, "Failed to determine the function type of the overloaded.");
...
void NameAndTypeResolver::importInheritedScope(ContractDefinition const& _base)
{
	...
	polAssert(iterator != end(m_scopes), "");
		...
		for (auto const& declaration: nameAndDeclaration.second)
			...
			if (declaration->scope() == &_base && declaration->isVisibleInDerivedContracts())
				if (!m_currentScope->registerDeclaration(*declaration))
				{
					...
					polAssert(conflictingDeclaration, "");
	...
	m_globalContext(_globalContext)
{
	...
	polAssert(m_currentScope == _currentScope, "Scopes not correctly closed.");
}
...
)
{
	...
	polAssert(!(_inactive && !_declaration.isVisibleInContract()), "");
...
void DeclarationRegistrationHelper::endVisit(VariableDeclarationStatement& _variableDeclarationStatement)
{
	...
	polAssert(m_currentFunction, "Variable declaration without function.");
...
void DeclarationRegistrationHelper::enterNewSubScope(ASTNode& _subScope)
{
	...
	polAssert(newlyAdded, "Unable to add new scope.");
	m_currentScope = &_subScope;
}

void DeclarationRegistrationHelper::closeCurrentScope()
{
	polAssert(m_currentScope && m_scopes.count(m_currentScope), "Closed non-existing scope.");
	m_currentScope = m_scopes[m_currentScope]->enclosingNode();
}

void DeclarationRegistrationHelper::registerDeclaration(Declaration& _declaration, bool _opensScope)
{
	polAssert(m_currentScope && m_scopes.count(m_currentScope), "No current scope.");
