...
public:
	...
	{
		if (yul::SVMDialect const* dialect = dynamic_cast<decltype(dialect)>(&m_dialect))
			if (yul::BuiltinFunctionForSVM const* fun = dialect->builtin(_funCall.functionName.name))
				if (fun->instruction)
...
private:
	void checkInstruction(SourceLocation _location, dev::sof::Instruction _instruction)
	{
		if (sof::SemanticInformation::invalidInViewFunctions(_instruction))
			m_reportMutability(StateMutability::NonPayable, _location);
		else if (sof::SemanticInformation::invalidInPureFunctions(_instruction))
...
{
	polAssert(m_currentFunction == &_funDef, "");
	if (
...
{
	polAssert(m_currentFunction == nullptr, "");
...
void ViewPureChecker::endVisit(Identifier const& _identifier)
{
	...
	else if (MagicVariableDeclaration const* magicVar = dynamic_cast<MagicVariableDeclaration const*>(declaration))
	{
		...
		case Type::Category::Contract:
			polAssert(_identifier.name() == "this" || _identifier.name() == "super", "");
...
void ViewPureChecker::endVisit(IndexAccess const& _indexAccess)
{
	if (!_indexAccess.indexExpression())
		polAssert(_indexAccess.annotation().type->category() == Type::Category::TypeType, "");
	else
	{
...
void ViewPureChecker::endVisit(ModifierInvocation const& _modifier)
{
	...
	if (ModifierDefinition const* mod = dynamic_cast<decltype(mod)>(_modifier.name()->annotation().referencedDeclaration))
	{
		...
		polAssert(dynamic_cast<ContractDefinition const*>(_modifier.name()->annotation().referencedDeclaration), "");
}
