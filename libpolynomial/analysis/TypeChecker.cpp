...
void TypeChecker::endVisit(FunctionTypeName const& _funType)
{
	...
	if (fun.kind() == FunctionType::Kind::External)
		polAssert(fun.interfaceType(false), "External function type uses internal types.");
}
		...
		if (!var.annotation().type)
		{
			...
			else if (valueComponentType->category() == Type::Category::RationalNumber)
			{
				...
				if (auto type = dynamic_cast<IntegerType const*>(var.annotation().type))
				{
					...
					polAssert(dynamic_cast<FixedPointType const*>(var.annotation().type), "Unknown type.");
			}
...
)
{
	...
	else
	{
		...
		else
		{
			...
			{
				polAssert(dynamic_cast<ContractType const*>(resultType)->isPayable(), "");
				polAssert(
