...
#include <libsvmasm/Assembly.h> // needed for inline assembly
...
bool TypeChecker::visit(InlineAssembly const& _inlineAssembly)
{
	...
	codeGen.typeCheck([&](assembly::Identifier const& _identifier, sof::Assembly& _assembly, assembly::CodeGenerator::IdentifierContext _context) {
		...
		else
		{
			...
			if (auto varDecl = dynamic_cast<VariableDeclaration const*>(declaration))
			{
				...
				for (unsigned i = 0; i < declaration->type()->sizeOnStack(); ++i)
					_assembly.append(sof::Instruction::POP); // remove value just to verify the stack height
			}
			else
				return false;
		}
