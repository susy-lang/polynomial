...
void TypeChecker::checkDoubleStorageAssignment(Assignment const& _assignment)
{
	...
	for (size_t i = 0; i < lhs.components().size(); ++i)
	{
		...
		polAssert(rhsPos < rhs.components().size(), "");
		toStorageCopies++;
...
bool TypeChecker::visit(FunctionCall const& _functionCall)
{
	...
	polAssert(_functionCall.annotation().kind != FunctionCallKind::Unset, "");
	...
	bool allowDynamicTypes = m_svmVersion.supportsReturndata();
	if (!functionType)
	{
	...
	{
		polAssert(_functionCall.annotation().kind == FunctionCallKind::FunctionCall, "");
		m_errorReporter.typeError(
			_functionCall.location(),
			"Need at least " +
			toString(parameterTypes.size()) +
...
bool TypeChecker::visit(MemberAccess const& _memberAccess)
{
	...
	if (exprType->category() == Type::Category::Contract)
	{
		...
		for (auto const& addressMember: IntegerType(160, IntegerType::Modifier::Address).nativeMembers(nullptr))
			...
			{
				polAssert(!v050, "Address member still present on contract in v0.5.0.");
				m_errorReporter.warning(
					_memberAccess.location(),
