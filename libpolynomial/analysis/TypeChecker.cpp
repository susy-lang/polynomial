...
{
	polAssert(!!_expression.annotation().type, "Type requested but not present.");
	return _expression.annotation().type;
}

TypePointer const& TypeChecker::type(VariableDeclaration const& _variable) const
{
	polAssert(!!_variable.annotation().type, "Type requested but not present.");
...
void TypeChecker::checkDoubleStorageAssignment(Assignment const& _assignment)
{
	...
	{
		polAssert(m_errorReporter.hasErrors(), "");
		return;
	}
...
void TypeChecker::endVisit(InheritanceSpecifier const& _inheritance)
{
	...
	polAssert(base, "Base contract not available.");
...
bool TypeChecker::visit(StructDefinition const& _struct)
{
	for (ASTPointer<VariableDeclaration> const& member: _struct.members())
		polAssert(type(*member)->canBeStored(), "Type cannot be used in struct.");
...
bool TypeChecker::visit(FunctionDefinition const& _function)
{
	...
	auto checkArgumentAndReturnParameter = [&](VariableDeclaration const& var) {
		if (type(var)->category() == Type::Category::Mapping)
		{
			...
			{
				polAssert(isLibraryFunction || !_function.isPublic(), "Mapping types for parameters or return variables can only be used in internal or library functions.");
			}
		}
		else
		{
...
bool TypeChecker::visit(VariableDeclaration const& _variable)
{
	...
	polAssert(!!varType, "Variable type not provided.");
...
{
	polAssert(_eventDef.visibility() > Declaration::Visibility::Internal, "");
...
void TypeChecker::endVisit(FunctionTypeName const& _funType)
{
	...
	if (fun.kind() == FunctionType::Kind::External)
		polAssert(fun.interfaceType(false).get(), "External function type uses internal types.");
}
...
bool TypeChecker::visit(InlineAssembly const& _inlineAssembly)
{
	...
	)
	{
		...
		{
			polAssert(var->type(), "Expected variable type!");
			if (var->isConstant())
			{
		...
		{
			polAssert(!!declaration->type(), "Type of declaration required but not yet determined.");
...
string createTupleDecl(vector<ASTPointer<VariableDeclaration>> const& _decls)
{
	...
	for (ASTPointer<VariableDeclaration> const& decl: _decls)
		if (decl)
		{
			polAssert(decl->annotation().type, "");
...
bool TypeChecker::visit(VariableDeclarationStatement const& _statement)
{
	if (!_statement.initialValue())
	{
		...
		if (auto ref = dynamic_cast<ReferenceType const*>(type(varDecl).get()))
		{
			if (ref->dataStoredIn(DataLocation::Storage))
			{
				...
				polAssert(varDecl.referenceLocation() != VariableDeclaration::Location::Unspecified, "Expected a specified location at this point");
				polAssert(m_scope, "");
	...
	for (size_t i = 0; i < min(variables.size(), valueTypes.size()); ++i)
	{
		...
		if (!var.annotation().type)
		{
			...
			if (!var.annotation().type)
			{
				...
				polAssert(false, "Cannot declare variable with void (empty tuple) type.");
			else if (valueComponentType->category() == Type::Category::RationalNumber)
			{
				...
				if (auto type = dynamic_cast<IntegerType const*>(var.annotation().type.get()))
				{
					...
					polAssert(dynamic_cast<FixedPointType const*>(var.annotation().type.get()), "Unknown type.");
			}
	...
	{
		polAssert(m_errorReporter.hasErrors(), "Should have errors!");
...
void TypeChecker::checkExpressionAssignment(Type const& _type, Expression const& _expression)
{
	if (auto const* tupleExpression = dynamic_cast<TupleExpression const*>(&_expression))
	{
		...
		for (size_t i = 0; i < min(tupleExpression->components().size(), types.size()); i++)
			if (types[i])
			{
				polAssert(!!tupleExpression->components()[i], "");
...
{
	polAssert(_functionCall.annotation().kind == FunctionCallKind::TypeConversion, "");
	...
	else
	{
		...
		else
		{
			...
			{
				polAssert(dynamic_cast<ContractType const*>(resultType.get())->isPayable(), "");
				polAssert(
...
)
{
	...
	polAssert(!!_functionType, "");
	polAssert(_functionType->kind() != FunctionType::Kind::ABIDecode, "");

	// Check for unsupported use of bare static call
	if (
		_functionType->kind() == FunctionType::Kind::BareStaticCall &&
		!m_svmVersion.hasStaticCall()
	)
		m_errorReporter.typeError(
			_functionCall.location(),
...
)
{
	...
	polAssert(_functionType->takesArbitraryParameters(), "ABI functions should be variadic.");

	bool const isPacked = _functionType->kind() == FunctionType::Kind::ABIEncodePacked;
	polAssert(_functionType->padArguments() != isPacked, "ABI function with unexpected padding");
...
)
{
	...
	for (size_t i = 0; i < paramArgMap.size(); ++i)
	{
		polAssert(!!paramArgMap[i], "unmapped parameter");
...
bool TypeChecker::visit(FunctionCall const& _functionCall)
{
	...
	case FunctionCallKind::FunctionCall:
	{
		...
		default:
		{
			...
			returnTypes = m_svmVersion.supportsReturndata() ?
...
void TypeChecker::endVisit(NewExpression const& _newExpression)
{
	...
	polAssert(!!type, "Type name not resolved.");
	...
	case Type::Category::Array:
	{
		...
		else
		{
			...
			if (!m_errorReporter.hasErrors())
				...
				{
					polAssert(!numberType->isFractional(), "");
	...
	case Type::Category::TypeType:
	{
		...
		else
		{
			...
			if (expectType(*index, IntegerType::uint256()))
			{
				...
				polAssert(m_errorReporter.hasErrors(), "Expected errors as expectType returned false");
	...
	if (!annotation.referencedDeclaration)
	{
		...
		else
		{
			...
			for (Declaration const* declaration: annotation.overloadedDeclarations)
			{
				...
				polAssert(!!functionType, "Requested type not present.");
	...
	if (_literal.looksLikeAddress())
	{
		...
		if (!msg.empty())
			m_errorReporter.syntaxError(
				...
				"For more information please see https://polynomial.readthedocs.io/en/develop/types.html#address-literals"
			);
	}
...
{
	polAssert(!!_identifier.annotation().referencedDeclaration, "Declaration not stored.");
	return *_identifier.annotation().referencedDeclaration;
}

Declaration const& TypeChecker::dereference(UserDefinedTypeName const& _typeName) const
{
	polAssert(!!_typeName.annotation().referencedDeclaration, "Declaration not stored.");
