...
bool ReferencesResolver::visit(ElementaryTypeName const& _typeName)
{
	if (!_typeName.annotation().type)
	{
		...
		if (_typeName.stateMutability().is_initialized())
		{
			...
			polAssert(_typeName.annotation().type->category() == Type::Category::Address, "");
...
{
	polAssert(!m_returnParameters.empty(), "");
...
void ReferencesResolver::endVisit(ArrayTypeName const& _typeName)
{
	...
	if (!baseType)
	{
		polAssert(!m_errorReporter.errors().empty(), "");
		return;
	}
...
bool ReferencesResolver::visit(InlineAssembly const& _inlineAssembly)
{
	...
	[&](yul::Identifier const& _identifier, yul::IdentifierContext, bool _crossesFunctionBoundary) {
		...
		{
			declarationError(_identifier.location, "Multiple matching identifiers. Repolving overloaded identifiers is not supported.");
			return size_t(-1);
		}
		...
		if (auto var = dynamic_cast<VariableDeclaration const*>(declarations.front()))
			...
			{
				declarationError(_identifier.location, "Cannot access local Polynomial variables from inside an inline assembly function.");
				return size_t(-1);
			}
	...
	yul::AsmAnalyzer(
		...
		errorsIgnored,
		errorTypeForLoose,
		yul::SVMDialect::looseAssemblyForSVM(SVMVersion{}),
		resolver
...
void ReferencesResolver::endVisit(VariableDeclaration const& _variable)
{
	...
	if (!_variable.typeName())
	{
		...
		// However, we cannot blindingly polAssert() for that here, as the TypeChecker (which is
	...
	if (!allowedDataLocations.count(varLoc))
	{
		...
		polAssert(!allowedDataLocations.empty(), "");
	...
	{
		polAssert(varLoc == Location::Unspecified, "");
	...
	else if (_variable.isStateVariable())
	{
		...
		case Location::Unspecified:
			polAssert(!_variable.hasReferenceOrMappingType(), "Data location not properly set.");
		}
