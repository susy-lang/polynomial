...
public:
	...
	void check() { polAssert(m_context.stackHeight() == stackHeight, "I sense a disturbance in the stack."); }
private:
...
)
{
	...
	sof::AssemblyItem runtimeSub = m_context.addSubroutine(cloneRuntime());
...
void Compiler::packIntoContractCreator(ContractDefinition const& _contract, CompilerContext const& _runtimeContext)
{
	...
	sof::AssemblyItem runtimeSub = m_context.addSubroutine(_runtimeContext.assembly());
	polAssert(runtimeSub.data() < numeric_limits<size_t>::max(), "");
	m_runtimeSub = size_t(runtimeSub.data());
		...
		case Type::Category::Array:
		{
			...
			else
			{
				...
				else
				{
					...
					m_context << sof::Instruction::DUP1;
					m_context << u256(calldataType->calldataEncodedSize()) << sof::Instruction::ADD;
				}
...
bool Compiler::visit(Return const& _return)
{
	...
	{
		polAssert(_return.functionReturnParameters(), "Invalid return parameters pointer.");
