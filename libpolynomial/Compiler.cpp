...
public:
	...
	void check() { polAssert(m_context.getStackHeight() == stackHeight, "I sense a disturbance in the stack."); }
private:
...
)
{
	...
	sof::AssemblyItem runtimeSub = m_context.addSubroutine(getCloneRuntime());
	polAssert(runtimeSub.data() < numeric_limits<size_t>::max(), "");
	m_runtimeSub = size_t(runtimeSub.data());

	// stack contains sub size
	m_context << sof::Instruction::DUP1 << runtimeSub << u256(0) << sof::Instruction::CODECOPY;
	m_context << u256(0) << sof::Instruction::RETURN;
...
sof::AssemblyItem Compiler::getFunctionEntryLabel(FunctionDefinition const& _function) const
{
...
void Compiler::packIntoContractCreator(ContractDefinition const& _contract, CompilerContext const& _runtimeContext)
{
	...
	sof::AssemblyItem runtimeSub = m_context.addSubroutine(_runtimeContext.getAssembly());
	polAssert(runtimeSub.data() < numeric_limits<size_t>::max(), "");
	m_runtimeSub = size_t(runtimeSub.data());
...
void Compiler::appendConstructor(FunctionDefinition const& _constructor)
{
	...
	if (!_constructor.getParameters().empty())
	{
		...
		if (argumentSize == 0)
		{
			...
			m_context << sof::Instruction::CODESIZE << sof::Instruction::SUB;
		}
		else
...
void Compiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	// sophy with constant gas
	if (interfaceFunctions.size() > 5 || fallback)
	{
		m_context << sof::Instruction::CALLDATASIZE << sof::Instruction::ISZERO;
	...
	for (auto const& it: interfaceFunctions)
	{
		...
		m_context << sof::dupInstruction(1) << u256(FixedHash<4>::Arith(it.first)) << sof::Instruction::EQ;
...
)
{
	...
	for (TypePointer const& type: _typeParameters)
	{
		...
		case Type::Category::Array:
		{
			...
			polAssert(!arrayType.getBaseType()->isDynamicallySized(), "Nested arrays not yet implemented.");
			if (_fromMemory)
			{
				polAssert(
			...
			else
			{
				...
				else
				{
					...
					m_context << sof::Instruction::DUP1;
					m_context << u256(calldataType->getCalldataEncodedSize()) << sof::Instruction::ADD;
				}
...
void Compiler::appendReturnValuePacker(TypePointers const& _typeParameters)
{
	...
	if (_typeParameters.empty())
		m_context << sof::Instruction::STOP;
	else
	{
...
bool Compiler::visit(FunctionDefinition const& _function)
{
	...
	polAssert(stackLayout.size() <= 17, "Stack too deep, try removing local variables.");
	while (stackLayout.back() != int(stackLayout.size() - 1))
		...
		{
			m_context << sof::swapInstruction(stackLayout.size() - stackLayout.back() - 1);
...
bool Compiler::visit(Return const& _return)
{
	...
	{
		polAssert(_return.getFunctionReturnParameters(), "Invalid return parameters pointer.");
...
void Compiler::appendModifierOrFunctionCode()
{
	...
	else
	{
		...
		polAssert(modifier.getParameters().size() == modifierInvocation->getArguments().size(), "");
...
sof::Assembly Compiler::getCloneRuntime()
{
	sof::Assembly a;
	...
	//@todo adjust for larger return values, make this dynamic.
	a << u256(0x20) << u256(0) << sof::Instruction::RETURN;
	return a;
}
