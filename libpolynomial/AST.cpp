...
#include <libpolynomial/AST_accept.h>
...
map<FixedHash<4>, FunctionTypePointer> ContractDefinition::getInterfaceFunctions() const
{
	...
	polAssert(exportedFunctionList.size() == exportedFunctions.size(),
...
void Identifier::checkTypeRequirements(TypePointers const* _argumentTypes)
{
	...
	polAssert(!!m_referencedDeclaration, "Referenced declaration is null after overload resolution.");
...
{
	polAssert(!!m_referencedDeclaration, "Identifier not resolved.");
	return *m_referencedDeclaration;
}

void Identifier::overloadResolution(TypePointers const& _argumentTypes)
{
	polAssert(!m_referencedDeclaration, "Referenced declaration should be null before overload resolution.");
	polAssert(!m_overloadedDeclarations.empty(), "No candidates for overload resolution found.");
