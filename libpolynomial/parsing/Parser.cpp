...
ASTPointer<PragmaDirective> Parser::parsePragmaDirective()
{
	...
	do
	{
		...
		if (token == Token::Illegal)
			parserError("Token incompatible with Polynomial parser as part of pragma directive.");
		else
		{
	...
	if (literals.size() >= 2 && literals[0] == "polynomial")
	{
		parsePragmaVersion(
...
ContractDefinition::ContractKind Parser::parseContractKind()
{
	...
	case Token::Library:
		...
		break;
	default:
		polAssert(false, "Invalid contract kind.");
	}
	m_scanner->next();
	return kind;
}
...
Declaration::Visibility Parser::parseVisibilitySpecifier()
{
	...
	switch (token)
	{
		...
		case Token::External:
			...
			break;
		default:
			polAssert(false, "Invalid visibility specifier.");
	}
	m_scanner->next();
	return visibility;
}
...
StateMutability Parser::parseStateMutability()
{
	...
	switch(token)
	{
		...
		case Token::Constant:
			...
			break;
		default:
			polAssert(false, "Invalid state mutability specifier.");
	}
	m_scanner->next();
...
Parser::FunctionHeaderParserResult Parser::parseFunctionHeader(bool _forceEmptyName, bool _allowModifiers)
{
	...
	if (m_scanner->currentToken() == Token::Constructor)
		...
		polAssert(false, "Function or constructor expected.");
	m_scanner->next();
...
)
{
			...
			else if (_options.allowLocationSpecifier && TokenTraits::isLocationSpecifier(token))
			{
				...
				else
				{
					...
					case Token::CallData:
						...
						break;
					default:
						polAssert(false, "Unknown data location.");
					}
				}
			}
			else
	...
	if (_options.allowEmptyName && m_scanner->currentToken() != Token::Identifier)
	{
		...
		polAssert(!_options.allowVar, ""); // allowEmptyName && allowVar makes no sense
	}
	else
	{
		nodeFactory.markEndPosition();
...
ASTPointer<FunctionTypeName> Parser::parseFunctionType()
{
	...
	polAssert(!header.isConstructor, "Tried to parse type as constructor.");
...
ASTPointer<InlineAssembly> Parser::parseInlineAssembly(ASTPointer<ASTString> const& _docString)
{
	...
	yul::Parser asmParser(m_errorReporter, yul::SVMDialect::looseAssemblyForSVM());
...
ASTPointer<Expression> Parser::parsePrimaryExpression()
{
	...
	case Token::FalseLiteral:
		...
		break;
	case Token::Number:
		if (TokenTraits::isSophySubdenomination(m_scanner->peekNextToken()))
		{
