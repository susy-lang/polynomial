...
void SMTChecker::endVisit(UnaryOperation const& _op)
{
	...
	case Token::Not: // !
	{
		polAssert(isBool(_op.annotation().type->category()), "");
	...
	case Token::Dec: // -- (pre- or postfix)
	{
		...
		polAssert(isInteger(_op.annotation().type->category()), "");
...
void SMTChecker::compareOperation(BinaryOperation const& _op)
{
	...
	if (isSupportedType(_op.annotation().commonType->category()))
	{
		...
		{
			polUnimplementedAssert(isBool(_op.annotation().commonType->category()), "Operation not yet supported");
			value = make_shared<smt::Expression>(
				op == Token::Equal ? (left == right) :
				/*op == Token::NotEqual*/ (left != right)
			);
...
bool SMTChecker::createVariable(VariableDeclaration const& _varDecl)
{
	...
	polAssert(m_variables.count(&_varDecl) == 0, "");
...
{
	createExpr(_e);
	polAssert(smtKind(_e.annotation().type->category()) != smt::Kind::Function, "Equality operator applied to type that is not fully supported");
