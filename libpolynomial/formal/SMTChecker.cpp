	...
	m_context(*m_interface)
{
	...
	if (!_smtlib2Responses.empty())
		m_errorReporter.warning(
			...
			"but this Polynomial binary uses an SMT solver (Z3/CVC4) directly."
...
void SMTChecker::analyze(SourceUnit const& _source, shared_ptr<Scanner> const& _scanner)
{
	...
	polAssert(m_interface->solvers() > 0, "");
...
void SMTChecker::visitFunctionOrModifier()
{
	...
	polAssert(!m_modifierDepthStack.empty(), "");
...
void SMTChecker::endVisit(FunctionDefinition const&)
{
	...
	polAssert(m_modifierDepthStack.back() == -1, "");
...
void SMTChecker::endVisit(VariableDeclarationStatement const& _varDecl)
{
	if (_varDecl.declarations().size() != 1)
	{
		if (auto init = _varDecl.initialValue())
		{
			...
			if (!components.empty())
			{
				polAssert(components.size() == declarations.size(), "");
	...
	else if (_tuple.annotation().type->category() == Type::Category::Tuple)
	{
		...
		for (auto const& component: _tuple.components())
			if (component)
			{
				...
				{
					polAssert(m_context.knownExpression(*component), "");
					components.push_back(m_context.expression(*component));
				}
			}
			else
				components.push_back(nullptr);
		polAssert(components.size() == _tuple.components().size(), "");
...
{
	polAssert(_target.type != OverflowTarget::Type::Overflow, "");
...
{
	polAssert(_target.type != OverflowTarget::Type::Underflow, "");
	auto intType = dynamic_cast<IntegerType const*>(_target.intType);
	checkCondition(
...
void SMTChecker::endVisit(UnaryOperation const& _op)
{
	...
	case Token::Not: // !
	{
		polAssert(smt::isBool(_op.annotation().type->category()), "");
	...
	case Token::Dec: // -- (pre- or postfix)
	{
		...
		polAssert(smt::isInteger(_op.annotation().type->category()), "");
		polAssert(_op.subExpression().annotation().lValueRequested, "");
	...
	case Token::Delete:
	{
		...
		{
			polAssert(m_context.knownExpression(subExpr), "");
...
{
	polAssert(_funCall.annotation().kind != FunctionCallKind::Unset, "");
	createExpr(_funCall);
...
void SMTChecker::visitAssert(FunctionCall const& _funCall)
{
	...
	polAssert(args.size() == 1, "");
	polAssert(args[0]->annotation().type->category() == Type::Category::Bool, "");
...
void SMTChecker::inlineFunctionCall(FunctionCall const& _funCall)
{
	...
	else
	{
		...
		if (returnParams.size() > 1)
		{
			...
			{
				polAssert(m_context.variable(*param), "");
...
{
	polAssert(_funCall.annotation().kind == FunctionCallKind::TypeConversion, "");
	polAssert(_funCall.arguments().size() == 1, "");
...
{
	polAssert(_literal.annotation().type, "Expected type for AST node");
...
void SMTChecker::endVisit(Return const& _return)
{
	if (_return.expression() && m_context.knownExpression(*_return.expression()))
	{
		...
		if (returnParams.size() > 1)
		{
			...
			polAssert(components.size() == returnParams.size(), "");
...
void SMTChecker::endVisit(IndexAccess const& _indexAccess)
{
	...
	{
		polAssert(m_context.knownExpression(*innerAccess), "");
...
)
{
	...
	polAssert(validOperators.count(_op), "");
	polAssert(_commonType, "");
	polAssert(_commonType->category() == Type::Category::Integer, "");
...
{
	polAssert(_op.annotation().commonType, "");
	if (smt::isSupportedType(_op.annotation().commonType->category()))
	{
		...
		{
			polUnimplementedAssert(smt::isBool(_op.annotation().commonType->category()), "Operation not yet supported");
			value = make_shared<smt::Expression>(
				op == Token::Equal ? (left == right) :
				/*op == Token::NotEqual*/ (left != right)
			);
...
{
	polAssert(_op.getOperator() == Token::And || _op.getOperator() == Token::Or, "");
	polAssert(_op.annotation().commonType, "");
...
)
{
	...
	else if (auto tuple = dynamic_cast<TupleExpression const*>(&_left))
	{
		...
		if (!_right.empty())
		{
			polAssert(_right.size() == components.size(), "");
...
smt::Expression SMTChecker::compoundAssignment(Assignment const& _assignment)
{
	...
	Token op = _assignment.assignmentOperator();
	polAssert(compoundToArithmetic.count(op), "");
...
)
{
	...
	case smt::CheckResult::SATISFIABLE:
	{
		...
		if (m_callStack.size())
		{
			...
			modelMessage << "  for:\n";
			polAssert(values.size() == expressionNames.size(), "");
...
void SMTChecker::checkBooleanNotConstant(Expression const& _condition, string const& _description)
{
	...
	else
	{
		...
		{
			polAssert(negatedResult == smt::CheckResult::UNSATISFIABLE, "");
			value = "true";
		}
		else
		{
			polAssert(positiveResult == smt::CheckResult::UNSATISFIABLE, "");
			polAssert(negatedResult == smt::CheckResult::SATISFIABLE, "");
			value = "false";
		}
		m_errorReporter.warning(
			_condition.location(),
...
void SMTChecker::initializeFunctionCallParameters(CallableDeclaration const& _function, vector<smt::Expression> const& _callArgs)
{
	...
	polAssert(funParams.size() == _callArgs.size(), "");
...
void SMTChecker::mergeVariables(set<VariableDeclaration const*> const& _variables, smt::Expression const& _condition, VariableIndices const& _indicesEndTrue, VariableIndices const& _indicesEndFalse)
{
	...
	{
		polAssert(_indicesEndTrue.count(decl) && _indicesEndFalse.count(decl), "");
		int trueIndex = _indicesEndTrue.at(decl);
		int falseIndex = _indicesEndFalse.at(decl);
		polAssert(trueIndex != falseIndex, "");
...
{
	polAssert(m_context.knownVariable(_decl), "");
...
{
	createExpr(_e);
	polAssert(smt::smtKind(_e.annotation().type->category()) != smt::Kind::Function, "Equality operator applied to type that is not fully supported");
	m_interface->addAssertion(expr(_e) == _value);
}

void SMTChecker::popPathCondition()
{
	polAssert(m_pathConditions.size() > 0, "Cannot pop path condition, empty.");
...
SecondarySourceLocation SMTChecker::currentCallStack()
{
	...
	polAssert(m_callStack.front().second == nullptr, "");
...
FunctionDefinition const* SMTChecker::inlinedFunctionCallToDefinition(FunctionCall const& _funCall)
{
	...
	{
		polAssert(fun->components().size() == 1, "");
...
VariableDeclaration const* SMTChecker::identifierToVariable(Expression const& _expr)
{
	if (auto identifier = dynamic_cast<Identifier const*>(&_expr))
	{
		...
		{
			polAssert(m_context.knownVariable(*decl), "");
			return decl;
		}
	}
	return nullptr;
