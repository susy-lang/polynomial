...
void SMTChecker::endVisit(UnaryOperation const& _op)
{
	...
	case Token::Not: // !
	{
		polAssert(_op.annotation().type->category() == Type::Category::Bool, "");
	...
	{
		polAssert(_op.annotation().type->category() == Type::Category::Integer, "");
...
void SMTChecker::endVisit(Literal const& _literal)
{
	...
	if (type.category() == Type::Category::Integer || type.category() == Type::Category::RationalNumber)
	{
		if (RationalNumberType const* rational = dynamic_cast<RationalNumberType const*>(&type))
			polAssert(!rational->isFractional(), "");
...
void SMTChecker::arithmeticOperation(BinaryOperation const& _op)
{
	...
	case Token::Div:
	{
		...
		polAssert(_op.annotation().commonType->category() == Type::Category::Integer, "");
...
void SMTChecker::mergeVariables(vector<Declaration const*> const& _variables, smt::Expression const& _condition, VariableSequenceCounters const& _countersEndTrue, VariableSequenceCounters const& _countersEndFalse)
{
	...
	for (auto const* decl: uniqueVars)
	{
		...
		polAssert(trueCounter != falseCounter, "");
...
bool SMTChecker::createVariable(VariableDeclaration const& _varDecl)
{
	...
	{
		polAssert(m_currentSequenceCounter.count(&_varDecl) == 0, "");
		polAssert(m_nextFreeSequenceCounter.count(&_varDecl) == 0, "");
...
{
	polAssert(m_currentSequenceCounter.count(&_decl), "");
...
{
	polAssert(m_nextFreeSequenceCounter.count(&_decl), "");
...
{
	polAssert(_decl.type()->category() == Type::Category::Integer, "");
...
{
	polAssert(m_variables.count(&_decl), "");
