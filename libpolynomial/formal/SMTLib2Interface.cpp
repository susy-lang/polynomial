...
{
	polAssert(!m_accumulatedOutput.empty(), "");
...
{
	polAssert(_sort.kind == smt::Kind::Function, "");
...
string SMTLib2Interface::toSmtLibSort(Sort const& _sort)
{
	...
	default:
		polAssert(false, "Invalid SMT sort");
	}
}
...
string SMTLib2Interface::checkSatAndGetValuesCommand(vector<Expression> const& _expressionsToEvaluate)
{
	...
	else
	{
		...
		for (size_t i = 0; i < _expressionsToEvaluate.size(); i++)
		{
			...
			polAssert(e.sort->kind == Kind::Int || e.sort->kind == Kind::Bool, "Invalid sort for expression to evaluate.");
