 ...
 * @date 2015
 * Component that translates Polynomial code into the why3 programming language.
 */
...
bool Why3Translator::process(SourceUnit const& _source)
{
	...
	{
		polAssert(false, "There is a call to toFormalType() that does not catch NoFormalType exceptions.");
	}
	catch (FatalError& /*_e*/)
	{
...
string Why3Translator::toFormalType(Type const& _type) const
{
	...
	{
		polAssert(mappingType->keyType(), "A mappingType misses a keyType.");
		if (dynamic_cast<IntegerType const*>(&*mappingType->keyType()))
		{
			...
			polAssert(mappingType->valueType(), "A mappingType misses a valueType.");
			// Not catching NoFormalType exception.  Let the caller deal with it.
...
bool Why3Translator::visit(Return const& _node)
{
	...
	if (_node.expression())
	{
		polAssert(!!_node.annotation().functionReturnParameters, "");
...
bool Why3Translator::visit(FunctionCall const& _node)
{
	...
	{
		add("let amount = ");
		polAssert(_node.arguments().size() == 1, "");
...
bool Why3Translator::visit(PragmaDirective const& _pragma)
{
	if (_pragma.tokens().empty())
		...
		error(_pragma, "Not supported");
	else if (_pragma.literals()[0] != "polynomial")
		error(_pragma, "Not supported");
	else if (_pragma.tokens()[0] != Token::Identifier)
		error(_pragma, "A literal 'polynomial' is not an identifier.  Strange");
