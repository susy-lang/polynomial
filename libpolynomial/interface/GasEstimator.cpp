...
{
	polAssert(std::count(_ast.begin(), _ast.end(), nullptr) == 0, "");
...
{
	polAssert(std::count(_roots.begin(), _roots.end(), nullptr) == 0, "");
...
)
{
	...
	if (!_signature.empty())
	{
		...
		Id calldata = classes.find(sof::Instruction::CALLDATALOAD, Ids{classes.find(u256(0))});
		classes.forceEqual(hashValue, sof::Instruction::DIV, Ids{
			calldata,
...
)
{
	...
	if (parametersSize > 0)
		state->feedItem(sof::swapInstruction(parametersSize));
