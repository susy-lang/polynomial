...
)
{
	...
	{
		polAssert(false, "Assembly optimizer exception for bytecode");
	}
	...
	{
		polAssert(false, "Assembly optimizer exception for deployed bytecode");
	}
	catch(sof::AssemblyException const&)
	{
	...
	try
	{
		...
		{
			Compiler cloneCompiler(m_svmVersion, m_optimize, m_optimizeRuns);
...
Json::Value CompilerStack::gasEstimates(string const& _contractName) const
{
	...
	if (sof::AssemblyItems const* items = assemblyItems(_contractName))
	{
		...
		Gas codeDepositGas = bytecodeSize * sof::GasCosts::createDataGas;
