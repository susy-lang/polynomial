...
#include <libpolynomial/codegen/ir/IRGenerator.h>
	...
	m_errorReporter{m_errorList}
{
	...
	polAssert(g_compilerStackCounts == 0, "You shall not have another CompilerStack aside me.");
...
void CompilerStack::setRemappings(vector<Remapping> const& _remappings)
{
	...
	for (auto const& remapping: _remappings)
		polAssert(!remapping.prefix.empty(), "");
	m_remappings = _remappings;
}

void CompilerStack::setSVMVersion(langutil::SVMVersion _version)
{
	if (m_stackState >= ParsingSuccessful)
		BOOST_THROW_EXCEPTION(CompilerError() << errinfo_comment("Must set SVM version before parsing."));
	m_svmVersion = _version;
}
...
void CompilerStack::reset(bool _keepSettings)
{
	...
	if (!_keepSettings)
	{
		m_remappings.clear();
		m_libraries.clear();
		m_svmVersion = langutil::SVMVersion();
		m_generateIR = false;
...
bool CompilerStack::parse()
{
	...
	for (size_t i = 0; i < sourcesToParse.size(); ++i)
	{
		...
		source.ast = Parser(m_errorReporter, m_svmVersion).parse(source.scanner);
		if (!source.ast)
			polAssert(!Error::containsOnlyWarnings(m_errorReporter.errors()), "Parser returned null but did not report error.");
		else
		{
...
bool CompilerStack::analyze()
{
	...
	try {
		...
		TypeChecker typeChecker(m_svmVersion, m_errorReporter);
		for (Source const* source: m_sourceOrder)
			for (ASTPointer<ASTNode> const& node: source->ast->nodes())
				if (ContractDefinition* contract = dynamic_cast<ContractDefinition*>(node.get()))
...
{
	polAssert(m_stackState >= CompilationSuccessful, "");
...
{
	polAssert(m_stackState < ParsingSuccessful, "");
	StringMap newSources;
	for (auto const& node: _ast.nodes())
		if (ImportDirective const* import = dynamic_cast<ImportDirective*>(node.get()))
		{
			polAssert(!import->path().empty(), "Import path cannot be empty.");
...
{
	polAssert(m_stackState >= AnalysisSuccessful, "");
	...
	shared_ptr<Compiler> compiler = make_shared<Compiler>(m_svmVersion, m_optimiserSettings);
	...
	catch(sof::OptimizerException const&)
	{
		polAssert(false, "Optimizer exception during compilation");
	}
	...
	catch(sof::AssemblyException const&)
	{
		polAssert(false, "Assembly exception for bytecode");
	}
	...
	{
		polAssert(false, "Assembly exception for deployed bytecode");
	}
...
void CompilerStack::generateIR(ContractDefinition const& _contract)
{
	...
	IRGenerator generator(m_svmVersion, m_optimiserSettings);
...
string CompilerStack::createMetadata(Contract const& _contract) const
{
	...
	polAssert(static_cast<Json::LargestUInt>(m_optimiserSettings.expectedExecutionsPerDeployment) < std::numeric_limits<Json::LargestUInt>::max(), "");
	...
	meta["settings"]["svmVersion"] = m_svmVersion.name();
...
public:
	...
	bytes serialise() const
	{
		...
		polAssert(size <= 0xffff, "Metadata too large.");
		polAssert(m_entryCount <= 0x1f, "Too many map entries.");
...
private:
	void pushTextString(string const& key)
	{
		...
		else if (length <= 256)
		{
			...
			polAssert(false, "Text string too large.");
	}
	void pushByteString(bytes const& key)
	{
		...
		else if (length <= 256)
		{
			...
			polAssert(false, "Byte string too large.");
	}
...
bytes CompilerStack::createCBORMetadata(string const& _metadata, bool _experimentalMode)
{
	...
	if (m_release)
		encoder.pushBytes("polc", VersionCompactBytes);
	else
		encoder.pushString("polc", VersionStringStrict);
...
string CompilerStack::computeSourceMapping(sof::AssemblyItems const& _items) const
{
	...
	for (auto const& item: _items)
	{
		...
		int sourceIndex =
			...
			-1;
		char jump = '-';
		if (item.getJumpType() == sof::AssemblyItem::JumpType::IntoFunction)
			jump = 'i';
		else if (item.getJumpType() == sof::AssemblyItem::JumpType::OutOfFunction)
			jump = 'o';
...
Json::Value CompilerStack::gasEstimates(string const& _contractName) const
{
	...
	GasEstimator gasEstimator(m_svmVersion);
	Json::Value output(Json::objectValue);

	if (sof::AssemblyItems const* items = assemblyItems(_contractName))
	{
		Gas executionGas = gasEstimator.functionalEstimation(*items);
		Gas codeDepositGas{sof::GasMeter::dataGas(runtimeObject(_contractName).bytecode, false)};
	...
	if (sof::AssemblyItems const* items = runtimeAssemblyItems(_contractName))
	{
		/// External functions
