...
Json::Value StandardCompiler::compile(Json::Value const& _input) noexcept
{
	try
	{
		...
		if (settings.language == "Polynomial")
			return compilePolynomial(std::move(settings));
		else if (settings.language == "Yul")
			return compileYul(std::move(settings));
		else
			return formatFatalError("JSONError", "Only \"Polynomial\" or \"Yul\" is supported as a language.");
	}
