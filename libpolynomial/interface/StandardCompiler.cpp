...
Json::Value StandardCompiler::compilePolynomial(StandardCompiler::InputsAndSettings _inputsAndSettings)
{
...
Json::Value StandardCompiler::compile(Json::Value const& _input) noexcept
{
	try
	{
		...
		if (parsed.type() == typeid(InputsAndSettings))
			return compilePolynomial(boost::get<InputsAndSettings>(std::move(parsed)));
		else
