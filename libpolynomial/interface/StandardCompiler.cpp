...
bool isBinaryRequested(Json::Value const& _outputSelection)
{
	...
	static vector<string> const outputsThatRequireBinaries{
		...
		"svm.bytecode.linkReferences",
		"svm.gasEstimates", "svm.legacyAssembly", "svm.assembly"
	};
...
Json::Value collectSVMObject(sof::LinkerObject const& _object, string const* _sourceMap)
{
	Json::Value output = Json::objectValue;
...
{
	static set<string> keys{"svmVersion", "libraries", "metadata", "optimizer", "outputSelection", "remappings"};
...
Json::Value StandardCompiler::compileInternal(Json::Value const& _input)
{
	...
	if (_input["language"] != "Polynomial")
		return formatFatalError("JSONError", "Only \"Polynomial\" is supported as a language.");
	...
	if (settings.isMember("svmVersion"))
	{
		if (!settings["svmVersion"].isString())
			return formatFatalError("JSONError", "svmVersion must be a string.");
		boost::optional<langutil::SVMVersion> version = langutil::SVMVersion::fromString(settings["svmVersion"].asString());
		if (!version)
			return formatFatalError("JSONError", "Invalid SVM version requested.");
		m_compilerStack.setSVMVersion(*version);
	}
	...
	for (string const& contractName: analysisSuccess ? m_compilerStack.contractNames() : vector<string>())
	{
		...
		// @TODO: add ir
		if (compilationSuccess && isArtifactRequested(outputSelection, file, name, "svm.assembly"))
			svmData["assembly"] = m_compilerStack.assemblyString(contractName, createSourceList(_input));
		if (compilationSuccess && isArtifactRequested(outputSelection, file, name, "svm.legacyAssembly"))
		...
		if (compilationSuccess && isArtifactRequested(outputSelection, file, name, "svm.gasEstimates"))
