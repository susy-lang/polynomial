	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			else
			{
				...
				m_context << sof::swapInstruction(arrayType.sizeOnStack() + 1) << sof::Instruction::POP;
				// stack: ... <end_of_mem''> <value...>
				// copy data part
				ArrayUtils(m_context).copyArrayToMemory(arrayType, _padToWordBoundaries);
				// stack: ... <end_of_mem'''>
			...
			if (!targetType || *sourceType != *targetType || _cleanupNeeded)
			{
				...
				if (sourceSize > 0 || targetSize > 0)
				{
					...
					for (unsigned j = 0; j < min(sourceSize, targetSize); ++j)
						m_context <<
							sof::swapInstruction(depth + targetSize - sourceSize) <<
							sof::Instruction::POP;
					// Value shrank
...
void CompilerUtils::cleanHigherOrderBits(IntegerType const& _typeOnStack)
{
	if (_typeOnStack.numBits() == 256)
		...
		m_context << u256(_typeOnStack.numBits() / 8 - 1) << sof::Instruction::SIGNEXTEND;
	else
		m_context << ((u256(1) << _typeOnStack.numBits()) - 1) << sof::Instruction::AND;
}
