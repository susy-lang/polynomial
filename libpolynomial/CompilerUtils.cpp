...
#include <libpolynomial/ArrayUtils.h>
...
{
	m_context << u256(freeMemoryPointer) << sof::Instruction::MLOAD;
}

void CompilerUtils::storeFreeMemoryPointer()
{
	m_context << u256(freeMemoryPointer) << sof::Instruction::MSTORE;
}
...
{
	fetchFreeMemoryPointer();
	m_context << sof::Instruction::DUP1 << sof::Instruction::SWAP2 << sof::Instruction::SUB;
...
{
	polAssert(_type.getCategory() != Type::Category::Array, "Unable to statically load dynamic type.");
...
{
	polAssert(_type.getCategory() != Type::Category::Array, "Arrays not yet implemented.");
...
void CompilerUtils::storeInMemory(unsigned _offset)
{
	...
	if (numBytes > 0)
		m_context << u256(_offset) << sof::Instruction::MSTORE;
}
...
void CompilerUtils::storeInMemoryDynamic(Type const& _type, bool _padToWordBoundaries)
{
	...
	{
		polAssert(ref->location() == DataLocation::Memory, "");
...
)
{
	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			m_context << sof::Instruction::DUP1 << u256(32) << sof::Instruction::ADD;
			dynPointers++;
		}
		else
		{
	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			m_context << sof::dupInstruction(2 + dynPointers) << sof::Instruction::DUP2;
			m_context << sof::Instruction::SUB;
			m_context << sof::dupInstruction(2 + dynPointers - thisDynPointer);
			...
			{
				polAssert(_givenTypes[i]->getCategory() == Type::Category::Array, "Unknown dynamic type.");
				auto const& arrayType = dynamic_cast<ArrayType const&>(*_givenTypes[i]);
				// now copy the array
				copyToStackTop(argSize - stackPos + dynPointers + 2, arrayType.getSizeOnStack());
				// stack: ... <end_of_mem> <value...>
				// copy length to memory
				m_context << sof::dupInstruction(1 + arrayType.getSizeOnStack());
				...
				m_context << sof::swapInstruction(arrayType.getSizeOnStack() + 1) << sof::Instruction::POP;
				// stack: ... <end_of_mem''> <value...>
				// copy data part
				ArrayUtils(m_context).copyArrayToMemory(arrayType, _padToWordBoundaries);
				// stack: ... <end_of_mem'''>
	...
	m_context << sof::swapInstruction(argSize + dynPointers + 1);
...
void CompilerUtils::memoryCopy()
{
	...
	m_context << sof::Instruction::DUP3 << sof::Instruction::SWAP1;
...
void CompilerUtils::convertType(Type const& _typeOnStack, Type const& _targetType, bool _cleanupNeeded)
{
	...
	case Type::Category::FixedBytes:
	{
		...
		if (targetTypeCategory == Type::Category::Integer)
		{
			...
			m_context << (u256(1) << (256 - typeOnStack.numBytes() * 8)) << sof::Instruction::SWAP1 << sof::Instruction::DIV;
		...
		else
		{
			...
			if (targetType.numBytes() < typeOnStack.numBytes())
			{
				if (targetType.numBytes() == 0)
					m_context << sof::Instruction::DUP1 << sof::Instruction::XOR;
				else
				{
	...
	case Type::Category::Enum:
		polAssert(targetTypeCategory == Type::Category::Integer || targetTypeCategory == Type::Category::Enum, "");
		break;
	...
	case Type::Category::IntegerConstant:
		...
		{
			polAssert(stackTypeCategory == Type::Category::Integer || stackTypeCategory == Type::Category::IntegerConstant,
			...
			m_context << (u256(1) << (256 - targetBytesType.numBytes() * 8)) << sof::Instruction::MUL;
		}
		...
		{
			polAssert(targetTypeCategory == Type::Category::Integer || targetTypeCategory == Type::Category::Contract, "");
	...
	case Type::Category::StringLiteral:
	{
		...
		else if (targetTypeCategory == Type::Category::Array)
		{
			...
			allocateMemory();
			// stack: mempos
			m_context << sof::Instruction::DUP1 << u256(data.size());
	...
	case Type::Category::Array:
	{
		...
		case DataLocation::Memory:
		{
			...
			if (typeOnStack.location() != DataLocation::Memory)
			{
				...
				{
					polAssert(typeOnStack.getBaseType()->isValueType(), "");
	...
	case Type::Category::Struct:
	{
		...
		case DataLocation::Memory:
			...
			if (typeOnStack.location() != DataLocation::Memory)
			{
				polAssert(typeOnStack.location() == DataLocation::Storage, "");
				// stack: <source ref>
...
void CompilerUtils::pushZeroValue(const Type& _type)
{
	...
	else if (auto arrayType = dynamic_cast<ArrayType const*>(&_type))
	{
		...
		{
			m_context << arrayType->getLength() << sof::Instruction::SWAP1;
		...
		polAssert(false, "Requested initialisation for unknown type: " + _type.toString());
...
void CompilerUtils::moveToStackVariable(VariableDeclaration const& _variable)
{
	...
	for (unsigned i = 0; i < size; ++i)
		m_context << sof::swapInstruction(stackPosition - size + 1) << sof::Instruction::POP;
}
...
void CompilerUtils::copyToStackTop(unsigned _stackDepth, unsigned _itemSize)
{
	...
	for (unsigned i = 0; i < _itemSize; ++i)
		m_context << sof::dupInstruction(_stackDepth);
}
...
{
	storeInMemory(0);
	m_context << u256(32) << u256(0) << sof::Instruction::SHA3;
}
...
void CompilerUtils::storeStringData(bytesConstRef _data)
{
	...
	else
	{
		...
		m_context << u256(_data.size()) << sof::Instruction::SWAP2;
		m_context << sof::Instruction::CODECOPY;
	}
}
...
unsigned CompilerUtils::loadFromMemoryHelper(Type const& _type, bool _fromCalldata, bool _padToWordBoundaries)
{
	...
	if (numBytes == 0)
		m_context << sof::Instruction::POP << u256(0);
	else
	{
		polAssert(numBytes <= 32, "Static memory load of more than 32 bytes requested.");
		m_context << (_fromCalldata ? sof::Instruction::CALLDATALOAD : sof::Instruction::MLOAD);
		if (numBytes != 32)
		{
			// add leading or trailing zeros by dividing/multiplying depending on alignment
			u256 shiftFactor = u256(1) << ((32 - numBytes) * 8);
			m_context << shiftFactor << sof::Instruction::SWAP1 << sof::Instruction::DIV;
			if (leftAligned)
				m_context << shiftFactor << sof::Instruction::MUL;
		}
	}
...
void CompilerUtils::cleanHigherOrderBits(IntegerType const& _typeOnStack)
{
	if (_typeOnStack.getNumBits() == 256)
		...
		m_context << u256(_typeOnStack.getNumBits() / 8 - 1) << sof::Instruction::SIGNEXTEND;
	else
		m_context << ((u256(1) << _typeOnStack.getNumBits()) - 1) << sof::Instruction::AND;
}
...
unsigned CompilerUtils::prepareMemoryStore(Type const& _type, bool _padToWordBoundaries) const
{
	...
	{
		polAssert(numBytes <= 32, "Memory store of more than 32 bytes requested.");
		if (numBytes != 32 && !leftAligned && !_padToWordBoundaries)
			// shift the value accordingly before storing
			m_context << (u256(1) << ((32 - numBytes) * 8)) << sof::Instruction::MUL;
	}
	return numBytes;
}
