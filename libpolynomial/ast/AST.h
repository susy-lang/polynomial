...
/**
 * Pragma directive, only version requirements in the form `pragma polynomial "^0.4.0";` are
 * supported for now.
 */
...
public:
	...
	Block const& body() const { polAssert(m_body, ""); return *m_body; }
...
public:
	...
	{
		polAssert(false, "MagicVariableDeclaration used inside real AST.");
	}
...
public:
	...
	{
		polAssert(!_stateMutability.is_initialized() || _elem.token() == Token::Address, "");
	}
...
/**
 * @brief The Throw statement to throw that triggers a polynomial exception(jump to ErrorTag)
 */
...
public:
	...
	{
		polAssert(TokenTraits::isAssignmentOp(_assignmentOperator), "");
	}
	void accept(ASTVisitor& _visitor) override;
	void accept(ASTConstVisitor& _visitor) const override;
...
public:
	...
	{
		polAssert(TokenTraits::isUnaryOp(_operator), "");
	}
	void accept(ASTVisitor& _visitor) override;
	void accept(ASTConstVisitor& _visitor) const override;
...
public:
	...
	{
		polAssert(TokenTraits::isBinaryOp(_operator) || TokenTraits::isCompareOp(_operator), "");
	}
	void accept(ASTVisitor& _visitor) override;
	void accept(ASTConstVisitor& _visitor) const override;
...
public:
	enum class SubDenomination
	{
		...
		Sophy = static_cast<int>(Token::SubSophy),
