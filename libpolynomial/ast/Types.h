...
{
	polAssert(_denominator != 0, "division by zero");
...
public:
	...
	TypePointer memberType(std::string const& _name) const
	{
		...
		for (auto const& it: m_memberTypes)
			if (it.name == _name)
			{
				polAssert(!type, "Requested member type by non-unique name.");
				type = it.type;
			}
		return type;
	}
...
public:
	...
	/// @returns a valid polynomial identifier such that two types should compare equal if and
	...
	/// sign-extended.
	virtual bool leftAligned() const { polAssert(false, "Alignment property of non-value type requested."); }
	...
	{
		polAssert(false, "Literal value requested for type without literals: " + toString(false));
	}
	...
	/// @returns a type that will be used outside of Polynomial for e.g. function signatures.
	/// This for example returns address for contract types.
	/// If there is no such type, returns an empty shared pointer.
...
public:
	...
	TypePointer const& baseType() const { polAssert(!!m_baseType, ""); return m_baseType;}
...
public:
	...
	unsigned storageBytes() const override { polAssert(!isSuper(), ""); return 20; }
	bool leftAligned() const override { polAssert(!isSuper(), ""); return false; }
...
{
public:
	/// How this function is invoked on the SVM.
	enum class Kind
	{
	...
	{
		polAssert(m_declaration, "Requested declaration from a FunctionType that has none");
...
public:
	...
	bool hasSimpleZeroValueInMemory() const override { polAssert(false, ""); }
 ...
 * (The SVM currently cannot access dynamically-sized return values).
 */
