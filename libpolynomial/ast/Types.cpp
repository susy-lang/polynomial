...
string Type::identifier() const
{
	...
	polAssert(ret.find_first_of("0123456789") != 0, "Identifier cannot start with a number.");
	polAssert(
...
{
	polAssert(TokenTraits::isElementaryTypeName(_type.token()),
...
TypePointer Type::fromElementaryTypeName(string const& _name)
{
	...
	polAssert(nameParts.size() == 1 || nameParts.size() == 2, "Cannot parse elementary type: " + _name);
	Token token;
	...
	if (auto* ref = dynamic_cast<ReferenceType const*>(t.get()))
	{
		...
		if (nameParts.size() == 2)
		{
			if (nameParts[1] == "storage")
				...
				polAssert(false, "Unknown data location: " + nameParts[1]);
		}
	...
	else if (t->category() == Type::Category::Address)
	{
		if (nameParts.size() == 2)
		{
			if (nameParts[1] == "payable")
				...
				polAssert(false, "Invalid state mutability for address type: " + nameParts[1]);
		}
		return make_shared<AddressType>(StateMutability::NonPayable);
	}
	else
	{
		polAssert(nameParts.size() == 1, "Storage location suffix only allowed for reference types");
		return t;
	}
}
...
{
	polAssert(m_stateMutability == StateMutability::Payable || m_stateMutability == StateMutability::NonPayable, "");
}
...
{
	polAssert(_literal, "");
	polAssert(_literal->value().substr(0, 2) == "0x", "");
...
{
	polAssert(_literal.token() == Token::Number, "");
...
tuple<bool, rational> RationalNumberType::isValidLiteral(Literal const& _literal)
{
	...
	switch (_literal.subDenomination())
	{
		...
		case Literal::SubDenomination::Finney:
			...
			break;
		case Literal::SubDenomination::Sophy:
	...
	else
	{
		...
		case Token::Exp:
		{
			if (other.isFractional())
				return TypePointer();
			polAssert(other.m_value.denominator() == 1, "");
...
u256 RationalNumberType::literalValue(Literal const*) const
{
	...
	else
	{
		...
		polAssert(fixed, "Rational number cannot be represented as fixed point type.");
	...
	polAssert(shiftedValue <= u256(-1), "Number constant too large.");
	polAssert(shiftedValue >= -(bigint(1) << 255), "Number constant too small.");
...
{
	polAssert(!isFractional(), "integerType() called for fractional number.");
...
u256 BoolType::literalValue(Literal const* _literal) const
{
	...
	if (_literal->token() == Token::TrueLiteral)
		...
		polAssert(false, "Bool type constructed from non-boolean literal.");
}
...
unsigned ArrayType::calldataEncodedSize(bool _padded) const
{
	...
	polAssert(size <= numeric_limits<unsigned>::max(), "Array size does not fit unsigned.");
...
TypeResult ArrayType::interfaceType(bool _inLibrary) const
{
	...
	{
		polAssert(!baseInterfaceType.message().empty(), "Expected detailed error message!");
...
{
	polAssert(!isDynamicallySized(), "");
	polAssert(m_location == DataLocation::Memory, "");
	bigint size = bigint(m_length) * m_baseType->memoryHeadSize();
	polAssert(size <= numeric_limits<unsigned>::max(), "Array size does not fit u256.");
	return u256(size);
}
...
MemberList::MemberMap ContractType::nativeMembers(ContractDefinition const* _contract) const
{
	...
	if (m_super)
	{
		...
		polAssert(bases.size() >= 1, "linearizedBaseContracts should at least contain the most derived contract.");
		...
		for (ContractDefinition const* base: bases | boost::adaptors::sliced(1, bases.size()))
			for (FunctionDefinition const* function: base->definedFunctions())
			{
				...
				for (auto const& member: members)
				{
					...
					polAssert(!!memberType, "Override changes type.");
...
unsigned StructType::calldataOffsetOfMember(std::string const& _member) const
{
	...
	for (auto const& member: members(nullptr))
	{
			...
			return offset;
		{
			// Struct members are always padded.
			unsigned memberSize = member.type->calldataEncodedSize(true);
			polAssert(memberSize != 0, "");
			offset += memberSize;
		}
	}
	polAssert(false, "Struct member not found.");
}

bool StructType::isDynamicallyEncoded() const
{
	polAssert(interfaceType(false).get(), "");
	for (auto t: memoryMemberTypes())
	{
		polAssert(t, "Parameter should have external type.");
...
TypeResult StructType::interfaceType(bool _inLibrary) const
{
	...
	)
	{
		...
		for (ASTPointer<VariableDeclaration> const& variable: _struct.members())
		{
			...
			if (!iType.get())
			{
				polAssert(!iType.message().empty(), "Expected detailed error message!");
				result = iType;
				return;
			}
		}
...
string StructType::signatureInExternalFunction(bool _structsByName) const
{
	...
	else
	{
		...
		{
			polAssert(_t, "Parameter should have external type.");
...
pair<u256, unsigned> const& StructType::storageOffsetsOfMember(string const& _name) const
{
	...
	polAssert(offsets, "Storage offset of non-existing member requested.");
	return *offsets;
}
...
u256 StructType::memoryOffsetOfMember(string const& _name) const
{
	...
	polAssert(false, "Member not found in struct.");
	return 0;
}
...
unsigned EnumType::memberValue(ASTString const& _member) const
{
	...
	polAssert(false, "Requested unknown enum value " + _member);
}
...
{
	polAssert(false, "Storage size of non-storable tuple type requested.");
}
...
TypePointer TupleType::closestTemporaryType(TypePointer const& _targetType) const
{
	...
	polAssert(components().size() == targetComponents.size(), "");
	...
	m_stateMutability(_typeName.stateMutability())
{
	if (_typeName.isPayable())
		polAssert(m_kind == Kind::External, "Internal payable function type used.");
	...
	{
		polAssert(t->annotation().type, "Type not set for return parameter.");
		if (m_kind == Kind::External)
			polAssert(
				t->annotation().type->interfaceType(false).get(),
...
FunctionTypePointer FunctionType::newExpressionType(ContractDefinition const& _contract)
{
	...
	polAssert(!_contract.isInterface(), "");
...
BoolResult FunctionType::isImplicitlyConvertibleTo(Type const& _convertTo) const
{
	...
	// payable should be convertible to non-payable, because you are free to pay 0 sophy
...
{
	polAssert(m_kind == Kind::External, "");
	return "function";
}
...
u256 FunctionType::storageSize() const
{
	if (m_kind == Kind::External || m_kind == Kind::Internal)
		...
		polAssert(false, "Storage size of non-storable function type requested.");
}
...
bool FunctionType::leftAligned() const
{
	if (m_kind == Kind::External)
		...
		polAssert(false, "Alignment property of non-exportable function type requested.");
}
...
unsigned FunctionType::sizeOnStack() const
{
	...
	{
		polAssert(m_returnParameterTypes.size() == 1, "");
...
FunctionTypePointer FunctionType::interfaceFunctionType() const
{
	...
	polAssert(m_declaration, "Declaration needed to determine interface function type.");
...
) const
{
	...
	{
		polAssert(_arguments.numArguments() == _arguments.numNames(), "Expected equal sized type & name vectors");
...
bool FunctionType::equalExcludingStateMutability(FunctionType const& _other) const
{
	...
	polAssert(!bound() || *selfType() == *_other.selfType(), "");
...
{
	polAssert(m_declaration != nullptr, "External signature of function needs declaration");
	polAssert(!m_declaration->name().empty(), "Fallback function has no signature.");
	switch (kind())
	{
	case Kind::Internal:
	case Kind::External:
	...
	case Kind::Event:
		break;
	default:
		polAssert(false, "Invalid function type for requesting external signature.");
	}
	...
	polAssert(!!external, "External function type requested.");
...
{
	if (_bound)
		polAssert(!m_parameterTypes.empty(), "");
	...
	if (_inLibrary)
	{
		polAssert(!!m_declaration, "Declaration has to be available.");
...
{
	polAssert(bound(), "Function is not bound.");
	polAssert(m_parameterTypes.size() > 0, "Function has no self type.");
...
{
	polAssert(keyType()->interfaceType(_inLibrary).get(), "Must be an elementary type!");
...
{
	polAssert(false, "Storage size of non-storable type type requested.");
}
...
{
	polAssert(_type && _type->category() == Type::Category::Contract, "Only contracts supported for now.");
...
MemberList::MemberMap MagicType::nativeMembers(ContractDefinition const*) const
{
	...
	polAssert(false, "Unknown kind of magic.");
	return {};
}
...
{
	polAssert(m_kind == Kind::MetaType, "");
	polAssert(m_typeArgument, "");
