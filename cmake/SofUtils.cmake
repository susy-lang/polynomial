...
macro(sof_add_test NAME) 
	...
	while (index LESS count)
		...
		add_test(NAME "${NAME}.${index}" COMMAND testsof ${run_test} ${test_arguments})
	...
	add_custom_target("test.${NAME}"
		...
		COMMAND ${CMAKE_COMMAND} -DSOF_TEST_NAME="${NAME}" -DCTEST_COMMAND="${CTEST_COMMAND}" -P "${SOF_SCRIPTS_DIR}/runtest.cmake"
	)

endmacro()

# Creates C resources file from files
function(sof_add_resources RESOURCE_FILE OUT_FILE SOF_RES_DIR)
	include("${RESOURCE_FILE}")
	set(OUTPUT  "${SOF_RESOURCE_LOCATION}/${SOF_RESOURCE_NAME}.hpp")
	#message(FATAL_ERROR "res:! ${SOF_RESOURCE_LOCATION}")
	include_directories("${SOF_RESOURCE_LOCATION}")
	set(${OUT_FILE} "${OUTPUT}"  PARENT_SCOPE)

	set(filenames "${RESOURCE_FILE}")
	list(APPEND filenames "${SOF_SCRIPTS_DIR}/resources.cmake")
	foreach(resource ${SOF_RESOURCES})
		list(APPEND filenames "${${resource}}")
	endforeach(resource)

	add_custom_command(OUTPUT ${OUTPUT}
		COMMAND ${CMAKE_COMMAND} -DSOF_RES_FILE="${RESOURCE_FILE}" -DSOF_RES_DIR="${SOF_RES_DIR}"  -P "${SOF_SCRIPTS_DIR}/resources.cmake"
		DEPENDS ${filenames}
	)
endfunction()

macro(sof_default_option O DEF)
	if (DEFINED ${O})
		if (${${O}})
			set(${O} ON)
		else ()
...
macro(sof_check_library_link L)
