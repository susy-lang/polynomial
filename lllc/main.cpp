...
int main(int argc, char** argv)
{
	...
	{
		cout << dev::sof::disassemble(fromHex(src)) << endl;
	}
	else if (mode == Binary || mode == Hex)
	{
		auto bs = compileLLL(std::move(src), langutil::SVMVersion{}, optimise ? true : false, &errors, readFileAsString);
		if (mode == Hex)
			cout << toHex(bs) << endl;
		else if (mode == Binary)
	...
	{
		cout << compileLLLToAsm(std::move(src), langutil::SVMVersion{}, optimise ? true : false, &errors, readFileAsString) << endl;
	}
