Checklist for making a release:
 ...
 - [ ] Once the ``~sophon/ubuntu/sophon-static`` PPA build is finished and published for all platforms (make sure not to do this earlier), copy the static package to the ``~sophon/ubuntu/sophon`` PPA for the destination series ``Trusty`` and ``Xenial`` while selecting ``Copy existing binaries``.
