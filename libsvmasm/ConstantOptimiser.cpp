...
{
	return sof::bytesRequired(_items, 3); // assume 3 byte addresses
}
...
AssemblyItems ComputeMethod::findRepresentation(u256 const& _value)
{
	...
	else
	{
		...
		for (unsigned bits = 255; bits > 8 && m_maxSteps > 0; --bits)
		{
			...
			if (m_params.svmVersion.hasBitwiseShifting())
			{
...
{
	// This is a tiny SVM that can only evaluate some instructions.
	vector<u256> stack;
	for (AssemblyItem const& item: _routine)
	{
		...
		case Operation:
		{
			...
			case Instruction::SHL:
				assertThrow(
					m_params.svmVersion.hasBitwiseShifting(),
					OptimizerException,
					"Shift generated for invalid SVM version."
				);
