...
GasMeter::GasConsumption PathGasMeter::handleQueueItem()
{
	...
	for (; index < m_items.size() && !gas.isInfinite; ++index)
	{
		...
		if (item.type() == Tag || item == AssemblyItem(sof::Instruction::JUMPDEST))
		{
		...
		else if (item == AssemblyItem(sof::Instruction::JUMP))
		{
			branchStops = true;
		...
		else if (item == AssemblyItem(sof::Instruction::JUMPI))
		{
