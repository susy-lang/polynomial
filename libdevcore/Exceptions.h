...
/// Base class for all SRLP exceptions.
struct SRLPException: virtual Exception { SRLPException(std::string _message = std::string()): Exception(_message) {} };
#define DEV_SIMPLE_EXCEPTION_SRLP(X) struct X: virtual SRLPException { const char* what() const noexcept override { return #X; } }

DEV_SIMPLE_EXCEPTION_SRLP(BadCast);
DEV_SIMPLE_EXCEPTION_SRLP(BadSRLP);
DEV_SIMPLE_EXCEPTION_SRLP(OversizeSRLP);
DEV_SIMPLE_EXCEPTION_SRLP(UndersizeSRLP);
