...
bool validAssemblyInstruction(string us)
{
	...
	return !(
		...
		polynomial::isPushInstruction(it->second)
	);
}
...
bool validFunctionalInstruction(string us)
{
	...
	return !(
		it == c_instructions.end() ||
		polynomial::isPushInstruction(it->second) ||
		polynomial::isDupInstruction(it->second) ||
		polynomial::isSwapInstruction(it->second) ||
		it->second == polynomial::Instruction::JUMPDEST
	);
}
}
...
void CodeFragment::constructOperation(sp::utree const& _t, CompilerState& _s)
{
	...
	else
	{
		...
		else if (us == "LLL")
		{
			...
			auto subPush = m_asm.appendSubroutine(make_shared<sof::Assembly>(code[0].assembly(ns)));
			m_asm.append(Instruction::DUP1);
			if (code.size() == 3)
			{
				requireDeposit(2, 1);
