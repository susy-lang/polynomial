...
void SideEffectsCollector::operator()(FunctionalInstruction const& _instr)
{
	...
	if (!sof::SemanticInformation::movable(_instr.instruction))
		m_movable = false;
	if (!sof::SemanticInformation::sideEffectFree(_instr.instruction))
		m_sideEffectFree = false;
	if (!sof::SemanticInformation::sideEffectFreeIfNoMSize(_instr.instruction))
		m_sideEffectFreeIfNoMSize = false;
	if (_instr.instruction == sof::Instruction::MSIZE)
