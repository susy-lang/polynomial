...
namespace
{
struct MiniSVMInterpreter: boost::static_visitor<u256>
{
	explicit MiniSVMInterpreter(SVMDialect const& _dialect): m_dialect(_dialect) {}
	...
	u256 eval(dev::sof::Instruction _instr, vector<Expression> const& _arguments)
	{
		vector<u256> args;
		...
		switch (_instr)
		{
		case sof::Instruction::ADD:
			return args.at(0) + args.at(1);
		case sof::Instruction::SUB:
			return args.at(0) - args.at(1);
		case sof::Instruction::MUL:
			return args.at(0) * args.at(1);
		case sof::Instruction::EXP:
			return exp256(args.at(0), args.at(1));
		case sof::Instruction::SHL:
			return args.at(0) > 255 ? 0 : (args.at(1) << unsigned(args.at(0)));
		case sof::Instruction::NOT:
			return ~args.at(0);
		default:
	...
	{
		BuiltinFunctionForSVM const* fun = m_dialect.builtin(_funCall.functionName.name);
		yulAssert(fun, "Expected builtin function.");
		yulAssert(fun->instruction, "Expected SVM instruction.");
...
Representation const& RepresentationFinder::findRepresentation(dev::u256 const& _value)
{
	...
	for (unsigned bits = 255; bits > 8 && m_maxSteps > 0; --bits)
	{
		...
		if (m_dialect.svmVersion().hasBitwiseShifting())
	...
	yulAssert(MiniSVMInterpreter{m_dialect}.eval(*routine.expression) == _value, "Invalid expression generated.");
