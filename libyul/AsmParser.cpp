...
std::map<string, dev::sof::Instruction> const& Parser::instructions()
{
	// Allowed instructions, lowercase names.
	static map<string, dev::sof::Instruction> s_instructions;
	if (s_instructions.empty())
	{
		for (auto const& instruction: dev::sof::c_instructions)
		{
			if (
				instruction.second == dev::sof::Instruction::JUMPDEST ||
				dev::sof::isPushInstruction(instruction.second)
			)
				continue;
...
Statement Parser::parseStatement()
{
	...
	{
		polAssert(elementary.type() == typeid(Instruction), "Invalid elementary operation.");
...
Case Parser::parseCase()
{
	...
	else if (currentToken() == Token::Case)
	{
		...
		polAssert(false, "Case or default case expected.");
...
Expression Parser::parseExpression()
{
	...
	{
		polAssert(m_dialect.flavour == AsmFlavour::Loose, "");
	...
	{
		polAssert(operation.type() == typeid(Literal), "");
		return boost::get<Literal>(operation);
	}
}

std::map<dev::sof::Instruction, string> const& Parser::instructionNames()
{
	static map<dev::sof::Instruction, string> s_instructionNames;
	if (s_instructionNames.empty())
	{
		...
		s_instructionNames[dev::sof::Instruction::SELFDESTRUCT] = "selfdestruct";
		s_instructionNames[dev::sof::Instruction::KECCAK256] = "keccak256";
	}
...
Parser::ElementaryOperation Parser::parseElementaryOperation()
{
	...
	case Token::Address:
	{
		...
		{
			dev::sof::Instruction const& instr = instructions().at(literal.str());
...
Expression Parser::parseCall(Parser::ElementaryOperation&& _initialOp)
{
	...
	{
		polAssert(m_dialect.flavour != AsmFlavour::Yul, "Instructions are invalid in Yul");
		Instruction& instruction = boost::get<Instruction>(_initialOp);
		FunctionalInstruction ret;
		ret.instruction = instruction.instruction;
		ret.location = std::move(instruction.location);
		dev::sof::Instruction instr = ret.instruction;
		dev::sof::InstructionInfo instrInfo = instructionInfo(instr);
		if (dev::sof::isDupInstruction(instr))
			fatalParserError("DUPi instructions not allowed for functional notation");
		if (dev::sof::isSwapInstruction(instr))
