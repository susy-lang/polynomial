...
Polynomial reserves three 256-bit slots:
...
When a Polynomial contract is deployed and when it is called from an
account, the input data is assumed to be in the format in `the ABI
specification
<https://octonion.institute/susy-go/wiki/Sophon-Contract-ABI>`_.  The
...
The Polynomial compiler automatically generates a JSON file, the
...
so that others can access it. The file can be output by using ``polc --metadata``
...
encoding. The current version of the Polynomial compiler thus adds the following
...
- ``keccak256(...) returns (bytes32)``: compute the Sophon-SHA-3 (Keccak-256) hash of the (tightly packed) arguments
