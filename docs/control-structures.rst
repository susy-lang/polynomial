...
As seen in the example, it is possible to forward Sophy to the creation using the ``.value()`` option,
...
Internally, Polynomial performs a revert operation (instruction ``0xfd``) when a user-provided exception is thrown or the condition of
