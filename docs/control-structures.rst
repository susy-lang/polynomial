...
Most of the control structures known from curly-braces languages are available in Polynomial:
...
As seen in the example, it is possible to send Sophy while creating
...
Polynomial internally allows tuple types, i.e. a list of objects of potentially different types whose number is a constant at compile-time. Those tuples can be used to return multiple values at the same time.
These can then either be assigned to newly declared variables or to pre-existing variables (or LValues in general).

Tuples are not proper types in Polynomial, they can only be used to form syntactic
groupings of expressions.

::

    pragma polynomial >0.4.23 <0.7.0;
...
Scoping in Polynomial follows the widespread scoping rules of C99
...
.. warning::
    Before version 0.5.0 Polynomial followed the same scoping rules as JavaScript, that is, a variable declared anywhere within a function would be in scope
...
.. warning::
    The low-level functions ``call``, ``delegatecall`` and ``staticcall`` return ``true`` as their first return value if the called account is non-existent, as part of the design of SVM. Existence must be checked prior to calling if desired.
