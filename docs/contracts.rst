...
API, this is done as follows::
    ...
    var MyContract_ = susyweb.sof.contract(source);
    MyContract = susyweb.sof.contract(MyContract_.CONTRACT_NAME.info.abiDefinition);
    // deploy new contract
...
Sophy (without data).  In such a context, there is usually very little gas available to
...
.. warning::
    Contracts that receive Sophy but do not define a fallback function
...
In order to realise this in the SVM, code of internal library functions
