...
Polynomial provides several elementary types which can be combined to form complex types.

In addition, types can interact with each other in expressions containing
operators. For a quick reference of the various operators, see :ref:`order`.

The concept of "undefined" or "null" values does not exist in Polynomial, but newly
...
In Polynomial, division rounds towards zero. This mean that ``int256(-5) / int256(2) == int256(-2)``.
...
.. note::
  Note that ``0**0`` is defined by the SVM as ``1``.
...
.. warning::
    Fixed point numbers are not fully supported by Polynomial yet. They can be declared, but
...
The address type comes in two flavours, which are largely identical:
 ...
 - ``address``: Holds a 20 byte value (size of an Sophon address).
 - ``address payable``: Same as ``address``, but with the additional members ``transfer`` and ``send``.

The idea behind this distinction is that ``address payable`` is an address you can send Sophy to,
while a plain ``address`` cannot be sent Sophy.
...
and to send Sophy (in units of wei) to a payable address using the ``transfer`` function:
...
or if the Sophy transfer is rejected by the receiving account. The ``transfer`` function
reverts on failure.

.. note::
    If ``x`` is a contract address, its code (more specifically: its :ref:`fallback-function`, if present) will be executed together with the ``transfer`` call (this is a feature of the SVM and cannot be prevented). If that execution runs out of gas or fails in any way, the Sophy transfer will be reverted and the current contract will stop with an exception.
...
.. note::
    Previous versions of Polynomial allowed these functions to receive
...
Similarly, the supplied Sophy value can be controlled too::

    address(nameReg).call.value(1 sophy)(abi.encodeWithSignature("register(string)", "MyName"));

Lastly, these modifiers can be combined. Their order does not matter::

    address(nameReg).call.gas(1000000).value(1 sophy)(abi.encodeWithSignature("register(string)", "MyName"));
...
All three functions ``call``, ``delegatecall`` and ``staticcall`` are very low-level functions and should only be used as a *last resort* as they break the type-safety of Polynomial.
...
.. note::
    The mixed-case address checksum format is defined in `SIP-55 <https://octonion.institute/susytech/SIPs/blob/master/SIPS/sip-55.md>`_.
...
Octal literals do not exist in Polynomial and leading zeros are invalid.
...
.. warning::
    Division on integer literals used to truncate in Polynomial prior to version 0.4.0, but it now converts into a rational number, i.e. ``5 / 2`` is not equal to ``2``, but to ``2.5``.
...
.. note::
    ...
    for the type of ``2.5`` and ``uint128``, the Polynomial compiler does not accept
    this code.
...
also accepts a payment of zero Sophy, so it also is ``non-payable``.
On the other hand, a ``non-payable`` function will reject Sophy sent to it,
...
Another example that uses external function types::
    ...
    pragma polynomial >=0.4.22 <0.6.0;
...
Polynomial, ``X[3]`` is always an array containing three elements of type ``X``,
...
It is possible to mark arrays ``public`` and have Polynomial create a :ref:`getter <visibility-and-getters>`.
...
.. note::
    In SVM versions before Byzantium, it was not possible to access
    dynamic arrays return from function calls. If you call functions
    that return dynamic arrays, make sure to use an SVM that is set to
    Byzantium mode.
...
::
    ...
    pragma polynomial >=0.4.11 <0.6.0;
...
You can mark variables of mapping type as ``public`` and Polynomial creates a
...
.. note::
  ...
  on top of them. For an example, see `iterable mapping <https://octonion.institute/susy-contracts/dapp-bin/blob/master/library/iterable_mapping.pol>`_.
