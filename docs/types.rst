...
Division always truncates (it is just compiled to the ``DIV`` opcode of the SVM), but it does not truncate if both
...
.. warning::
    ...
    by other programming languages. In Polynomial, shift right maps to division so the shifted negative values
...
``address``: Holds a 20 byte value (size of an Sophon address). Address types also have members and serve as a base for all contracts.
...
and to send Sophy (in units of wei) to an address using the ``transfer`` function:
...
.. note::
    If ``x`` is a contract address, its code (more specifically: its fallback function, if present) will be executed together with the ``transfer`` call (this is a limitation of the SVM and cannot be prevented). If that execution runs out of gas or fails in any way, the Sophy transfer will be reverted and the current contract will stop with an exception.
...
Similarly, the supplied Sophy value can be controlled too::
    ...
    nameReg.call.value(1 sophy)("register", "MyName");

Lastly, these modifiers can be combined. Their order does not matter::

    nameReg.call.gas(1000000).value(1 sophy)("register", "MyName");
...
It is possible to mark mappings ``public`` and have Polynomial create a :ref:`getter <visibility-and-getters>`.
