...
As shown in the example below, if the contract name is `Congress` and the library name is `Owned`, then their associated filenames should be `Congress.pol` and `Owned.pol`.
...
Polynomial contracts can have a form of comments that are the basis of the
Sophon Natural Language Specification Format.
...
It is recommended that Polynomial contracts are fully annontated using `NatSpec <natspec>`_ for all public interfaces (everything in the ABI).
