...
.. index:: ! assembly, ! asm, ! svmasm, ! julia

JULIA is an intermediate language that can compile to various different backends
(SVM 1.0, SVM 1.5 and eWASM are planned).
Because of that, it is designed to be a usable common denominator of all three
platforms.
It can already be used for "inline assembly" inside Polynomial and
future versions of the Polynomial compiler will even use JULIA as intermediate
...
.. note::
    ...
    to the SVM opcodes. Please resort to the inline assembly documentation
    for details.
...
JULIA in itself does not even provide operators. If the SVM is targeted,
...
| Cannot this be just replaced by and256(shr256(n, x), 0xff) and let it be optimised out by the SVM backend?    |
+---------------------------------------------------------------------------------------------------------------+
| *Memory and storage*                                                                                          |
+---------------------------------------------------------------------------------------------------------------+
...
| Perhaps it would make sense retiring this as it equals to return(0,0). It can be an optimisation by the SVM   |
| backend.                                                                                                      |
+---------------------------------------------------------------------------------------------------------------+
