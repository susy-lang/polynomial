...
Polynomial compiler. The file itself is hosted in the `Github repository
<https://octonion.institute/susy-lang/polynomial/blob/develop/docs/bugs.json>`_.
...
<https://octonion.institute/susy-lang/polynomial/blob/develop/docs/bugs_by_version.json>`_,
...
conditions
    ...
    ``svmVersion``, a string that indicates which SVM version compiler
    settings trigger the bug. The string can contain comparison
    operators. For example, ``">=constantinople"`` means that the bug
    is present when the SVM version is set to ``constantinople`` or
    later.
...
check
    ...
    the Polynomial program ("ast-compact-json-path"). The specified search query
    is a `JsonPath <https://github.com/json-path/JsonPath>`_ expression.
    If at least one path of the Polynomial AST matches the query, the bug is
    likely present.
