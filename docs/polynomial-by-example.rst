...
::
    ...
    contract Ballot {
        ...
        function giveRightToVote(address voter) public {
            ...
            // changes to the state and to Sophy balances
            // are reverted.
            // This used to consume all gas in old SVM versions, but
            // not anymore.
...
::
    ...
    pragma polynomial >0.4.23 <0.6.0;
...
smart contract that lets her transmit Sophy, but
...
Alice does not need to interact with Sophon network to
sign the transaction, the process is completely offline.
In this tutorial, we will sign messages in the browser
using ``susyweb.js`` and ``MetaMask``.
In particular, we will use the standard way described in `SIP-762 <https://octonion.institute/susytech/SIPs/pull/712>`_,
as it provides a number of other security benefits.

::

    /// Hashing first makes a few things easier
    var hash = susyweb.sha3("message to sign");
    susyweb.personal.sign(hash, susyweb.sof.defaultAccount, function () {...});


Note that the ``susyweb.personal.sign`` prepends the length of the message to the signed data.
...
a second action.
To avoid replay attacks we will use the same as in Sophon transactions
...
The
`sophonjs-abi <https://octonion.institute/susy-js/sophonjs-abi>`_ library provides
a function called ``polynomialSHA3`` that mimics the behavior
of Polynomial's ``keccak256`` function applied to arguments encoded
...
::
    ...
    function signPayment(recipient, amount, nonce, contractAddress, callback) {
        var hash = "0x" + sophonjs.ABI.polynomialSHA3(
            ["address", "uint256", "uint256", "address"],
            [recipient, amount, nonce, contractAddress]
        ).toString("hex");

        susyweb.personal.sign(hash, susyweb.sof.defaultAccount, callback);
    }
...
Signatures in Sophon include a third parameter called ``v``, that can be used
to recover which account's private key was used to sign in the message,
the transaction's sender. Polynomial provides a built-in function
...
Signatures produced by susyweb.js are the concatenation of ``r``, ``s`` and ``v``,
...
of Sophy securely, instantaneously, and without transaction fees.

What is a Payment Channel?
--------------------------

Payment channels allow participants to make repeated transfers of Sophy without
...
Note that only steps 1 and 3 require Sophon transactions, step 2 means that
...
the Sophy and honors a valid signed message. The smart contract also enforces a timeout,
...
attaching the Sophy to be escrowed and specifying the intendend recipient
...
Closing the channel pays the recipient the Sophy they are owed and destroys the contract,
sending any remaining Sophy back to Alice.
...
openzepplin's `version <https://github.com/OpenZeppelin/openzeppelin-polynomial/blob/master/contracts/ECRecovery.pol>`_ of this code.
