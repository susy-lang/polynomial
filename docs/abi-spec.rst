...
The Application Binary Interface is the standard way to interact with contracts in the Sophon ecosystem, both
...
This specification does not address contracts whose interface is dynamic or otherwise known only at run-time. Should these cases become important they can be adequately handled as facilities built within the Sophon ecosystem.
...
.. note::
    Polynomial supports all the types presented above with the same names with the exception of tuples. The ABI tuple type is utilised for encoding Polynomial ``structs``.
...
- ``payable``: ``true`` if function accepts sophy, defaults to ``false``;
...
Sending non-zero sophy to non-payable function will throw. Don't do it.
...
Polynomial supports a non-standard packed mode where:
