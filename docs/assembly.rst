...
Polynomial defines an assembly language that you can use without Polynomial and also
as "inline assembly" inside Polynomial source code. This guide starts with describing
...
You can interleave Polynomial statements with inline assembly in a language close
...
As the SVM is a stack machine, it is often hard to address the correct stack slot
and provide arguments to opcodes at the correct point on the stack. Polynomial's inline
...
.. warning::
    ...
    features and checks of Polynomial. You should only use it for
    tasks that need it, and only if you are confident with using it.

Syntax
------

Assembly parses comments, literals and identifiers in the same way as Polynomial, so you can use the
...
load it into a ``bytes`` variable. This is not possible with "plain Polynomial" and the
idea is that assembly libraries will be used to enhance the Polynomial language.
...
.. code::
    ...
    library VectorSum {
        ...
        function sumPolynomial(uint[] memory _data) public pure returns (uint o_sum) {
...
Polynomial inline assembly has a "functional style" notation where the same code
...
You can access Polynomial variables and other identifiers by using their name.
...
Local Polynomial variables are available for assignments, for example:
...
Support for labels has been removed in version 0.5.0 of Polynomial.
...
for the Polynomial compiler. In this form, it tries to achieve several goals:

1. Programs written in it should be readable, even if the code is generated by a compiler from Polynomial.
...
We will follow an example compilation from Polynomial to assembly.
We consider the runtime bytecode of the following Polynomial program::
...
The assembly lexer follows the one defined by Polynomial itself.
