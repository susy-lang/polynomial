...
.. index:: ! assembly, ! asm, ! svmasm, ! yul, julia, iulia
...
Support for SVM 1.0, SVM 1.5 and eWASM is planned, and it is designed to be a usable common denominator of all three
platforms. It can already be used for "inline assembly" inside Polynomial and future versions of the Polynomial compiler
...
.. warning::
    ...
    and with SVM 1.0 as target, :ref:`SVM opcodes <opcodes>` are used as built-in functions.
...
Yul in itself does not even provide operators. If the SVM is targeted,
...
The following example program assumes that the SVM opcodes ``mul``, ``div``
and ``mod`` are available either natively or as functions and computes exponentiation.
As per the warning above, the following code is untyped and can be compiled using ``polc --strict-assembly``.
...
instead of with recursion. Here, we need the SVM opcodes ``lt`` (less-than)
...
(which in the context of the SVM is the memory, storage and state of the
blockchain) and the local state object (the state of local variables, i.e. a
segment of the stack in the SVM).
...
|                                             | let it be optimised out by the SVM backend?                     |
+---------------------------------------------+-----------------------------------------------------------------+
| *Memory and storage*                                                                                          |
+---------------------------------------------+-----------------------------------------------------------------+
...
+---------------------------------------------+-----------------------------------------------------------------+
| abort()                                     | abort (equals to invalid instruction on SVM)                    |
+---------------------------------------------+-----------------------------------------------------------------+
...
prefixed with the name of the backend. We reserve ``svm_`` and ``ewasm_`` prefixes for the two proposed backends.

Backend: SVM
------------

The SVM target will have all the underlying SVM opcodes exposed with the `svm_` prefix.
