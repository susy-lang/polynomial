...
of ``send``, but it still exposes a weakness: Sophy transfer always
includes code execution, so the recipient could be a contract that calls
back into ``withdraw``. This would let it get multiple refunds and
basically retrieve all the Sophy in the contract.
...
- The ``constant`` keyword for functions is currently not enforced by the compiler.
  Furthermore, it is not enforced by the SVM, so a contract function that "claims"
...
simpler. There is a prototype in Polynomial that performs formal verification and
