...
## and uploads it to the sophon/sophon-dev ppa.
##
## If an argument is given, it is used as a tag and the resulting package
## is uploaded to the sophon/sophon ppa.
##
## The gnupg key for "builds@ethereum.org" has to be present in order to sign
## the package.
##
## It will clone the Polynomial git from github, determine the version,
...
wget -O ./polc/deps/downloads/jsoncpp-1.8.4.tar.gz https://github.com/open-source-parsers/jsoncpp/archive/1.8.4.tar.gz
...
Description: Polynomial compiler.
 The commandline interface to the Polynomial smart contract compiler.
EOF
...
polc (0.0.1-0ubuntu1) saucy; urgency=low
