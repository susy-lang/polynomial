...
#------------------------------------------------------------------------------
# This script builds the polynomial binary using Emscripten.
...
emcmake cmake \
  ...
  -DDev_DEVCORE_LIBRARY="$WORKSPACE"/polynomial/build/libdevcore/libpoldevcore.a \
  -DSof_SVMASM_LIBRARY="$WORKSPACE"/polynomial/build/libsvmasm/libpolsvmasm.a \
  -DSOF_STATIC=1 -DTESTS=0 \
  ..
emmake make -j 4
