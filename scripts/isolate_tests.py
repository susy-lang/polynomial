...
# Look for `pragma polynomial` and abort a line not indented properly.
    ...
    for l in lines:
      if inside:
        ...
        if ignore:
          ...
          m = re.search(r'^    pragma polynomial .*[0-9]+\.[0-9]+\.[0-9]+;$', l)
          if m:
            inside = True
            tests += [l]
...
def write_cases(tests):
    for test in tests:
        open('test_%s.pol' % hashlib.sha256(test).hexdigest(), 'wb').write(test)
