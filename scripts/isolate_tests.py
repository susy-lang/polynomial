...
# scripts/isolate_tests.py test/libpolynomial/*
...
# Look for `pragma polynomial`, `contract`, `library` or `interface`
...
def extract_docs_cases(path):
    ...
    for l in open(path, 'rb').read().splitlines():
        ...
        if inside:
            tests[-1] += l + '\n'
    # Filter all tests that do not contain Polynomial
    return [
        test for test in tests
        if re.search(r'^    [ ]*(pragma polynomial|contract |library |interface )', test, re.MULTILINE)
    ]

def write_cases(f, tests):
    cleaned_filename = f.replace(".","_").replace("-","_").replace(" ","_").lower()
    for test in tests:
        open('test_%s_%s.pol' % (hashlib.sha256(test).hexdigest(), cleaned_filename), 'wb').write(test)
