...
#------------------------------------------------------------------------------
# Bash script implementing release flow for polynomial for Linux and macOS.
#
...
# Copy all the polynomial executables into a temporary directory prior to ZIP creation

cp $REPO_ROOT/build/lllc/lllc        $ZIP_TEMP_DIR
cp $REPO_ROOT/build/polc/polc        $ZIP_TEMP_DIR
cp $REPO_ROOT/build/poltest/poltest  $ZIP_TEMP_DIR
...
# See https://octonion.institute/susy-cpp/webthree-umbrella/issues/495.

cp $REPO_ROOT/build/libdevcore/*.$DLL_EXT   $ZIP_TEMP_DIR
cp $REPO_ROOT/build/libsvmasm/*.$DLL_EXT    $ZIP_TEMP_DIR
cp $REPO_ROOT/build/libpolynomial/*.$DLL_EXT  $ZIP_TEMP_DIR
...
zip -j $REPO_ROOT/polynomial-develop-$ZIP_SUFFIX.zip $ZIP_TEMP_DIR/*
