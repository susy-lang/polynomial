...
public:
	...
	): m_testCaseCreator(_testCaseCreator), m_name(_name), m_path(_path), m_ipcPath(_ipcPath), m_formatted(_formatted), m_svmVersion(_svmVersion)
	{}
...
TestTool::Result TestTool::process()
{
	...
	try
	{
		m_test = m_testCaseCreator(TestCase::Config{m_path.string(), m_ipcPath, m_svmVersion});
		if (m_test->supportedForSVMVersion(m_svmVersion))
...
)
{
	...
	while (!paths.empty())
	{
		...
		{
			++testCount;
			TestTool testTool(_testCaseCreator, currentPath.string(), fullpath, _ipcPath, _formatted, _svmVersion);
...
)
{
	...
	TestStats stats = TestTool::processPath(_testCaseCreator, _basePath, _subdirectory, _ipcPath, _formatted, _svmVersion);
...
int main(int argc, char const *argv[])
{
	...
	dev::test::IpolTestOptions options(&TestTool::editor);
	...
	for (auto const& ts: g_interactiveTestsuites)
	{
		...
		if (auto stats = runTestSuite(ts.title, options.testPath / ts.path, ts.subpath, options.ipcPath.string(), ts.testCaseCreator, !options.noColor, options.svmVersion()))
			global_stats += *stats;
		else
			return 1;
	}
