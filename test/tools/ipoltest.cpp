...
using TestOptions = dev::test::IpolTestOptions;
...
public:
	explicit TestFilter(string const& _filter): m_filter(_filter)
	{
		...
		m_filterExpression = regex{"(" + filter + "(\\.pol|\\.yul))"};
	}
...
TestTool::Result TestTool::process()
{
	...
	try
	{
		if (m_filter.matches(m_name))
		{
			...
			m_test = m_testCaseCreator(TestCase::Config{m_path.string(), m_options.ipcPath.string(), m_options.svmVersion()});
			if (m_test->validateSettings(m_options.svmVersion()))
