...
Allowed options)",
	...
	if (auto stats = runTestSuite("Syntax", testPath / "libpolynomial", "syntaxTests", SyntaxTest::create, formatted))
		global_stats += *stats;
	else
		return 1;

	if (auto stats = runTestSuite("JSON AST", testPath / "libpolynomial", "ASTJSON", ASTJSONTest::create, formatted))
		global_stats += *stats;
	else
		return 1;
