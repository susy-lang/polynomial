...
int main(int argc, char *argv[])
{
	...
	po::options_description options(
		R"(ipoltest, tool for interactively managing test contracts.
Usage: ipoltest [Options] --testpath path
...
Allowed options)",
	...
	if (testPath.empty())
	{
		...
		{
			fs::path syntaxTestPath = basePath / "libpolynomial" / "syntaxTests";
	...
	fs::path syntaxTestPath = testPath / "libpolynomial" / "syntaxTests";

	if (fs::exists(syntaxTestPath) && fs::is_directory(syntaxTestPath))
	{
		auto stats = SyntaxTestTool::processPath(testPath / "libpolynomial", "syntaxTests", formatted);
