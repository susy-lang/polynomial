...
#define SOF_BOOST_CHECK_IMPL(_message, _requireOrCheck) BOOST_TEST_TOOL_DIRECT_IMPL( \
		false,															\
		_requireOrCheck,												\
		_message														\
	)
#else
#define SOF_BOOST_CHECK_IMPL(_message, _requireOrCheck) BOOST_CHECK_IMPL( \
		false,															\
		_message,														\
		_requireOrCheck,												\
		CHECK_MSG														\
...
#define SOF_TEST_REQUIRE_NO_THROW(_statement, _message)				\
	...
	{																	\
		...
		{																\
			...
			SOF_BOOST_CHECK_IMPL(msg, REQUIRE);							\
		}																\
		catch (...)														\
		{																\
			SOF_BOOST_CHECK_IMPL(										\
...
#define SOF_TEST_CHECK_NO_THROW(_statement, _message)					\
	...
	{																	\
		...
		{																\
			auto msg = std::string(_message " due to an exception thrown by " \
				BOOST_STRINGIZE(_statement) "\n") + boost::diagnostic_information(_e); \
			SOF_BOOST_CHECK_IMPL(msg, CHECK);							\
		}																\
		catch (...)														\
		{																\
			SOF_BOOST_CHECK_IMPL(										\
