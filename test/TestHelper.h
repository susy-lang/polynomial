...
#define SOF_TEST_REQUIRE_NO_THROW(_statement, _message)				\
	do																	\
	{																	\
		try															\
		{																\
...
/// @param _message       A message to act as a prefix to the expression's error information
#define SOF_TEST_CHECK_NO_THROW(_statement, _message)					\
	do																	\
	{																	\
		try															\
		{																\
...
public:
	...
	static void importState(json_spirit::mObject& _o, sof::State& _state);
	static void importState(json_spirit::mObject& _o, sof::State& _state, sof::AccountMaskMap& o_mask);
	static void importTransaction (json_spirit::mObject const& _o, sof::Transaction& o_tr);
	void importTransaction(json_spirit::mObject const& _o);
	static json_spirit::mObject& makeAllFieldsHex(json_spirit::mObject& _o);

	bytes executeTest();
	int exportTest(bytes const& _output);
	static int compareStates(sof::State const& _stateExpect, sof::State const& _statePost, sof::AccountMaskMap const _expectedStateOptions = sof::AccountMaskMap(), WhenError _throw = WhenError::Throw);

	sof::State m_statePre;
	sof::State m_statePost;
	sof::EnvInfo m_envInfo;
	sof::Transaction m_transaction;	
	sof::LogEntries m_logs;
	sof::LogEntries m_logsExpected;

private:
	json_spirit::mObject& m_testObject;
};

class ZeroGasPricer: public sof::GasPricer
{
protected:
	u256 ask(sof::Block const&) const override { return 0; }
	u256 bid(sof::TransactionPriority = sof::TransactionPriority::Medium) const override { return 0; }
};
...
sof::LogEntries importLog(json_spirit::mArray& _o);
json_spirit::mArray exportLog(sof::LogEntries _logs);
void checkOutput(bytes const& _output, json_spirit::mObject& _o);
void checkStorage(std::map<u256, u256> _expectedStore, std::map<u256, u256> _resultStore, Address _expectedAddr);
void checkLog(sof::LogEntries _resultLogs, sof::LogEntries _expectedLogs);
...
sof::LastHashes lastHashes(u256 _currentBlockNumber);
json_spirit::mObject fillJsonWithState(sof::State _state);
json_spirit::mObject fillJsonWithTransaction(sof::Transaction _txn);
...
{
public:
	bool vmtrace = false;	///< Create SVM execution tracer // TODO: Link with log verbosity?
