...
#include <libsophon/BlockChain.h>
#include <libsophon/ClientBase.h>
...
#define SOF_CHECK_EQUAL(x, y) { dev::WriteGuard(x_boostTest); BOOST_CHECK_EQUAL(x, y); }
#define SOF_CHECK_EQUAL_COLLECTIONS(xb, xe, yb, ye) { dev::WriteGuard(x_boostTest); BOOST_CHECK_EQUAL_COLLECTIONS(xb, xe, yb, ye); }
#define SOF_REQUIRE(x) { dev::WriteGuard(x_boostTest); BOOST_REQUIRE(x); }
...
{
	void enumerateBlockchains(std::function<void(Json::Value const&, dev::sof::BlockChain const&, dev::sof::State state)> callback) const;
};

struct ClientBaseFixture: public BlockChainFixture
{
	void enumerateClients(std::function<void(Json::Value const&, dev::sof::ClientBase&)> callback) const;
};
