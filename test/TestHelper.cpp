...
#include <thread>
#include <chrono>
#include <libsofcore/SofashAux.h>
#include <libsophon/Client.h>
#include <libsvm/ExtVMFace.h>
#include <liblll/Compiler.h>
#include <libsvm/VMFactory.h>
#include "Stats.h"
...
void connectClients(Client& c1, Client& c2)
{
	(void)c1;
	(void)c2;
	// TODO: Move to WebThree. sof::Client no longer handles networking.
#if 0
	short c1Port = 20000;
	short c2Port = 21000;
...
{
	std::unique_ptr<SealEngineFace> sealer(Sofash::createSealEngine());
	s.commitToSeal(_bc);
...
void mine(Sofash::BlockHeader& _bi)
{
	std::unique_ptr<SealEngineFace> sealer(Sofash::createSealEngine());
	Notified<bytes> sealed;
	sealer->onSealGenerated([&](bytes const& sealedHeader){ sealed = sealedHeader; });
	...
	sealed.waitNot({});
	sealer.reset();
	_bi = Sofash::BlockHeader(sealed, CheckNothing, h256{}, HeaderData);
}
...
ImportTest::ImportTest(json_spirit::mObject& _o, bool isFiller, testType testTemplate):
	m_statePre(OverlayDB(), sof::BaseState::Empty),
	m_statePost(OverlayDB(), sof::BaseState::Empty),
	m_testObject(_o)
{
...
void ImportTest::importTransaction (json_spirit::mObject const& _o, sof::Transaction& o_tr)
{
	...
	{
		SRLPStream transactionSRLPStream = createSRLPStreamFromTransactionFields(_o);
		SRLP transactionSRLP(transactionSRLPStream.out());
		try
		{
			o_tr = Transaction(transactionSRLP.data(), CheckTransaction::Everything);
		}
...
int ImportTest::exportTest(bytes const& _output)
{
	...
	{
		sof::AccountMaskMap stateMap;
		State expectState(OverlayDB(), sof::BaseState::Empty);
...
json_spirit::mArray exportLog(sof::LogEntries _logs)
{
...
void checkCallCreates(sof::Transactions _resultCallCreates, sof::Transactions _expectedCallCreates)
{
...
void userDefinedTest(std::function<void(json_spirit::mValue&, bool)> doTests)
{
	...
	{
		cnote << "Missing user test specification\nUsage: testsof --singletest <filename> <testname>\n";
		return;
	}
...
void executeTests(const string& _name, const string& _testPathAppendix, const boost::filesystem::path _pathToFiller, std::function<void(json_spirit::mValue&, bool)> doTests)
{
	...
	try
	{
		...
		TBOOST_REQUIRE_MESSAGE((s.length() > 0), "Contents of " + testPath + "/" + _name + ".json is empty. Have you cloned the 'tests' repo branch develop and set SOPHON_TEST_PATH to its path?");
		json_spirit::read_string(s, v);
...
SRLPStream createSRLPStreamFromTransactionFields(json_spirit::mObject const& _tObj)
{
	//Construct Rlp of the given transaction
	SRLPStream srlpStream;
	srlpStream.appendList(_tObj.size());

	if (_tObj.count("nonce"))
		srlpStream << bigint(_tObj.at("nonce").get_str());

	if (_tObj.count("gasPrice"))
		srlpStream << bigint(_tObj.at("gasPrice").get_str());

	if (_tObj.count("gasLimit"))
		srlpStream << bigint(_tObj.at("gasLimit").get_str());
	...
	if (_tObj.count("to"))
	{
		if (_tObj.at("to").get_str().empty())
			...
			srlpStream << importByteArray(_tObj.at("to").get_str());
	}

	if (_tObj.count("value"))
		srlpStream << bigint(_tObj.at("value").get_str());

	if (_tObj.count("data"))
		srlpStream << importData(_tObj);

	if (_tObj.count("v"))
		srlpStream << bigint(_tObj.at("v").get_str());

	if (_tObj.count("r"))
		srlpStream << bigint(_tObj.at("r").get_str());

	if (_tObj.count("s"))
		srlpStream <<  bigint(_tObj.at("s").get_str());

	if (_tObj.count("extrafield"))
		srlpStream << bigint(_tObj.at("extrafield").get_str());
...
dev::sof::Sofash::BlockHeader constructHeader(
	...
	dev::sof::LogBloom const& _logBloom,
...
{
	SRLPStream srlpStream;
	srlpStream.appendList(Sofash::BlockHeader::Fields);

	srlpStream << _parentHash << _sha3Uncles << _coinbaseAddress << _stateRoot << _transactionsRoot << _receiptsRoot << _logBloom
		<< _difficulty << _number << _gasLimit << _gasUsed << _timestamp << _extraData << h256{} << Nonce{};

	return Sofash::BlockHeader(srlpStream.out(), CheckNothing, h256{}, HeaderData);
}

void updateSofashSeal(dev::sof::Sofash::BlockHeader& _header, h256 const& _mixHash, dev::sof::Nonce const& _nonce)
{
	SRLPStream source;
	_header.streamSRLP(source);
	SRLP sourceRlp(source.out());
	SRLPStream header;
	header.appendList(Sofash::BlockHeader::Fields);
	for (size_t i = 0; i < BlockInfo::BasicFields; i++)
		header << sourceRlp[i];

	header << _mixHash << _nonce;
	_header = Sofash::BlockHeader(header.out(), CheckNothing, h256{}, HeaderData);
}
