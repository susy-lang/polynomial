...
ExecutionFramework::ExecutionFramework():
	ExecutionFramework(getIPCSocketPath(), dev::test::Options::get().svmVersion())
{
}
...
{
	return u256(m_rpc.sof_gasPrice());
}

u256 ExecutionFramework::blockHash(u256 const& _blockNumber) const
{
	return u256(m_rpc.sof_getBlockByNumber(toHex(_blockNumber, HexPrefix::Add), false)["hash"].asString());
}
...
void ExecutionFramework::sendMessage(bytes const& _data, bool _isCreation, u256 const& _value)
{
	...
	if (!_isCreation)
	{
		...
		BOOST_REQUIRE(m_rpc.sof_getCode(d.to, "pending").size() > 2);
		// Use sof_call to get the output
		m_output = fromHex(m_rpc.sof_call(d, "pending"), WhenError::Throw);
	}
