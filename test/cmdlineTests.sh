...
function test_polc_behaviour()
{
    ...
    local polc_command="$POLC ${filename} ${polc_args}"
    if [[ -n "$polc_stdin"  ]]; then polc_command+=" <$polc_stdin"  ; fi
    if [[ -n "$stdout_path" ]]; then polc_command+=" 1>$stdout_path"; fi
    if [[ -n "$stderr_path" ]]; then polc_command+=" 2>$stderr_path"; fi
