...
class AuctionRegistrarTestFramework: public PolynomialExecutionFramework
{
protected:
	void deployRegistrar()
	{
		if (!s_compiledRegistrar)
		{
			...
			SOF_TEST_REQUIRE_NO_THROW(m_compiler.compile(), "Compiling contract failed");
	...
	using ContractInterface = PolynomialExecutionFramework::ContractInterface;
	class RegistrarInterface: public ContractInterface
	{
	public:
		RegistrarInterface(PolynomialExecutionFramework& _framework): ContractInterface(_framework) {}
...
BOOST_FIXTURE_TEST_SUITE(PolynomialAuctionRegistrar, AuctionRegistrarTestFramework)
...
BOOST_AUTO_TEST_CASE(double_reserve_long)
{
	...
	sendSophy(account(1), u256(10) * sophy);
	m_sender = account(1);
	registrar.reserve(name);
...
BOOST_AUTO_TEST_CASE(properties)
{
	...
	{
		m_sender = account(0);
		sendSophy(account(count), u256(20) * sophy);
