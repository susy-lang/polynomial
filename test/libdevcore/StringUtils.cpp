...
#include <libpolynomial/ast/Types.h>  // for IntegerType
...
BOOST_AUTO_TEST_CASE(test_format_number_readable)
{
	...
	BOOST_CHECK_EQUAL(
			formatNumberReadable(polynomial::IntegerType(256).minValue()), "0");
	BOOST_CHECK_EQUAL(
			formatNumberReadable(polynomial::IntegerType(256).maxValue()), "2**256 - 1");
}
