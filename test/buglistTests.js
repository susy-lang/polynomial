...
async function checkJSONPath(name, buggy, fine)
{
	...
	{
		var url = "http://octonion.institute/susy-lang/polynomial/releases/download/v" + bugsByName[name].introduced + "/polc-static-linux"
		try {
			var tmpdir = await mktemp.createDir('XXXXX')
			var binary = tmpdir + "/polc-static-linux"
...
function checkJsonPathTest(code, tmpdir, binary, query, idx) {
    return new Promise(function(resolve, reject) {
        var polFile = tmpdir + "/jsonPath" + idx + ".pol"
        var astFile = tmpdir + "/ast" + idx + ".json"
        writeFilePromise(polFile, code)
        .then(() => {
            return exec(binary + " --ast-compact-json " + polFile + " > " + astFile)
        })
        .then(() => {
            var jsonRE = /(\{[\s\S]*\})/
