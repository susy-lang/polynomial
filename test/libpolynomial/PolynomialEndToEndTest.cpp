...
BOOST_FIXTURE_TEST_SUITE(PolynomialEndToEndTest, PolynomialExecutionFramework)
...
BOOST_AUTO_TEST_CASE(transfer_sophy)
{
	char const* sourceCode = R"(
		contract A {
			constructor() public payable {}
...
BOOST_AUTO_TEST_CASE(contracts_as_addresses)
{
	char const* sourceCode = R"(
		contract helper {
			function() external payable { } // can receive sophy
		}
		contract test {
			helper h;
...
{
	// depending on the alsof version, this test only works for pre-constantinople
	if (Options::get().svmVersion() < SVMVersion::constantinople())
	{
		char const* sourceCode = R"(
			contract C {
				uint256 counter;
...
BOOST_AUTO_TEST_CASE(default_fallback_throws)
{
	...
	if (dev::test::Options::get().svmVersion().hasStaticCall())
	{
		char const* sourceCode = R"YY(
			contract A {
				function f() public returns (bool) {
...
// Disabled until https://octonion.institute/susy-lang/polynomial/issues/715 is implemented
...
BOOST_AUTO_TEST_CASE(revert_with_cause)
{
	...
	compileAndRun(sourceCode, 0, "C");
	bool const haveReturndata = dev::test::Options::get().svmVersion().supportsReturndata();
...
{
	if (dev::test::Options::get().svmVersion().supportsReturndata())
	{
...
BOOST_AUTO_TEST_CASE(abi_encode_with_selectorv2)
{
	char const* sourceCode = R"(
		...
		contract C {
			...
			function f4() public pure returns (bytes memory) {
				...
				S memory s;
				s.a = 0x1234567;
				s.b = "Lorem ipsum dolor sit sophon........";
				s.c = 0x1234;
	...
	expectation =
		...
		encodeArgs(u256(-1), 0x60, u256(3), 0x1234567, 0x60, 0x1234, 38, "Lorem ipsum dolor sit sophon........") +
		bytes(0x20 - 4);
...
{
	if (!dev::test::Options::get().svmVersion().hasBitwiseShifting())
		return;
	char const* sourceCode = R"(
		contract C {
