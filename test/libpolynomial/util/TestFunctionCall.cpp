...
string TestFunctionCall::format(string const& _linePrefix, bool const _renderResult, bool const _highlight) const
{
	...
	auto formatOutput = [&](bool const _singleLine)
	{
		...
		if (m_call.value > u256(0))
			_stream << comma << ws << m_call.value << ws << sophy;
...
string TestFunctionCall::formatBytesParameters(bytes const& _bytes, dev::polynomial::test::ParameterList const& _params) const
{
	...
	for (auto const& param: _params)
	{
		...
		poltestAssert(offsetIter <= _bytes.end(), "Byte range can not be extended past the end of given bytes.");
		...
		case ABIType::UnsignedDec:
			...
			// incorrectly.
			poltestAssert(param.abiType.align == ABIType::AlignRight, "Unsigned decimals must be right-aligned.");
		...
		case ABIType::SignedDec:
			poltestAssert(param.abiType.align == ABIType::AlignRight, "Signed decimals must be right-aligned.");
			if (*byteRange.begin() & 0x80)
				resultStream << u2s(fromBigEndian<u256>(byteRange));
			else
				resultStream << fromBigEndian<u256>(byteRange);
		...
		{
			poltestAssert(param.abiType.align == ABIType::AlignRight, "Booleans must be right-aligned.");
		...
		case ABIType::Hex:
			poltestAssert(param.abiType.align == ABIType::AlignLeft, "Hex numbers must be left-aligned.");
			byteRange.erase(
	...
	poltestAssert(it == _bytes.end(), "Parameter encoding too short for the given byte range.");
	return resultStream.str();
}

string TestFunctionCall::formatRawParameters(dev::polynomial::test::ParameterList const& _params, std::string const& _linePrefix) const
{
	stringstream resultStream;
	for (auto const& param: _params)
	{
