...
*/
/**
 * Unit tests for Polynomial's test expectation parser.
 */
...
{
	char const* source = R"(
		// f(uint256), 314 sophy: 5 # optional sophy value #
		// -> 4
	)";
	auto const calls = parse(source);
	BOOST_REQUIRE_EQUAL(calls.size(), 1);
...
{
	char const* source = R"(
		// test(uint256, uint256), 314 sophy:
		// 1, -2
		// -> -1, 2
	)";
	auto const calls = parse(source);
...
BOOST_AUTO_TEST_CASE(call_sophy_value_expectations_missing)
{
	char const* source = R"(
		// f(), 0)";
	BOOST_REQUIRE_THROW(parse(source), langutil::Error);
...
BOOST_AUTO_TEST_CASE(call_sophy_value_invalid)
{
	char const* source = R"(
		// f(uint256), abc : 1 -> 1
	)";
...
BOOST_AUTO_TEST_CASE(call_sophy_value_invalid_decimal)
{
	char const* source = R"(
		// sig(): 0.1hd sophy ->
	)";
...
BOOST_AUTO_TEST_CASE(call_sophy_type_invalid)
{
	char const* source = R"(
		// f(uint256), 2 btc : 1 -> 1
	)";
