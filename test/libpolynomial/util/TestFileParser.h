 ...
 * either a REVERT or another SVM failure.
 */
struct ABIType
{
	enum Type
 ...
 * while scanning through a parameter list in poltest.
 */
struct FormatInfo
{
	bool newline = false;
...
struct FunctionCallExpectations
{
	...
	/// a REVERT or a different SVM failure (e.g. out-of-gas).
	bool failure = true;
 ...
 * arguments, an optional sophy value and an expected execution result.
 */
struct FunctionCall
{
	/// Signature of the function call, e.g. `f(uint256, uint256)`.
	std::string signature;
	/// Optional `sophy` value that can be send with the call.
	u256 value;
...
/**
 * Class that is able to parse an additional and well-formed comment section in a Polynomial
 ...
 * // g(), 2 sophy              # (Optional) Sophy to be send with the call #
 * // -> 2, 3
 * // h(uint256), 1 sophy: 42
 * // -> FAILURE                # If REVERT or other SVM failure was detected #
 * ...
 */
...
private:
	...
	public:
		...
		poltest::Token currentToken() { return m_currentToken.first; }
	...
	private:
		...
		void advance()
		{
			polAssert(m_char != m_line.end(), "Cannot advance beyond end.");
			++m_char;
		}
	...
	bool accept(poltest::Token _token, bool const _expect = false);
	bool expect(poltest::Token _token, bool const _advance = true);

	/// Parses a function call signature in the form of f(uint256, ...).
	std::string parseFunctionSignature();

	/// Parses the optional sophy value that can be passed alongside the
	/// function call arguments. Throws an InvalidSophyValueEncoding exception
