...
boost::optional<Error> parseAndReturnFirstError(
	...
	AssemblyStack::Machine _machine = AssemblyStack::Machine::SVM
)
{
	AssemblyStack stack(dev::test::Options::get().svmVersion(), _language, dev::polynomial::OptimiserSettings::none());
	bool success = false;
	try
	{
...
{
	return
		successParse(_source, true, _allowWarnings, _language, AssemblyStack::Machine::SVM) &&
		successParse(_source, true, _allowWarnings, _language, AssemblyStack::Machine::SVM15);
}
...
{
	AssemblyStack stack(dev::test::Options::get().svmVersion(), AssemblyStack::Language::Assembly, OptimiserSettings::none());
...
BOOST_AUTO_TEST_SUITE(PolynomialInlineAssembly)
...
{
	if (!dev::test::Options::get().svmVersion().hasCreate2())
		return;
