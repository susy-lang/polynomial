...
BOOST_AUTO_TEST_CASE(invalid_language)
{
	...
	BOOST_CHECK(containsError(result, "JSONError", "Only \"Polynomial\" or \"Yul\" is supported as a language."));
}
...
BOOST_AUTO_TEST_CASE(basic_compilation)
{
	char const* input = R"(
	{
		...
		"settings": {
			"outputSelection": {
				"fileA": {
					"A": [ "abi", "devdoc", "userdoc", "svm.bytecode", "svm.assembly", "svm.gasEstimates", "svm.legacyAssembly", "metadata" ],
					"": [ "legacyAST" ]
				}
			}
		}
	...
	BOOST_CHECK_EQUAL(contract["svm"]["gasEstimates"].size(), 1);
	BOOST_CHECK(contract["svm"]["gasEstimates"]["creation"].isObject());
	BOOST_CHECK_EQUAL(contract["svm"]["gasEstimates"]["creation"].size(), 3);
	BOOST_CHECK(contract["svm"]["gasEstimates"]["creation"]["codeDepositCost"].isString());
	BOOST_CHECK(contract["svm"]["gasEstimates"]["creation"]["executionCost"].isString());
	BOOST_CHECK(contract["svm"]["gasEstimates"]["creation"]["totalCost"].isString());
	BOOST_CHECK_EQUAL(
		u256(contract["svm"]["gasEstimates"]["creation"]["codeDepositCost"].asString()) +
		u256(contract["svm"]["gasEstimates"]["creation"]["executionCost"].asString()),
		u256(contract["svm"]["gasEstimates"]["creation"]["totalCost"].asString())
	);
	...
	BOOST_CHECK(contract["svm"]["legacyAssembly"].isObject());
	BOOST_CHECK(contract["svm"]["legacyAssembly"][".code"].isArray());
	BOOST_CHECK_EQUAL(
		dev::jsonCompactPrint(contract["svm"]["legacyAssembly"][".code"]),
...
BOOST_AUTO_TEST_CASE(library_filename_with_colon)
{
	char const* input = R"(
	{
		...
		"sources": {
			"fileA": {
				"content": "import \"git:library.pol\"; contract A { function f() public returns (uint) { return L.g(); } }"
			},
			"git:library.pol": {
...
BOOST_AUTO_TEST_CASE(library_linking)
{
	char const* input = R"(
	{
		...
		"sources": {
			"fileA": {
				"content": "import \"library.pol\"; import \"library2.pol\"; contract A { function f() public returns (uint) { L2.g(); return L.g(); } }"
			},
			"library.pol": {
				"content": "library L { function g() public returns (uint) { return 1; } }"
			},
...
BOOST_AUTO_TEST_CASE(svm_version)
{
	...
	result = compile(inputForVersion("\"svmVersion\": \"petersburg\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].asString().find("\"svmVersion\":\"petersburg\"") != string::npos);
	// test default
...
BOOST_AUTO_TEST_CASE(use_stack_optimization)
{
	...
	dev::polynomial::StandardCompiler compiler;
	Json::Value result = compiler.compile(parsedInput);

	BOOST_CHECK(containsAtMostWarnings(result));
	Json::Value contract = getContractResult(result, "fileA", "A");
	BOOST_REQUIRE(contract.isObject());
	BOOST_REQUIRE(contract["svm"]["bytecode"]["object"].isString());
	BOOST_CHECK(contract["svm"]["bytecode"]["object"].asString().length() > 20);
...
BOOST_AUTO_TEST_CASE(standard_output_selection_wildcard)
{
	char const* input = R"(
	{
			...
			"sources":
		{
			"A":
			{
				"content": "pragma polynomial >=0.0; contract C { function f() public pure {} }"
			}
		},
		"settings":
		{
	...
	BOOST_REQUIRE(result["contracts"]["A"]["C"]["svm"].isObject());
	BOOST_REQUIRE(result["contracts"]["A"]["C"]["svm"]["bytecode"].isObject());
...
BOOST_AUTO_TEST_CASE(standard_output_selection_wildcard_empty_source)
{
	...
	BOOST_REQUIRE(result["contracts"][""]["C"]["svm"].isObject());
	BOOST_REQUIRE(result["contracts"][""]["C"]["svm"]["bytecode"].isObject());
	BOOST_REQUIRE(result["sources"].isObject());
	BOOST_REQUIRE(result["sources"].size() == 1);
	BOOST_REQUIRE(result["sources"][""].isObject());
}
...
BOOST_AUTO_TEST_CASE(standard_output_selection_wildcard_multiple_sources)
{
	char const* input = R"(
	{
		...
		"sources":
		{
			...
			"B":
			{
				"content": "pragma polynomial >=0.0; contract D { function f() public pure {} }"
			}
		},
		"settings":
		{
	...
	BOOST_REQUIRE(result["contracts"]["B"]["D"]["svm"].isObject());
	BOOST_REQUIRE(result["contracts"]["B"]["D"]["svm"]["bytecode"].isObject());
	BOOST_REQUIRE(result["sources"].isObject());
