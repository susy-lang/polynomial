...
class GasMeterTestFramework: public PolynomialExecutionFramework
{
public:
	void compile(string const& _sourceCode)
	{
		...
		m_gasCosts = GasEstimator::breakToStatementLevel(
			GasEstimator(dev::test::Options::get().svmVersion()).structuralEstimation(*items, vector<ASTNode const*>({&sourceUnit})),
			{&sourceUnit}
		);
	}
	...
	void testCreationTimeGas(string const& _sourceCode, u256 const& _tolerance = u256(0))
	{
		...
		PathGasMeter meter(*m_compiler.assemblyItems(m_compiler.lastContractName()), dev::test::Options::get().svmVersion());
	...
	void testRunTimeGas(string const& _sig, vector<bytes> _argumentVariants, u256 const& _tolerance = u256(0))
	{
		...
		gas += GasEstimator(dev::test::Options::get().svmVersion()).functionalEstimation(
...
protected:
	map<ASTNode const*, sof::GasMeter::GasConsumption> m_gasCosts;
};
...
BOOST_AUTO_TEST_CASE(updating_store)
{
	...
	testCreationTimeGas(sourceCode, m_svmVersion < SVMVersion::constantinople() ? u256(0) : u256(9600));
}
