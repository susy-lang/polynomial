 ...
 * @date 2014
 * Tests for the Polynomial optimizer.
 */
...
#include <libsvmasm/BlockDeduplicator.h>
...
public:
	...
	)
	{
		...
		sof::eachInstruction(nonOptimizedBytecode, [&](Instruction, u256 const&) {
			nonOptimizedSize++;
		});
		size_t optimizedSize = 0;
		sof::eachInstruction(optimizedBytecode, [&](Instruction, u256 const&) {
			optimizedSize++;
		});
		BOOST_CHECK_MESSAGE(
	...
	sof::KnownState createInitialState(AssemblyItems const& _input)
	{
		sof::KnownState state;
		for (auto const& item: addDummyLocations(_input))
			state.feedItem(item, true);
		return state;
	}

	AssemblyItems getCSE(AssemblyItems const& _input, sof::KnownState const& _state = sof::KnownState())
	{
		AssemblyItems input = addDummyLocations(_input);

		sof::CommonSubexpressionEliminator cse(_state);
	...
	void checkCSE(
		...
		KnownState const& _state = sof::KnownState()
	)
	{
...
BOOST_FIXTURE_TEST_SUITE(PolynomialOptimizer, OptimizerTestFramework)
...
BOOST_AUTO_TEST_CASE(retain_information_in_branches)
{
	...
	size_t numSHA3s = 0;
	sof::eachInstruction(optimizedBytecode, [&](Instruction _instr, u256 const&) {
		if (_instr == sof::Instruction::SHA3)
			numSHA3s++;
	});
...
{
	sof::KnownState state;
	sof::CommonSubexpressionEliminator cse(state);
	AssemblyItems input{
...
{
	sof::KnownState state = createInitialState(AssemblyItems{
		u256(0x12),
		u256(0x20),
		Instruction::ADD
	});
...
{
	sof::KnownState state = createInitialState(AssemblyItems{Instruction::DUP1});
	AssemblyItems input{
		Instruction::EQ
	};
...
BOOST_AUTO_TEST_CASE(block_deduplicator)
{
	AssemblyItems input{
		...
		AssemblyItem(PushTag, 3),
		u256(6),
		sof::Instruction::SWAP3,
		sof::Instruction::JUMP,
		AssemblyItem(Tag, 1),
		u256(6),
		sof::Instruction::SWAP3,
		sof::Instruction::JUMP,
...
BOOST_AUTO_TEST_CASE(block_deduplicator_loops)
{
	AssemblyItems input{
		u256(0),
		sof::Instruction::SLOAD,
		AssemblyItem(PushTag, 1),
		AssemblyItem(PushTag, 2),
		sof::Instruction::JUMPI,
		sof::Instruction::JUMP,
		AssemblyItem(Tag, 1),
		u256(5),
		u256(6),
		sof::Instruction::SSTORE,
		AssemblyItem(PushTag, 1),
		sof::Instruction::JUMP,
		AssemblyItem(Tag, 2),
		u256(5),
