...
BOOST_AUTO_TEST_CASE(metadata_stamp)
{
	...
	for (auto release: std::set<bool>{true, VersionIsRelease})
	{
		...
		compilerStack.setSVMVersion(dev::test::Options::get().svmVersion());
		...
		BOOST_CHECK(cborMetadata.count("polc") == 1);
		if (release)
			BOOST_CHECK(cborMetadata.at("polc") == toHex(VersionCompactBytes));
		else
			BOOST_CHECK(cborMetadata.at("polc") == VersionStringStrict);
