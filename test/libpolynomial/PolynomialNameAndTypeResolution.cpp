...
BOOST_FIXTURE_TEST_SUITE(PolynomialNameAndTypeResolution, AnalysisFramework)
...
BOOST_AUTO_TEST_CASE(dynamic_return_types_not_possible)
{
	...
	if (dev::test::Options::get().svmVersion() == SVMVersion::homestead())
...
BOOST_AUTO_TEST_CASE(create2_as_variable)
{
	...
	// This needs special treatment, because the message mentions the SVM version,
	// so cannot be run via ipoltest.
	vector<pair<Error::Type, std::string>> expectations(vector<pair<Error::Type, std::string>>{
		{Error::Type::Warning, "Variable is shadowed in inline assembly by an instruction of the same name"}
	});
...
BOOST_AUTO_TEST_CASE(extcodehash_as_variable)
{
	...
	if (!dev::test::Options::get().svmVersion().hasExtCodeHash())
