...
TestCase::TestResult YulOptimizerTest::run(ostream& _stream, string const& _linePrefix, bool const _formatted)
{
	...
	poltestAssert(m_dialect, "Dialect not set.");
	...
	{
		GasMeter meter(dynamic_cast<SVMDialect const&>(*m_dialect), false, 200);
		ConstantOptimiser{dynamic_cast<SVMDialect const&>(*m_dialect), meter}(*m_ast);
	}
...
void YulOptimizerTest::printUpdatedSettings(ostream& _stream, const string& _linePrefix, const bool _formatted)
{
	...
	SVMVersionRestrictedTestCase::printUpdatedSettings(_stream, _linePrefix, _formatted);
}
...
bool YulOptimizerTest::parse(ostream& _stream, string const& _linePrefix, bool const _formatted)
{
	AssemblyStack stack(
		...
		dev::polynomial::OptimiserSettings::none()
	);
	...
	m_dialect = m_yul ? &Dialect::yul() : &SVMDialect::strictAssemblyForSVMObjects(dev::test::Options::get().svmVersion());
