...
BOOST_AUTO_TEST_CASE(break_outside_of_for_loop)
{
	CHECK_ERROR_DIALECT(
		...
		SVMDialect::strictAssemblyForSVMObjects(SVMVersion::constantinople())
	);
}
...
{
	auto const& dialect = SVMDialect::strictAssemblyForSVMObjects(SVMVersion::constantinople());
...
{
	auto const& dialect = SVMDialect::strictAssemblyForSVMObjects(SVMVersion{});
