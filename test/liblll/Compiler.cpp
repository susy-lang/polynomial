...
bool successCompile(string const& _sourceCode)
{
	...
	bytes bytecode = lll::compileLLL(_sourceCode, dev::test::Options::get().svmVersion(), false, &errors);
	if (!errors.empty())
		return false;
	if (bytecode.empty())
		return false;
...
BOOST_AUTO_TEST_CASE(valid_opcodes_functional)
{
	...
	{
		vector<string> errors;
		bytes code = lll::compileLLL(opcodes_lll[i], dev::test::Options::get().svmVersion(), false, &errors);
