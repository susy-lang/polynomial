...
bool successCompile(string const& _sourceCode)
{
	...
	bytes bytecode = sof::compileLLL(_sourceCode, dev::test::Options::get().svmVersion(), false, &errors);
	if (!errors.empty())
		return false;
	if (bytecode.empty())
		return false;
...
BOOST_AUTO_TEST_CASE(valid_opcodes_functional)
{
	...
	for (size_t i = 0; i < opcodes_bytecode.size(); i++) {
		vector<string> errors;
		bytes code = sof::compileLLL(opcodes_lll[i], dev::test::Options::get().svmVersion(), false, &errors);
