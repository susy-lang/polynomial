 ...
 * @date 2017
 * Tests for an SRC20 token implementation written in LLL
 */
...
static char const* src20Code = R"DELIMITER(
(seq
  ...
  ;;
  ;; Note: The REVERT opcode will be implemented in Metropolis (SIP 140).
  ;;   Meanwhile it just causes an invalid instruction exception (similar
  ;;   to a "throw" in Polynomial).  When fully implemented, Revert could be
  ...
  ;;   number of arguments. For one thing this addresses the "SRC20
  ...
  ;; --------------------------------------------------------------------------
  ;; Will revert if sent any Sophy. We use the macro immediately so as
  ;;   to abort if sent any Sophy during contract deployment.
...
static unique_ptr<bytes> s_compiledSrc20;

class LLLSRC20TestFramework: public LLLExecutionFramework
{
protected:
	void deploySrc20()
	{
...
// Test suite for an SRC20 contract written in LLL.
BOOST_FIXTURE_TEST_SUITE(LLLSRC20, LLLSRC20TestFramework)
...
BOOST_AUTO_TEST_CASE(transfer_from)
{
	...
	// Send account(1) some sophy for gas.
	sendSophy(account(1), 1000 * sophy);
	BOOST_REQUIRE(balanceAt(account(1)) >= 1000 * sophy);
