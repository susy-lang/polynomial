...
;;; ---------------------------------------------------------------------------
;;; @title The Sophon Name Service registry.
...
(seq
  ...
  ;; Jumping here causes an SVM error.
...
BOOST_AUTO_TEST_CASE(create_subnode)
{
	...
	// Set ownership of "sof" sub-node to account(1)
	BOOST_REQUIRE(callContractFunction("setSubnodeOwner(bytes32,bytes32,address)", 0x00, keccak256(string("sof")), ACCOUNT(1)) == encodeArgs());
	...
	BOOST_CHECK(m_logs[0].topics[1] == u256(0x00));
	BOOST_CHECK(m_logs[0].topics[2] == keccak256(string("sof")));

	// Verify that the sub-node owner is now account(1).
	u256 namehash = keccak256(h256(0x00).asBytes() + keccak256("sof").asBytes());
...
BOOST_AUTO_TEST_CASE(create_subnode_fail)
{
	...
	// account(1) tries to set ownership of the "sof" sub-node.
	m_sender = account(1);
	BOOST_REQUIRE(callContractFunction("setSubnodeOwner(bytes32,bytes32,address)", 0x00, keccak256(string("sof")), ACCOUNT(1)) == encodeArgs());
