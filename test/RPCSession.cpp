...
*/
/// @file RPCSession.cpp
/// Low-level IPC communication between the test framework and the Sophon node.
...
{
	return rpcCall("sof_getCode", { quote(_address), quote(_blockNumber) }).asString();
}

Json::Value RPCSession::sof_getBlockByNumber(string const& _blockNumber, bool _fullObjects)
{
	// NOTE: to_string() converts bool to 0 or 1
	return rpcCall("sof_getBlockByNumber", { quote(_blockNumber), _fullObjects ? "true" : "false" });
}

RPCSession::TransactionReceipt RPCSession::sof_getTransactionReceipt(string const& _transactionHash)
{
	TransactionReceipt receipt;
	Json::Value const result = rpcCall("sof_getTransactionReceipt", { quote(_transactionHash) });
...
string RPCSession::sof_sendTransaction(TransactionData const& _td)
{
	return rpcCall("sof_sendTransaction", { _td.toJson() }).asString();
}

string RPCSession::sof_call(TransactionData const& _td, string const& _blockNumber)
{
	return rpcCall("sof_call", { _td.toJson(), quote(_blockNumber) }).asString();
}

string RPCSession::sof_sendTransaction(string const& _transaction)
{
	return rpcCall("sof_sendTransaction", { _transaction }).asString();
}

string RPCSession::sof_getBalance(string const& _address, string const& _blockNumber)
{
	string address = (_address.length() == 20) ? "0x" + _address : _address;
	return rpcCall("sof_getBalance", { quote(address), quote(_blockNumber) }).asString();
}

string RPCSession::sof_getStorageRoot(string const& _address, string const& _blockNumber)
{
	string address = (_address.length() == 20) ? "0x" + _address : _address;
	return rpcCall("sof_getStorageRoot", { quote(address), quote(_blockNumber) }).asString();
}

string RPCSession::sof_gasPrice()
{
	return rpcCall("sof_gasPrice").asString();
}
...
{
	u256 startBlock = fromBigEndian<u256>(fromHex(rpcCall("sof_blockNumber").asString()));
	BOOST_REQUIRE(rpcCall("test_mineBlocks", { to_string(_number) }, true) == true);
	BOOST_REQUIRE(fromBigEndian<u256>(fromHex(rpcCall("sof_blockNumber").asString())) == startBlock + _number);
}
