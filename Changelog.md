...
Build System:
 * Poltest: Add commandline option `--test` / `-t` to ipoltest which takes a string that allows filtering unit tests.
 * poltest.sh: allow environment variable ``POLYNOMIAL_BUILD_DIR`` to specify build folder and add ``--help`` usage.
 ...
 * Poltest: Fix hex string update in poltest.
...
Build System:
 * Poltest: Add support for arrays in function signatures.
 * Poltest: Add support for struct arrays in function signatures.
 * Poltest: Add support for left-aligned, unpadded hex string literals.
...
Compiler Features:
 * Support ``petersburg`` as ``svmVersion`` and set as default.
 ...
 * Inline Assembly: Instructions unavailable to the currently configured SVM are errors now.
...
Build System:
 * Poltest: Add support for left-aligned, padded hex literals.
 * Poltest: Add support for right-aligned, padded boolean literals.
 ...
 * C API (``libpolc`` / raw ``poljson.js``): Introduce ``polynomial_free`` method which releases all internal buffers to save memory.
 ...
 * Poltest: Add parser that is used in the file-based unit test environment.
...
Bugfixes:
 * Emscripten: Split simplification rule initialization up further to work around issues with poljson.js in some browsers.
 ...
 * C API (``libpolc`` / raw ``poljson.js``): Removed the ``version``, ``license``, ``compileSingle``, ``compileJSON``, ``compileJSONCallback`` methods
   and replaced them with the ``polynomial_license``, ``polynomial_version`` and ``polynomial_compile`` methods.
 ...
 * Remove obsolete ``std`` directory from the Polynomial repository. This means accessing ``https://octonion.institute/susy-lang/polynomial/blob/develop/std/*.pol`` (or ``https://octonion.institute/susy-lang/polynomial/std/*.pol`` in Fourier) will not be possible.
 ...
 * Code Generator: ``CREATE2`` instruction has been updated to match SIP1014 (aka "Skinny CREATE2"). It also is accepted as part of Constantinople.
 * Code Generator: ``EXTCODEHASH`` instruction has been added based on SIP1052.
 ...
 * Tests: Fix chain parameters to make ipc tests work with newer versions of cpp-sophon.
 ...
 * General: Support accessing dynamic return data in post-byzantium SVMs.
 ...
 * Commandline interface: Support ``--svm-version constantinople`` properly.
 ...
 * Standard JSON: Support ``constantinople`` as ``svmVersion`` properly.
 ...
 * Code Generator: Do not retain any gas in calls (except if SVM version is set to homestead).
 ...
 * Interface: Provide ability to select target SVM version (homestead or byzantium, with byzantium being the default).
 ...
 * Resolver: Continue repolving references after the first error.
 ...
 * Build System: Shared libraries (``libdevcore``, ``libsvmasm``, ``libpolynomial``
...
Features:
 * Assembly: Add ``CREATE2`` (SIP86), ``STATICCALL`` (SIP214), ``RETURNDATASIZE`` and ``RETURNDATACOPY`` (SIP211) instructions.
 ...
 * Inline Assembly: Present proper error message when not supplying enough arguments to a functional
   instruction.
 * Inline Assembly: Warn when instructions shadow Polynomial variables.
 ...
 * Compiler interface: Fix a bug where source indexes could be inconsistent between Polynomial compiled
 ...
 * Introduce ``.transfer(value)`` for sending Sophy.
 * Code generator: Support ``revert()`` to abort with rolling back, but not consuming all gas.
 * Inline assembly: Support ``revert`` (SIP140) as an opcode.
 ...
 * Inline assembly: introduce ``invalid`` (SIP141) as an opcode.
 ...
 * Code generator: Internal errors (array out of bounds, etc.) now cause a reversion by using an invalid
   instruction (0xfe - SIP141) instead of an invalid jump. Invalid jump is still kept for explicit throws.
 ...
 * Output: Print assembly in new standardized Polynomial assembly format.
 ...
 * Running `polc` without arguments on a terminal will print help.
 ...
 * Import similar to ECMAScript6 import (`import "abc.pol" as d` and `import {x, y} from "abc.pol"`).
 * Commandline compiler polc automatically resolves missing imports and allows for "include directories".
 * Conditional: `x ? y : z`
 ...
 * Bugfix: combined-json output of polc.
 ...
 * Version string includes libsvmasm/libsophon's version (contains the optimizer).
